...
export async function detectKbUpgrade(name) {
  ...
  if (await dirExists(path.join(dir, 'principles'))) {
    items.push({ id: 'legacy-principles', reason: 'principles/ 目录存在（pre-Holy/Eden 布局）' });
  }
  ...
  if (meta) {
    if (Number.isFinite(meta.parserVersion)) {
      ...
      if (meta.parserVersion < PARSER_VERSION) {
        items.push({
          id: 'parser-version',
          reason: `parserVersion ${meta.parserVersion} < ${PARSER_VERSION}（符号 shard 由旧解析器产出）`,
    ...
    if (sameList(meta.includeGlobs, LEGACY_DEFAULT_INCLUDE_40)) {
      items.push({
        ...
        reason: 'includeGlobs 为旧版 40 项默认列表（缺 sgml/pdf/office 文档格式）',
      });
    }
  ...
  if (!supreme) {
    items.push({ id: 'supreme-code-missing', reason: '缺少 hk2-supreme-code 条目' });
  }
  ...
  if (!docInputsUsable && (hasDocs || fileExists(path.join(dir, 'doc_index.json')))) {
    items.push({
      id: 'doc-graph-empty',
      reason: 'doc_index.json 缺失或 _docInputs 为空（doc 引用图不可增量合并）',
    });
  }
