...
/**
 * KB storage path management. KB data lives under $HK2_KB_DIR/<projectId>/
 * (default $HK2_HOME/kb/<projectId>/; HK2_KB_DIR can relocate the root).
 *
 * Three-space layout:
 *   holy/      — Holy Space: stable knowledge (design principles, key algorithms).
 *                Confirmation is NOT enforced here: it belongs to the command
 *                / agent write path (explicit commands are the user's own
 *                intent; agent-proposed Holy writes confirm elsewhere). The
 *                storage layer's own hard protection covers the Supreme Code
 *                entry itself.
 ...
 *
 * History: v1 stored data under <hk2cli>/.kb/<name>/; v2 moved to
 * $HK2_KB_DIR/ (default $HK2_HOME/kb/).
...
export function symbolsPath(name, fileId) {
  ...
  const shardId = String(shardNum).padStart(4, '0');
  return path.join(kbDir(name), `symbols.${shardId}.json`);
}
...
export async function readSymbolsShard(name, shardNum) {
  const shardId = String(shardNum).padStart(4, '0');
  const p = path.join(kbDir(name), `symbols.${shardId}.json`);
...
export async function listSymbolShards(name) {
  ...
  const shards = [];
  for (const ent of entries) {
    const m = /^symbols\.([0-9]{4,})\.json$/.exec(ent);
    if (m) shards.push({ shardNum: parseInt(m[1], 10), path: path.join(kbDir(name), ent) });
  }
