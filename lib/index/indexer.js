...
import {
  ...
  writeInverted, writeCallgraph, writeSymbolsShard, readSymbolsShard, listSymbolShards, symbolsPath,
 ...
 * @param {number} [opts.checkpointInterval=100]  Save when mark counter reaches N;
 *                                                   invalid/<=0 values are not normalized here
...
export async function buildIndex(kbName, opts = {}) {
  ...
  // Keep the actual source of each logical index path. Named roots may
  // overlap a primary-root directory name, so metadata alone is ambiguous.
  const sourcePaths = Object.create(null);
  for (const f of allFiles) {
    if (sourcePaths[f.path] && sourcePaths[f.path] !== f.absPath) {
      throw new Error(`ambiguous indexed path ${f.path}: ${sourcePaths[f.path]} and ${f.absPath}`);
    }
    sourcePaths[f.path] = f.absPath;
  }
  filesIndex.sourcePaths = sourcePaths;

  // Reconcile only after the entire scan succeeds. Deleted, renamed and
  // newly excluded paths must not retain symbols in any derived index.
  const currentPaths = new Set(allFiles.map(f => f.path));
  for (const [filePath, id] of Object.entries(filesIndex.byPath)) {
    if (currentPaths.has(filePath)) continue;
    delete filesIndex.byPath[filePath];
    delete filesIndex.byId[id];
  }

  // Checkpoint setup
  const cp = new Checkpoint(kbName, {
    interval: resolveIndexerCheckpointInterval(opts, process.env),
  ...
  for (const f of allFiles) {
    ...
    // Parse checkpoints precede durable output. Only a checkpoint published
    // after all artifacts and the file registry were committed can skip work.
    if (cp.phase === 'committed' && cp.has(f.path, hash)
      && filesIndex.byId[existing]?.hash === hash) {
      cpHits.add(f.path);
    ...
    const uncommitted = cp.phase !== 'committed' && cp.has(f.path, hash);
    if (!opts.full && !uncommitted && existing !== undefined && filesIndex.byId[existing] && filesIndex.byId[existing].hash === hash) {
      if (skippedTrackedDocs.delete(f.path)) toParse.push({ ...f, hash });
      continue;
    }
    toParse.push({ ...f, hash });
  ...
  const parseResults = await asyncPool(toParse, async (f) => {
  ...
  // In particular, a failed checkpoint write must not become a successful build.
  const parseFailure = parseResults.find(result => result?.__error);
  if (parseFailure) throw parseFailure.__error;
  ...
  for (const entry of docEntries) {
    await writeKnowledge(kbName, 'eden', entry);
  }

  // Publish hashes only after the corresponding symbols and documents exist.
  // A crash before this point leaves the old hashes, forcing a safe re-parse.
  ...
  // Only this phase certifies durable file, symbol and document artifacts.
  cp.phase = 'committed';
...
/** Resolve the direct buildIndex checkpoint input without normalizing it. */
export function resolveIndexerCheckpointInterval(opts = {}, env = process.env) {
  return opts.checkpointInterval ?? parseInt(env.HK2_KB_CHECKPOINT_INTERVAL || '100', 10);
}
...
async function rewriteAllShards(kbName, filesIndex, newSymbolsByFile) {
  ...
  // Runtime loading enumerates every shard on disk, so unused shards must
  // disappear as well (including the last shard of a now-empty project).
  const activePaths = new Set(Object.keys(shardToFiles)
    .map(n => symbolsPath(kbName, Number(n) * 256)));
  for (const shard of await listSymbolShards(kbName)) {
    if (!activePaths.has(shard.path)) await fs.unlink(shard.path);
  }
}
