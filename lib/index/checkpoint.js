 ...
 *
 * Path: $HK2_KB_DIR/<kbName>/checkpoint.json (default $HK2_HOME/kb/<kbName>/)
 *
 * Format:
 *   {
 *     phase: 'parse' | 'committed',
 ...
 *   await cp.saveIfDue(cb)    # saves when counter >= interval; 0, negative, or NaN
 *                              therefore save on nearly every markDone
 ...
 * re-parses files whose output was not committed. Only phase='committed'
 * certifies that matching processedFiles have durable parsing artifacts;
 * legacy/parse checkpoints are progress records, not permission to skip.
 */
