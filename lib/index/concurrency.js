 ...
 * @returns {number}                    并行度，>= 1。注意：env 字符串路径只接受
 *                                    严格整数字符串（正则先拒绝小数/科学计数法，
 *                                    不会走到 floor）；floor 仅适用于显式
 *                                    deps.concurrency 数值参数
 */
export function resolveIndexConcurrency(deps = {}) {
  ...
  if (raw !== undefined && raw !== '') {
    ...
    // parseInt=1, which is surprising) and decimal notation before parsing.
