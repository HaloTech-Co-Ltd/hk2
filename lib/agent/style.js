/*-------------------------------------------------------------------------
 *
 * 版权所有 (c) 2019-2026, 易景科技保留所有权利。
 * Copyright (c) 2019-2026, Halo Tech Co.,Ltd. All rights reserved.
 *
 * 易景科技是Halo Database、Halo Database Management System、羲和数据
 * 库、羲和数据库管理系统（后面简称 Halo ）软件的发明人同时也为知识产权权
 * 利人。Halo 软件的知识产权，以及与本软件相关的所有信息内容（包括但不限
 * 于文字、图片、音频、视频、图表、界面设计、版面框架、有关数据或电子文档等）
 * 均受中华人民共和国法律法规和相应的国际条约保护，易景科技享有上述知识产
 * 权，但相关权利人依照法律规定应享有的权利除外。未免疑义，本条所指的"知识
 * 产权"是指任何及所有基于 Halo 软件产生的：（a）版权、商标、商号、域名、与
 * 商标和商号相关的商誉、设计和专利；与创新、技术诀窍、商业秘密、保密技术、非
 * 技术信息相关的权利；（b）人身权、掩模作品权、署名权和发表权；以及（c）在
 * 本协议生效之前已存在或此后出现在世界任何地方的其他工业产权、专有权、与"知
 * 识产权"相关的权利，以及上述权利的所有续期和延长，无论此类权利是否已在相
 * 关法域内的相关机构注册。
 *
 * This software and related documentation are provided under a license
 * agreement containing restrictions on use and disclosure and are
 * protected by intellectual property laws. Except as expressly permitted
 * in your license agreement or allowed by law, you may not use, copy,
 * reproduce, translate, broadcast, modify, license, transmit, distribute,
 * exhibit, perform, publish, or display any part, in any form, or by any
 * means. Reverse engineering, disassembly, or decompilation of this
 * software, unless required by law for interoperability, is prohibited.
 *
 * This software is developed for general use in a variety of
 * information management applications. It is not developed or intended
 * for use in any inherently dangerous applications, including applications
 * that may create a risk of personal injury. If you use this software or
 * in dangerous applications, then you shall be responsible to take all
 * appropriate fail-safe, backup, redundancy, and other measures to ensure
 * its safe use. Halo Corporation and its affiliates disclaim any
 * liability for any damages caused by use of this software in dangerous
 * applications.
 *
 *-------------------------------------------------------------------------
 */

/**
 * Themed output layer for hk2's REPL.
 *
 * Provides:
 *   - Color palette (titanium-inspired dark theme).
 *   - Unicode box-drawing (rounded corners) for tool-call cards and banners.
 *   - Braille spinner frames, ✓/✗ status icons, · bullet separator.
 *   - Helpers to render "cards": bordered frames with a title embedded in the
 *     top border, header line, body, status footer, bottom border.
 *
 * Color mode is chosen once per process. Callers do not need to pick — every
 * helper degrades gracefully. Mode resolution: NO_COLOR or
 * TERM=dumb → none; COLORTERM=truecolor|24bit → truecolor; WT_SESSION →
 * truecolor; TERM=linux or empty → 256; ANY other TERM (including
 * xterm-256color) falls through to truecolor. Override with a non-empty
 * HK2_NO_COLOR (any value, even "0", disables color).
 */

const NO_COLOR = !!process.env.HK2_NO_COLOR || process.env.NO_COLOR;
const TERM = process.env.TERM || '';

function detectColorMode() {
  if (NO_COLOR || TERM === 'dumb') return 'none';
  const ct = (process.env.COLORTERM || '').toLowerCase();
  if (ct === 'truecolor' || ct === '24bit') return 'truecolor';
  if (process.env.WT_SESSION) return 'truecolor';
  if (TERM === 'linux' || TERM === '') return '256';
  return 'truecolor';
}

const MODE = detectColorMode();

/**
 * Palette. Titanium-inspired hex values; ANSI-256 fallbacks chosen to
 * approximate the same hue/brightness on terminals without truecolor.
 */
const HEX = {
  accent:    '#00b4ff',  // electric blue
  muted:     '#9ca3b0',  // dim aluminum
  dim:       '#6b7280',  // darker gray
  success:   '#00ff88',  // readout green
  error:     '#ff4757',  // alert red
  warning:   '#ffb347',  // amber
  border:    '#2a3038',  // subtle gray
  bashMode:  '#00ff88',  // green (same as success)
  pythonMode:'#f0c040',  // warm yellow
};
const ANSI256 = {
  accent: 39, muted: 245, dim: 240, success: 84, error: 203,
  warning: 215, border: 236, bashMode: 84, pythonMode: 221,
};

/**
 * Detect whether the terminal likely renders UTF-8 box-drawing and symbol
 * characters. When false, ICON / BOX / SPINNER fall back to ASCII so we
 * don't show `��` replacement glyphs on Windows cmd.exe or any terminal
 * with a non-UTF-8 codepage. Override with HK2_ASCII=1 to force.
 */
function detectUtf8() {
  if (process.env.HK2_ASCII) return false;
  const ct = (process.env.LC_ALL || process.env.LC_CTYPE || process.env.LANG || '').toLowerCase();
  if (ct.includes('utf-8') || ct.includes('utf8')) return true;
  // macOS & most Linux distros default to UTF-8.
  if (process.platform === 'darwin' || process.platform === 'linux') return true;
  // Windows: only assume UTF-8 for known-modern terminals.
  if (process.platform === 'win32') {
    return !!(process.env.WT_SESSION || process.env.TERM_PROGRAM === 'vscode' ||
      process.env.TERM === 'xterm-256color');
  }
  return false;
}
const UTF8 = detectUtf8();

/** Box-drawing glyphs. Rounded preset under UTF-8, ASCII fallback otherwise. */
export const BOX = UTF8 ? {
  topLeft: '╭', topRight: '╮',
  bottomLeft: '╰', bottomRight: '╯',
  horizontal: '─', vertical: '│',
  teeDown: '┬', teeUp: '┴',
} : {
  topLeft: '+', topRight: '+',
  bottomLeft: '+', bottomRight: '+',
  horizontal: '-', vertical: '|',
  teeDown: '-', teeUp: '-',
};

/** Spinner frames. Braille under UTF-8, ASCII fallback otherwise. */
export const SPINNER = UTF8
  ? ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏']
  : ['|', '/', '-', '\\', '|', '/', '-', '\\'];

/** Status icons. Unicode under UTF-8, ASCII fallback otherwise. */
export const ICON = UTF8 ? {
  ok: '✓', err: '✗', warn: '⚠', bullet: '•', dot: '·', arrow: '→', ellipsis: '…',
  up: '↑', down: '↓',
} : {
  ok: '+', err: 'x', warn: '!', bullet: '*', dot: '.', arrow: '->', ellipsis: '...',
  up: '^', down: 'v',
};

/** Heavy underline used under H1/H2 in markdown rendering (or ASCII '='). */
export const HEADING_RULE = UTF8 ? '━' : '=';

/** Whether full Unicode is available (for callers that need to gate features). */
export const HAS_UTF8 = UTF8;

const RESET = '\x1b[0m';

function hexToRgb(hex) {
  const m = /^#?([0-9a-f]{6})$/i.exec(hex || '');
  if (!m) return [0, 0, 0];
  const v = parseInt(m[1], 16);
  return [(v >> 16) & 0xff, (v >> 8) & 0xff, v & 0xff];
}

// ---------------------------------------------------------------------------
// Theme overrides
// ---------------------------------------------------------------------------
//
// applyTheme() installs runtime color overrides consulted by paint() BEFORE
// the built-in palette. Color values accept three forms:
//   '#rrggbb'  — truecolor hex, quantized to the nearest ANSI-256 index so
//                256-color terminals still approximate the hue
//   'ansi:N'   — explicit ANSI-256 index (0-255)
//   '<token>'  — a built-in token name (accent / muted / dim / success /
//                error / warning / border / bashMode / pythonMode) resolved
//                from the built-in palette
// Invalid values are skipped with a warning; paint() keeps its built-in color.

const OVERRIDES = new Map(); // token -> { hex, ansi256 }
let TITLE_FOLLOWS = false;   // topBorder() title painted with the border token

// Standard xterm 16-color palette for ansi:0-15 (terminal-dependent in
// practice, but a fixed table keeps output deterministic).
const XTERM16 = [
  '#000000', '#800000', '#008000', '#808000', '#000080', '#800080', '#008080', '#c0c0c0',
  '#808080', '#ff0000', '#00ff00', '#ffff00', '#0000ff', '#ff00ff', '#00ffff', '#ffffff',
];

/** Quantize an RGB triple to the nearest ANSI-256 index (6x6x6 cube + grayscale ramp). */
export function rgbToAnsi256(r, g, b) {
  r = Math.max(0, Math.min(255, r | 0));
  g = Math.max(0, Math.min(255, g | 0));
  b = Math.max(0, Math.min(255, b | 0));
  if (r === g && g === b) {
    if (r < 8) return 16;
    if (r > 248) return 231;
    return Math.round(((r - 8) / 247) * 24) + 232;
  }
  return 16 + 36 * Math.round((r / 255) * 5) + 6 * Math.round((g / 255) * 5) + Math.round((b / 255) * 5);
}

/** Inverse of rgbToAnsi256 for indices 16-255 (cube + grayscale); 0-15 via the xterm table. */
export function ansi256ToRgb(n) {
  n = Math.max(0, Math.min(255, n | 0));
  if (n < 16) return hexToRgb(XTERM16[n]);
  if (n < 232) {
    const v = n - 16;
    const level = (x) => (x === 0 ? 0 : Math.round((x * 255) / 5));
    return [level(Math.floor(v / 36)), level(Math.floor((v % 36) / 6)), level(v % 6)];
  }
  const gray = Math.min(255, (n - 232) * 10 + 8);
  return [gray, gray, gray];
}

/**
 * Resolve a user-facing color value to { hex, ansi256 }, or null when invalid.
 * Accepts '#rrggbb', 'ansi:N' (0-255), and built-in token names.
 */
export function resolveColorValue(value) {
  const v = String(value || '').trim();
  if (!v) return null;
  if (/^#[0-9a-f]{6}$/i.test(v)) {
    const [r, g, b] = hexToRgb(v);
    return { hex: v.toLowerCase(), ansi256: rgbToAnsi256(r, g, b) };
  }
  const m = /^ansi:(\d{1,3})$/i.exec(v);
  if (m) {
    const n = Number(m[1]);
    if (!Number.isInteger(n) || n < 0 || n > 255) return null;
    const [r, g, b] = ansi256ToRgb(n);
    const hex = '#' + [r, g, b].map((x) => x.toString(16).padStart(2, '0')).join('');
    return { hex, ansi256: n };
  }
  if (HEX[v] && Number.isInteger(ANSI256[v])) {
    return { hex: HEX[v], ansi256: ANSI256[v] };
  }
  return null;
}

/**
 * Install theme overrides for paint(). `overrides` maps a token name (any
 * string callers paint() with, e.g. 'tool:bash') to a color value accepted
 * by resolveColorValue(). Passing `overrides` REPLACES the whole previous
 * override set; passing null leaves colors untouched. `titleFollowsBorder`
 * makes the title embedded in topBorder() use the border token color
 * instead of the fixed muted hue.
 *
 * @returns {{ applied: string[], warnings: Array<{token: string, value: string, reason: string}> }}
 */
export function applyTheme({ overrides = null, titleFollowsBorder = null } = {}) {
  const applied = [];
  const warnings = [];
  if (overrides === null && titleFollowsBorder === null) return { applied, warnings };
  if (overrides !== null) {
    OVERRIDES.clear();
    for (const [token, value] of Object.entries(overrides || {})) {
      const resolved = resolveColorValue(value);
      if (!resolved) {
        warnings.push({ token, value, reason: 'expected #rrggbb, ansi:0-255, or a built-in token name' });
        continue;
      }
      OVERRIDES.set(token, resolved);
      applied.push(token);
    }
  }
  if (titleFollowsBorder !== null) TITLE_FOLLOWS = !!titleFollowsBorder;
  return { applied, warnings };
}

/** Drop all overrides and reset the title-follow flag (back to built-ins). */
export function clearThemeOverrides() {
  OVERRIDES.clear();
  TITLE_FOLLOWS = false;
}

/**
 * Register ONE override entry without touching the rest (merge semantics —
 * unlike applyTheme, which replaces the whole set). Returns false and leaves
 * state untouched when the value is invalid. Used for ad-hoc swatches.
 */
export function setThemeOverride(token, value) {
  const resolved = resolveColorValue(value);
  if (!resolved) return false;
  OVERRIDES.set(token, resolved);
  return true;
}

/** Introspection for /theme list: resolved color of a token (override first). */
export function getTokenColor(token) {
  const ov = OVERRIDES.get(token);
  if (ov) return { ...ov, override: true };
  if (HEX[token] && Number.isInteger(ANSI256[token])) {
    return { hex: HEX[token], ansi256: ANSI256[token], override: false };
  }
  return null;
}

/** Whether topBorder() titles are painted with the border token color. */
export function isTitleFollowsBorder() {
  return TITLE_FOLLOWS;
}

/**
 * Wrap `text` in the named color token. Tokens: accent / muted / dim / success
 * / error / warning / border / bashMode / pythonMode. Honours embedded ANSI
 * (the closing reset restores to the named color, not to default — so colored
 * spans inside stay inside the token color until the caller's RESET).
 */
export function paint(token, text) {
  if (MODE === 'none' || !text) return text;
  const ov = OVERRIDES.get(token);
  if (ov) {
    if (MODE === 'truecolor') {
      const [r, g, b] = hexToRgb(ov.hex);
      return `\x1b[38;2;${r};${g};${b}m${text}${RESET}`;
    }
    return `\x1b[38;5;${ov.ansi256}m${text}${RESET}`;
  }
  if (MODE === 'truecolor') {
    const [r, g, b] = hexToRgb(HEX[token] || HEX.muted);
    const open = `\x1b[38;2;${r};${g};${b}m`;
    return `${open}${text}${RESET}`;
  }
  const code = ANSI256[token] ?? ANSI256.muted;
  return `\x1b[38;5;${code}m${text}${RESET}`;
}

export const accent    = (t) => paint('accent', t);
export const muted     = (t) => paint('muted', t);
export const dim       = (t) => paint('dim', t);
export const success   = (t) => paint('success', t);
export const errorT    = (t) => paint('error', t);
export const warning   = (t) => paint('warning', t);
export const border    = (t) => paint('border', t);
export const bashMode  = (t) => paint('bashMode', t);

export function bold(text) {
  if (MODE === 'none') return text;
  return `\x1b[1m${text}${RESET}`;
}
export function italic(text) {
  if (MODE === 'none') return text;
  return `\x1b[3m${text}${RESET}`;
}

/** Terminal width in columns (best-effort; fallback 80). */
export function termWidth() {
  return process.stdout.columns || process.stderr.columns || 80;
}

/**
 * Terminal column width of a single code point: 2 for CJK / Hangul / wide
 * punctuation / emoji-plane ranges, 1 otherwise. Approximate (BMP range
 * check), good enough for the box-drawing glyphs and CJK we actually emit.
 * Exported so the TUI InputBox's wrap/cursor math measures with the exact
 * same table visibleWidth() uses — cursor positioning and text measurement
 * can never disagree.
 */
export function charWidth(cp) {
  return cp >= 0x1100 && (
    (cp <= 0x115F) || (cp >= 0x2E80 && cp <= 0xA4CF && cp !== 0x303F) ||
    (cp >= 0xAC00 && cp <= 0xD7A3) || (cp >= 0xF900 && cp <= 0xFAFF) ||
    (cp >= 0xFE30 && cp <= 0xFE4F) || (cp >= 0xFF00 && cp <= 0xFF60) ||
    (cp >= 0xFFE0 && cp <= 0xFFE6) || (cp >= 0x1F300 && cp <= 0x1FAFF)
  ) ? 2 : 1;
}

/**
 * Split text into user-perceived characters (grapheme clusters), NOT code
 * points: 'e\u0301', ZWJ emoji families, and skin-tone modifiers are ONE
 * cluster each. Falls back to Array.from (code points) where
 * Intl.Segmenter is unavailable.
 */
export function graphemes(str) {
  const s = String(str ?? '');
  if (typeof Intl !== 'undefined' && Intl.Segmenter) {
    if (!_SEG) _SEG = new Intl.Segmenter(undefined, { granularity: 'grapheme' });
    return [..._SEG.segment(s)].map(x => x.segment);
  }
  return Array.from(s);
}
let _SEG = null;

/**
 * Display width of one grapheme cluster: the sum of its code points'
 * charWidth, but combining marks (Mn/Mc categories approximated by the
 * ranges below) and ZWJ/variation selectors count 0, so 'e\u0301' = 1
 * and '\u{1F468}\u200D...' families = 2.
 */
export function graphemeWidth(cluster) {
  let w = 0;
  for (const ch of Array.from(cluster)) {
    const cp = ch.codePointAt(0);
    if (cp === 0x200D) continue;                      // ZWJ
    if (cp >= 0xFE00 && cp <= 0xFE0F) continue;      // variation selectors
    if (cp >= 0x1F3FB && cp <= 0x1F3FF) continue;    // skin-tone modifiers
    if (cp >= 0x0300 && cp <= 0x036F) continue;      // combining diacritics
    w += charWidth(cp);
  }
  // A family emoji spans 2 columns no matter how many ZWJ-joined members.
  if (cluster.includes('\u200D') && w > 2) w = 2;
  return w;
}

/**
 * End index (exclusive) of the grapheme cluster starting at str[i], using
 * the same joining set graphemeWidth() treats as zero-width (ZWJ + its
 * joined base, variation selectors, skin tones, combining diacritics).
 * Lets ANSI-aware walkers (truncateVisible) consume whole clusters so a
 * width-2 cluster that FITS the budget is never cut to a bare '\u2026'.
 */
function clusterEnd(str, i) {
  let j = i + String.fromCodePoint(str.codePointAt(i)).length;
  for (;;) {
    const cp = str.codePointAt(j);
    if (cp === undefined) break;
    if (cp === 0x200D) { // ZWJ absorbs the next base into this cluster
      j += String.fromCodePoint(cp).length;
      const nxt = str.codePointAt(j);
      if (nxt !== undefined) j += String.fromCodePoint(nxt).length;
      continue;
    }
    if ((cp >= 0xFE00 && cp <= 0xFE0F) || (cp >= 0x1F3FB && cp <= 0x1F3FF)
      || (cp >= 0x0300 && cp <= 0x036F)) {
      j += String.fromCodePoint(cp).length;
      continue;
    }
    break;
  }
  return j;
}

/**
 * Visible width of a string with ANSI escapes stripped. Grapheme-cluster
 * based: 'é' counts 1, ZWJ emoji families 2, skin-tone pairs 2 — the
 * same table InputBox cursor math uses, so measurement can never disagree
 * with editing. Plain-ASCII strings (the overwhelming majority) take the
 * length fast path without touching the segmenter.
 */
export function visibleWidth(s) {
  if (!s) return 0;
  const str = String(s);
  // Fast path: no ESC and pure single-column ASCII → length IS the width.
  if (!str.includes('\x1b') && /^[\x20-\x7E]*$/.test(str)) return str.length;
  // Strip ANSI CSI sequences and OSC sequences (which end on BEL or ST).
  const stripped = str
    .replace(/\x1b\[[0-9;?]*[A-Za-z]/g, '')
    .replace(/\x1b\][^\x07\x1b]*(?:\x07|\x1b\\)/g, '');
  let w = 0;
  for (const g of graphemes(stripped)) {
    w += graphemeWidth(g);
  }
  return w;
}

/**
 * Truncate `s` to a given VISIBLE width, preserving all ANSI styling.
 *
 * Walks the string keeping escape sequences intact and only counts visible
 * (non-escape) characters against the budget. When the content exceeds the
 * budget it keeps the last 1 column for an ellipsis (`…` / `...`) and always
 * emits a closing RESET so a dangling escape can't bleed color into the rest
 * of the line. Widens-aware via visibleWidth().
 *
 * `maxWidth <= 0` returns ''. `maxWidth === 1` returns just the ellipsis.
 */
export function truncateVisible(s, maxWidth) {
  if (!s) return '';
  const str = String(s);
  const budget = Math.max(0, maxWidth | 0);
  if (budget === 0) return '';
  // Fast path: fits without truncation.
  if (visibleWidth(str) <= budget) return str;

  const ell = ICON.ellipsis; // '…' (UTF-8) or '...' (ASCII)
  const ellW = visibleWidth(ell);
  // Reserve room for the ellipsis. If the budget can't even fit it, show only
  // as much of the ellipsis as fits so the bar never wraps.
  const keepW = Math.max(0, budget - ellW);

  let out = '';
  let w = 0;
  let i = 0;
  while (i < str.length) {
    const code = str.charCodeAt(i);
    // ANSI CSI sequence: ESC [ ... <0x40-0x7E>
    if (code === 0x1b && str[i + 1] === '[') {
      let j = i + 2;
      while (j < str.length) {
        const c = str.charCodeAt(j);
        if (c >= 0x40 && c <= 0x7e) { j++; break; }
        j++;
      }
      out += str.slice(i, j);
      i = j;
      continue;
    }
    // OSC sequence: ESC ] ... (BEL | ESC \)
    if (code === 0x1b && str[i + 1] === ']') {
      let j = i + 2;
      while (j < str.length && str.charCodeAt(j) !== 0x07) {
        if (str.charCodeAt(j) === 0x1b && str[j + 1] === '\\') { j += 2; break; }
        j++;
      }
      out += str.slice(i, j);
      i = j;
      continue;
    }
    // Bare ESC (e.g. the save/restore cursors the bar emits) - pass through.
    if (code === 0x1b) { out += str[i]; i++; continue; }

    // Visible text. Consume ONE GRAPHEME CLUSTER per step (combining marks,
    // ZWJ families, skin tones travel with their base) measured with
    // graphemeWidth — a cluster that fits the budget is never split.
    const end = clusterEnd(str, i);
    const cluster = str.slice(i, end);
    const cw = graphemeWidth(cluster);
    if (w + cw > keepW) break;
    out += cluster;
    w += cw;
    i = end;
  }
  out += ell;
  // Always terminate styling so a truncated span can't leak color.
  if (MODE !== 'none') out += RESET;
  return out;
}

/** Repeat the horizontal box glyph to a visible width, painted as border. */
function hrule(n, token = 'border') {
  return paint(token, BOX.horizontal.repeat(Math.max(0, n)));
}

/**
 * Pick a card width that fits every body line, capped at term width.
 *
 * @param {string[]} lines              body lines (may contain ANSI codes)
 * @param {object} [opts]
 * @param {string} [opts.title]         card title (visible portion adds to width budget)
 * @param {number} [opts.floor=30]      minimum width
 * @param {number} [opts.ceil]          maximum width (default: terminal width)
 * @param {number} [opts.padding=4]     extra slack: leading/trailing space + the two verticals
 * @returns {number}                    body width (interior of the card, including 1-char leading space)
 */
export function fitCardWidth(lines, opts = {}) {
  const floor = opts.floor ?? 30;
  const ceil = opts.ceil ?? termWidth();
  const padding = opts.padding ?? 4;  // 2 verticals + 2 spaces of breathing room
  const titleBudget = opts.title ? visibleWidth(opts.title) + 2 /* " title " */ : 0;
  let maxContent = 0;
  for (const ln of lines || []) maxContent = Math.max(maxContent, visibleWidth(ln));
  const want = maxContent + padding;
  return Math.max(floor, Math.min(ceil, Math.max(want, titleBudget + padding)));
}

/**
 * Top border with optional embedded title: `╭── title ──╮`.
 * The title sits between two horizontal runs; sides close the box.
 */
export function topBorder(title, { width = termWidth(), token = 'border' } = {}) {
  const innerW = Math.max(0, width - 2);  // minus the two verticals
  const t = title ? ` ${title} ` : '';
  const tVis = visibleWidth(t);
  const left = hrule(Math.min(innerW, Math.max(1, 3)), token);
  // Title hue: fixed muted, or the border token color when the theme sets
  // titleFollowsBorder (topBorder title tracks the frame color).
  const titleText = (s) => (TITLE_FOLLOWS ? paint(token, s) : muted(s));
  if (!title) {
    return paint(token, BOX.topLeft + BOX.horizontal.repeat(innerW) + BOX.topRight);
  }
  if (tVis >= innerW) {
    // Title fills the row: drop the trailing run.
    const titleStr = bold(titleText(t.trim().slice(0, Math.max(1, innerW - 2))));
    return paint(token, BOX.topLeft) + left + titleStr + paint(token, BOX.horizontal.repeat(Math.max(0, innerW - visibleWidth(left) - visibleWidth(titleStr)))) + paint(token, BOX.topRight);
  }
  const after = innerW - visibleWidth(left) - tVis;
  return paint(token, BOX.topLeft) + left + bold(titleText(t)) + hrule(Math.max(0, after), token) + paint(token, BOX.topRight);
}

/** Bottom border: `╰──…──╯`. */
export function bottomBorder({ width = termWidth(), token = 'border' } = {}) {
  const innerW = Math.max(0, width - 2);
  return paint(token, BOX.bottomLeft + BOX.horizontal.repeat(innerW) + BOX.bottomRight);
}

/**
 * Wrap a body line with the vertical borders: `│ line │`. Pads or truncates
 * the line so the right border always lines up.
 *
 * Layout inside the card:
 *   │ <space> text <padding> │
 *   ^^^^^^^ ^^^^^^^^^ ^^^^^^^ ^^^
 *   left-v  lead-space content trailing pad  right-v
 *
 * Total visible width = 1 + 1 + vis + pad + 1 = width
 * → pad = width - 3 - vis = (width - 2) - vis - 1 = innerW - vis - 1.
 * When pad < 0 the content wouldn't fit even without padding; truncate.
 */
export function bodyLine(text, { width = termWidth(), token = 'border' } = {}) {
  // Collapse line breaks and tabs so a multi-line value (e.g. a bash heredoc
  // command like `cat > f << 'EOF'\n…\nEOF`) renders on a single physical row.
  // Without this the embedded newline splits the row and the right vertical
  // border dangles on the last fragment, breaking the box. ANSI escape
  // sequences (which never contain \r/\n/\t) pass through untouched.
  const safe = String(text == null ? '' : text)
    .replace(/\r\n|\r|\n/g, ' ')
    .replace(/\t/g, ' ');
  const innerW = Math.max(0, width - 2);
  const v = paint(token, BOX.vertical);
  const vis = visibleWidth(safe);
  // Content budget = innerW - 1 (the leading space eats one column).
  // If vis exceeds the budget, truncate and append an ellipsis in-place.
  const budget = innerW - 1;
  if (vis > budget) {
    const ellipsisVis = visibleWidth(ICON.ellipsis);
    const cap = Math.max(0, budget - ellipsisVis);
    let out = '';
    let w = 0;
    let inEsc = false;
    for (const g of graphemes(safe)) {
      // ESC is its own cluster; the rest of an SGR span follows as plain
      // clusters. Reassemble them without width until the closing 'm'.
      if (inEsc || g === '\x1b') {
        for (const ch of g) {
          out += ch;
          if (ch === 'm') inEsc = false;
        }
        if (g === '\x1b') inEsc = true;
        continue;
      }
      // Width-check BEFORE appending: a 2-wide CJK glyph / emoji that would
      // cross the cap must be cut here. Checking only the accumulated width
      // (`w >= cap`) let a wide glyph start at cap-1 and push the line one
      // column past the card, breaking the right border.
      if (w + graphemeWidth(g) > cap) break;
      out += g;
      w += graphemeWidth(g);
    }
    out += paint('dim', ICON.ellipsis);
    const used = visibleWidth(out);
    const pad = innerW - 1 - used;
    return `${v} ${out}${pad > 0 ? ' '.repeat(pad) : ''}${v}`;
  }
  const pad = innerW - 1 - vis;
  return `${v} ${safe}${pad > 0 ? ' '.repeat(pad) : ''}${v}`;
}

/**
 * Render a complete card: top border + body lines + bottom border. Optional
 * `title` is embedded in the top border; optional `token` colors the border.
 */
export function card({ title = '', lines = [], token = 'border', width = termWidth() } = {}) {
  const out = [topBorder(title, { width, token })];
  for (const ln of lines) out.push(bodyLine(ln, { width, token }));
  out.push(bottomBorder({ width, token }));
  return out;
}

/**
 * Loader line: `<spinner> <message>` with optional `<dim·suffix>`.
 */
export function loaderLine(spinnerFrame, message, suffix = '') {
  const head = accent(spinnerFrame);
  const tail = suffix ? ' ' + dim(`${ICON.dot} ` + suffix) : '';
  return `${head} ${muted(message)}${tail}`;
}

/**
 * Tip line: ` Tip: <body>` — italic, label in customMessageLabel (accent),
 * body in muted.
 */
export function tipLine(text) {
  return ' ' + italic(accent('Tip: ') + muted(text));
}

/** Standard "Error: …" line in the error color. */
export function errorLine(msg) {
  return errorT(`Error: ${msg}`);
}

/** One-line header for a tool execution card: `<token> bold(text)`. */
export function cardHeader(text, token = 'bashMode') {
  return paint(token, bold(text));
}
