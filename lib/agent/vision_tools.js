 ...
 *
 * Ten specialized tools (ui_to_artifact, extract_text_from_screenshot,
 * diagnose_error_screenshot, understand_technical_diagram,
 * analyze_data_visualization, ui_diff_check, image_analysis,
 * video_analysis, capture_and_analyze, record_and_analyze) that give ANY
 * session model multimodal capabilities — including two LIVE screen-capture
 * tools that grab the current screen themselves (screenshot / 1-60 s
...
import {
  ...
  RECORD_MAX_SECONDS,
  RECORD_MIN_SECONDS,
  captureBackendSummary,
} from './screen_capture.js';

/** Registry entries that capture the LIVE screen before analysis (no path input). */
const CAPTURE_TOOLS = new Set(['capture_and_analyze', 'record_and_analyze']);
...
export const VISION_TOOLS = [
  ...
  },
  {
    ...
    snippet: 'Capture a LIVE screenshot of the current screen and have a multimodal model analyze it to help locate or diagnose issues',
    media: 'image',
    capture: 'screenshot',
    inputs: [],
    instruction: [
      'The attached image is a LIVE screenshot of the user\'s current screen, captured just now to help locate or analyze an issue.',
      'Produce:',
      '1. Screen inventory — visible apps, windows, terminals, dialogs and their states.',
      '2. Errors and warnings — exact visible messages, codes, paths, line numbers (verbatim); quote them precisely.',
      '3. Context clues — what the user appears to be doing and what the likely goal is.',
      '4. Diagnosis — what looks wrong or suspicious, ranked by confidence, grounded only in what is visible.',
      '5. Next steps — concrete actions to investigate or fix (commands to run, files to open, what to capture next).',
      'The optional `context` argument gives the user\'s own description of the problem; trust it as intent, but verify against the screen.',
    ].join('\n'),
    extraParams: [{ arg: 'context', values: null, default: null }],
    parameters: {
      type: 'object',
      properties: {
        context: { type: 'string', description: 'Optional: what problem the user is investigating or what to look at on the screen' },
      },
      required: [],
    },
  },
  {
    ...
    snippet: 'Record the LIVE screen for a bounded clip (1-60 seconds) and have a multimodal model analyze the video to help locate or diagnose issues',
    media: 'video',
    capture: 'recording',
    inputs: [],
    instruction: [
      'The attached video is a LIVE recording of the user\'s screen (1-60 seconds, no system audio), captured to show a problem as it happens.',
      'Produce:',
      '1. Overview — what happens over the clip, app(s) and window(s) involved.',
      '2. Timeline of events — user actions and system responses in order, with approximate timestamps within the clip.',
      '3. Errors and anomalies — the moment something goes wrong; quote visible messages verbatim.',
      '4. Diagnosis — the likely root cause, grounded in what is visible.',
      '5. Next steps — concrete actions to investigate or fix.',
      'The optional `focus` argument narrows what to look for (e.g. "when the crash dialog appears", "the exact clicks before the error").',
    ].join('\n'),
    extraParams: [
      { arg: 'seconds', values: null, default: String(RECORD_DEFAULT_SECONDS) },
      { arg: 'focus', values: null, default: null },
    ],
    parameters: {
      type: 'object',
      properties: {
        seconds: { type: 'number', description: `Recording duration in seconds, ${RECORD_MIN_SECONDS}-${RECORD_MAX_SECONDS} (default ${RECORD_DEFAULT_SECONDS})` },
 ...
 *
 * Capture-type tools (capture: 'screenshot' | 'recording') take NO path
 * input: they grab the LIVE screen first, run it through the normal
 * analysis pipeline, and always delete the temporary capture afterwards
 * (success or failure). Regular tools validate declared path/URL inputs.
 *
...
export async function runVisionTool(tool, args, vision) {
  ...
  let tempFiles = [];
  if (tool.capture) {
    // LIVE capture: the tool's only input is the screen itself. The result
    // is a temporary file in os.tmpdir(), deleted after the analysis.
    // `tool.captureFn` is a TEST-INJECTION hook (registry entries are plain
    // mutable objects): when present it replaces the real capture call.
    const grabbed = tool.captureFn
      ? await tool.captureFn(args || {})
      : tool.capture === 'recording'
        ? await recordScreen({ seconds: (args || {}).seconds })
        : await captureScreen();
    if (!grabbed.ok) return { error: grabbed.error };
    refs = [grabbed.path];
    labels = [tool.capture === 'recording' ? 'live screen recording' : 'live screen screenshot'];
    tempFiles = [grabbed.path];
  } else {
  ...
  let out;
  try {
    out = await buildAttachmentBlocks(refs, {
...
/** Best-effort removal of temporary live-capture files; never throws. */
async function cleanupTemp(files) {
  for (const f of files || []) {
    try { await rm(f, { force: true }); } catch { /* best effort */ }
  }
}
...
export async function buildVisionTools(vision, opts = {}) {
  ...
  for (const t of VISION_TOOLS) {
    ...
    const extras = (t.extraParams || [])
      ...
      .map(p => `\`${p.arg}\``)
      .join(', ');
    const captureGuideline = t.capture
      ? (t.capture === 'recording'
        ? [`This tool captures the LIVE screen itself (no input path) for ${RECORD_MIN_SECONDS}-${RECORD_MAX_SECONDS} seconds (default ${RECORD_DEFAULT_SECONDS}), then forwards the clip to a dedicated multimodal model (${eff.cfg.ref}) and returns its textual analysis — the session model itself never needs multimodal input.`,
          `Platform support: ${captureBackendSummary()}. The recording is a temporary file, deleted after the analysis; it is also subject to the 8 MB analysis cap — prefer shorter clips (10-15 s usually suffice).`]
        : [`This tool captures the LIVE screen itself (no input path), then forwards the screenshot to a dedicated multimodal model (${eff.cfg.ref}) and returns its textual analysis — the session model itself never needs multimodal input.`,
          `Platform support: ${captureBackendSummary()}. The screenshot is a temporary file, deleted after the analysis.`])
      : [
    ...
    tools.push({
      name: t.name,
      snippet: t.snippet,
      guidelines: captureGuideline,
      description: `${t.snippet}. ${t.capture
        ? `Captures the ${t.capture === 'recording' ? `live screen (${RECORD_MIN_SECONDS}-${RECORD_MAX_SECONDS}s)` : 'live screen'} itself (no input path), then forwards the capture to a dedicated multimodal model (configured via /tool set-model, falling back to the session model when it is multimodal) and returns its analysis as text`
        : `Forwards the given ${t.media} to a dedicated multimodal model (configured via /tool set-model, falling back to the session model when it is multimodal) and returns its analysis as text`}${extras ? `; extra arguments: ${extras}` : ''}. The session model does NOT need multimodal input to use this tool.`,
