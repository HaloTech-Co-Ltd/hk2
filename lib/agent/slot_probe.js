...
export function installSlotProbe(stdin, bar) {
  ...
  // A tail that is UNAMBIGUOUSLY a partial answer opener: ESC[ followed by
  // only digits/semicolons. Unlike a bare ESC — which, after a whole answer
  // was already stripped, is far likelier the USER's interrupt key — this
  // shape never occurs in user input, so the straggler completion branch can
  // keep holding it across chunks (a ≥3-fragment byte-split answer, e.g.
  // ESC[ | 24;1 | R) without ever stranding user keys.
  const ANSWER_OPEN_TAIL = /\x1b\[[\d;]*$/;
  ...
  const finish = (healthy, rows, silent = false) => {
    if (!open) return;
    pending = 0;
    if (timer) { clearTimeout(timer); timer = null; }
    closeWindow();
    // `silent` = the terminal never answered at all: lets the bar tell
    // "drift PROVEN by an answer" from "terminal is DSR-deaf" and back its
    // probe cadence off instead of healing (full repaint + cursor jump)
    // every ~5s forever on a terminal that can never answer anyway.
    try { bar.slotProbeResult(healthy, rows, { silent }); } catch { /* bar gone */ }
  };
  ...
  // everything else is buffered for replay. The tail-holdback rule applies
  // to EVERY chunk while an answer may still complete — a byte-split answer
  // can split at ANY boundary (≥3 fragments, e.g. ESC[ | 24;1 | R: the old
  // 2-fragment assumption released `\x1b[24;1` after fragment 2 and injected
  // the trailing `R` into the draft). The filter disarms — re-attaching
  // readline and replaying the buffer — as soon as a chunk resolves with no
  // pending tail (answer consumed, or plain user keys that were never an
  // answer), OR after STRAGGLER_MAX_CHUNKS chunks (bounded hunt: a tail that
  // keeps not completing is likelier USER keys — a bare ESC, an arrow-key
  // prefix — and the stream must go back to readline before keys are lost).
  const onStragglerData = (chunk) => {
    if (!straggler) return;
    const raw = String(chunk);
    schunks++;
    if (schunks > STRAGGLER_MAX_CHUNKS) {
      // Bounded hunt exhausted: give the stream back. Held-back bytes were
      // user keys; replay them with this chunk, never drop.
      if (carry.length > 0) { sreplay.push(carry); carry = ''; }
      sreplay.push(raw);
      disarmStraggler();
      return;
    ...
    // answer — the USER's interrupt key) passes through. A NEW tail is held
    // back only when it is UNAMBIGUOUSLY answer-shaped (ESC[ + digits /
    // semicolons): that is the middle fragment of a ≥3-fragment byte-split
    // answer (e.g. `\x1b[24;1` awaiting its `R`), and releasing it would
    // leak the tail digits / 'R' into readline's draft. Holding exactly that
    // shape can never strand user keys, so no additional chunk bound is
    // needed beyond STRAGGLER_MAX_CHUNKS.
    const s = carry + raw;
    carry = '';
    const stripped = s.replace(DSR_ANSWER, '');
    const tail = stripped.match(ANSWER_OPEN_TAIL);
    if (tail) {
      carry = tail[0];
      const head = stripped.slice(0, stripped.length - tail[0].length);
      if (head.length > 0) sreplay.push(head);
      return; // still hunting the completion
    }
  ...
  const probe = () => {
    ...
    // ATOMICITY: the whole probe (slot restore + DSR ask + re-save + park)
    // must go out as ONE write. The previous two-write split let the tty
    // driver flush between the writes, and terminals render per flush — the
    // frame between the flushes showed the cursor at the DECRC-restored
    // WORKSPACE position (it left the input box for a few ms every ~5s),
    // perceived as the input-box cursor periodically "blinking". One write
    // is parsed in one pass; the intermediate position is never rendered.
    // DECTCEM bracket on top of atomicity: one write still is not guaranteed
    // one rendered frame (pty/ConPTY read chunking can split it), and the
    // sequence's own DECRC parks the cursor on the workspace continuation row
    // — usually BLANK space — before the park returns it to the box. A split at
    // that point rendered "a cursor flashing by in blank space"; hidden, the
    // intermediate frame shows no cursor at all. Show ALWAYS re-emitted in the
    // same write, so the pairing can never tear.
    let probeSeq = '\x1b[?25l\x1b8\x1b[6n\x1b7';
    const park = typeof bar.parkSeq === 'function' ? bar.parkSeq() : null;
    if (park) probeSeq += park;
    probeSeq += '\x1b[?25h';
    bar.rawWrite(probeSeq);
    timer = setTimeout(() => {
      timer = null;
      if (pending > 0) {
        ...
        finish(false, undefined, true); // silent terminal: unknown (DSR-deaf?)
        armStraggler();
      }
    }, 160);
  };
