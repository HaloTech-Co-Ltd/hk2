...
export async function buildRequestGraph(rt, query, opts = {}) {
  ...
  const contentAbsByPath = await resolveKbContentPaths(contentPaths, rt.sourceConfig);
  ...
  // collectDocs authorizes the actual disk paths before reading them.
...
async function collectDocs(rt, query, project) {
  ...
  const scored = [];
  const readable = await filterAllowedPaths(candidates);
  for (const f of candidates) {
    if (!readable.has(path.resolve(f))) continue;
    let text = '';
