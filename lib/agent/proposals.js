 ...
 * `resolve` then either applies (best-effort, non-transactional multi-file apply with attempted restoration) or
 * discards (dropping the entry).
 *
 * Bounds:
 *   - MAX_PROPOSALS is 16, but stage() prunes before insertion, so a seventeenth proposal can be retained temporarily until a later prune.
 *   - 10-minute fixed creation-time TTL; lastTouched affects LRU ordering only, not expiry.
 *
