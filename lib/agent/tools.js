  ...
  const tools = [
    {
      ...
      description: `Read a UTF-8 text file. Files larger than 5 MiB are rejected before reading (offset/limit cannot bypass this). Files whose first 8192 decoded characters contain a NUL byte are rejected as binary (a NUL-scan heuristic, not full binary-format detection). Output is limited to ${MAX_READ_LINES} lines or ${Math.floor(MAX_READ_BYTES / 1024)}KB; use offset/limit to continue reading eligible text files. For eligible indexed source files, an outline from the KB may be prepended unless outline=false.`,
      parameters: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: `Execute a bash command in the current working directory. Returns stdout and stderr. Output is truncated to ${MAX_READ_LINES} lines or ${Math.floor(MAX_BASH_OUTPUT / 1024)}KB (whichever is hit first). Optional timeout in seconds: default 60, hard-capped at 60 (larger values are clamped; 0 falls back to the default).`,
      parameters: {
        type: 'object',
        properties: {
  ...
  if (rt) {
    tools.push(
      ...
      },
      {
        ...
        description: 'Bounded breadth-first (BFS) traversal over the call graph. Returns callers and/or callees up to max_depth hops, capped at max_nodes total.',
        parameters: {
          type: 'object',
          properties: {
