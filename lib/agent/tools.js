...
import { mediaClassOf, MAX_ATTACHMENT_BYTES } from './attachments.js';
 ...
 *                                kb_save_knowledge). When read() prepends a
 *                                KB-sourced outline it calls noteKbAssist().
 *                                Both are called AFTER the tool's own
 *                                precondition checks so a failed KB operation
 *                                does not falsely suppress a later fallback hint.
...
export class KbFirstGuard {
  ...
  bashHint(cmd) {
    ...
    return `[kb-first policy hint] This bash command looks like code discovery (grep/find/cat on source files). The project KB has a prebuilt index — try kb_search("<natural-language query>") or kb_symbol("<exact name>") instead; they are usually more targeted and avoid a fresh filesystem scan. Only fall back to bash when the KB genuinely doesn't have what you need (and at end of turn hk2 will offer or automatically run the update according to HK2_ENABLE_AUTOUPDATEKB to synchronize new symbols and parser-owned doc:* Eden entries).`;
  }
  shouldHintRead(p) {
  ...
  readHint(p) {
    ...
    return `[kb-first policy hint] You're reading a source or documentation file recognized by hk2's tool-level KB heuristic without having queried the KB yet this LLM call. Consider kb_search / kb_symbol first to confirm this is the right file. Use kb_neighbors for one-hop outgoing callees; use kb_callchain(direction="backward" or "both") or kb_refs(kind="call") for callers. (Hint shown once per LLM call.)`;
  }
  /**
...
async function toolRead({ path: p, offset = 1, limit, outline = true }, guard, rt, multimodal) {
  ...
  // Media gate (multimodal): image/video/audio extensions classify
  // deterministically BEFORE any UTF-8 read — these are never text files, so
  // the read-as-text path (mojibake, or the NUL-binary error below) is never
  // the useful outcome. When the session's active model accepts multimodal
  // input, return a compact `attach` marker instead: the turn pipeline
  // re-reads the file at the round boundary and injects real image_url /
  // video_url / input_audio content blocks into the conversation (see
  // turn.js onRoundBoundary). The marker deliberately carries NO Base64 —
  // tool results flow through the transcript, the UI, summarizers and the
  // stuck-detector fingerprint, all of which must stay payload-free (the
  // multimodal no-leak invariant).
  const mediaClass = mediaClassOf(p);
  if (mediaClass) {
    if (!multimodal) {
      return {
        error: `${p}: ${mediaClass} file — read only supports text, and the active model has no multimodal input. Enable it with /model set <ref> --multimodal=on on a capable model type (e.g. glm-5.3-flash), or extract metadata via bash instead.`,
      };
    }
    if (stat.size > MAX_ATTACHMENT_BYTES) {
      return { error: `${p}: file too large (${(stat.size / 1048576).toFixed(1)} MB > ${Math.floor(MAX_ATTACHMENT_BYTES / 1048576)} MB multimodal attachment limit)` };
    }
    ...
    return {
      ...
      media: mediaClass,
      bytes: stat.size,
      // `source` is the ABSOLUTE path so the round-boundary encoder is immune
      // to any cwd drift between this read and the injection.
      attach: { media: mediaClass, source: abs },
      note: `${mediaClass} file staged for multimodal input — its actual content (pixels/audio) is injected into the conversation automatically as content blocks. Analyze the real content; do not fall back to metadata-only analysis.`,
    };
  }
  ...
  // KB-first guardrail: if this path matches the source/document heuristic AND the agent hasn't used
  // any KB tool yet this LLM call, prepend a one-time hint nudging toward
  // kb_search / kb_symbol. We still return the requested content.
  const hint = guard?.shouldHintRead(p) ? guard.readHint(p) : null;

  // KB outline prepend: when the indexed file is eligible for read auto-annotation and the
 ...
 */
async function toolEdit({ path: p, old_string, new_string, oldText, newText, edits, tag, replaceAll }) {
  if (!p) return { error: 'path required' };
  const res = await resolvePermitted(p, 'w');
  if (res.error) return { error: res.error };
  const abs = res.abs;
  ...
  if (tag && shortHash(text) !== tag) {
    return {
      error: `stale tag: the current file hash differs from the supplied KB-index snapshot tag (expected ${tag}, got ${shortHash(text)})`,
  ...
  for (let ei = 0; ei < editList.length; ei++) {
    ...
    // and splices the replacement between every character via split('')).
    if (e.old === '') return { error: 'edit.oldText must be a non-empty string' };
    ...
    // Fuzzy anchor fallback: the second failure class. read() shows lines
    // with a `  NNN\t` prefix and the model RECONSTRUCTS oldText from that
    // display — indentation drift (2-space copied as 4-space), tab↔space
    // swaps and trailing-whitespace drift are routine, and byte-exact
    // matching rejects them all as "oldText not found". Match on a
    // whitespace-insensitive line SIGNATURE (each line trimmed; the needle
    // may skip blank lines) as a sequence; when the sequence occurs exactly
    // once in the file, anchor the replacement on the file's own bytes and
    // REASSEMBLE newText line-by-line: unchanged lines reuse the file's
    // actual indentation bytes, changed lines re-emit their indent with the
    // transformation learned from the matched region (uniform ratio first,
    // then uniform shift) in the file's own indent unit (tab file → tabs).
    // Never applied when verbatim matched (this is a fallback, not a
    // replacement) and never a partial/paraphrase match — a wrong-word
    // oldText simply has a different signature and keeps failing, which is
    // the safe outcome.
    let fuzzyApplied = false;
    if (idx === -1) {
      const fuzzyResult = matchFuzzyAnchor(next, e.old);
      if (fuzzyResult && fuzzyResult.matches === 1) {
        const { start, end } = fuzzyResult;
        const fileBlock = next.slice(start, end);
        // When the oldText contains NO blank lines but the matched file span
        // does, the model never SAW those blanks (it collapsed them while
        // reconstructing from the numbered display) — silently deleting them
        // would be content loss, so they are preserved. When the oldText
        // DOES contain blanks, the model was blank-aware and the wholesale
        // replacement stands (deleting one is then intent).
        const oldHasBlanks = e.old.split('\n').some((l) => lineSignature(l) === '');
        // CRLF conversion keyed on the WHOLE current text, not just the
        // matched block: a single-line oldText block carries no terminator,
        // yet a multi-line newText still needs CRLF breaks in a CRLF file.
        const usesCrlf = next.includes('\r\n');
        let replacement = reassembleFuzzyReplacement(e.old, e.neu, fileBlock, !oldHasBlanks);
        if (usesCrlf) replacement = replacement.replace(/\n/g, '\r\n');
        next = next.slice(0, start) + replacement + next.slice(end);
        fuzzyApplied = true;
        fuzzyAdaptations.push({ oldText: `${e.old.slice(0, 60)}${e.old.length > 60 ? '…' : ''}`, reason: 'whitespace-insensitive match (indent/tab/trailing-ws drift)' });
      }
    }
    if (idx === -1 && !fuzzyApplied) {
      const failIdx = ei;
      const appliedBefore = ei;
      return {
        error: buildEditFailureMessage(next, e.old, failIdx, appliedBefore, editList.length),
    ...
    if (idx2 !== -1) {
      if (replaceAllThis) {
        ...
        let from = idx2;
        for (;;) {
          const more = next.indexOf(probe, from + 1);
          if (more === -1) break;
          count++; from = more;
        }
        let replacement = e.neu;
        if (eolAdapted) replacement = replacement.replace(/\n/g, '\r\n');
        next = next.split(probe).join(replacement);
    ...
    // replacement is multi-line (a single-line oldText carries no terminator
    // to inherit, so unconverted LF breaks would inject mixed endings) — the
    // replacement's line breaks become CRLF too.
    let replacement = e.neu;
    if (eolAdapted || (!fuzzyApplied && text.includes('\r\n'))) {
  ...
  return {
    ...
    ...(fuzzyAdaptations.length > 0 ? { fuzzyAdapted: fuzzyAdaptations } : {}),
  };
}

/**
 * Whitespace-insensitive line signature: the line with leading/trailing
 * whitespace stripped (including the CR of a CRLF terminator — split('\n')
 * keeps it glued to the line's tail). Blank lines contribute nothing.
 */
...
/**
 * Split text into physical lines WITHOUT losing their byte offsets. Returns
 * [{ start, end, text }] where text is the raw line EXCLUDING the line
 * terminator (start/end cover the same span in the original string).
 */
function splitLinesWithOffsets(text) {
  ...
  while (i < text.length) {
    ...
    const end = nl === -1 ? text.length : nl; // excludes '\n' (and any '\r' stays in text — stripped by signature)
...
/**
 * Find occurrences of `needle` inside `haystack` by whitespace-insensitive
 * line-signature sequence matching. The needle's blank lines are skipped
 * (the model often collapses them when reconstructing from a numbered
 * display); file blank lines inside a matched span are consumed by the
 * span. Returns { start, end, matches } — byte offsets of the FIRST match
 * (start at the first signature line's line-start; end right after the
 * last signature line's content, before its terminator) — or null when the
 * signature sequence doesn't occur at all (e.g. paraphrased oldText whose
 * words actually differ). A signature line never matches mid-line: the
 * match always consumes whole physical lines on the file side.
 */
function matchFuzzyAnchor(haystack, needle) {
  const hLines = splitLinesWithOffsets(haystack);
  const nSig = needle.split('\n').map(lineSignature).filter((s) => s !== '');
  if (nSig.length === 0) return null;
  const hSig = hLines.map((l) => lineSignature(l.text));
  let matches = 0;
  let first = null;
  for (let i = 0; i + nSig.length <= hLines.length; i++) {
    // A match must START on a signature line: allowing blank starts would
    // double-count the same occurrence (the inner loop skips blanks before
    // the first comparison too), inflating matches to 2 and rejecting unique
    // anchors whose region is preceded by a blank line.
    if (hSig[i] === '') continue;
    let ok = true;
    let k = i;
    for (let j = 0; j < nSig.length; j++, k++) {
      // Advance k past file blank lines (needle blanks were dropped). For
      // j > 0 only — a blank before the FIRST signature line was already
      // excluded by the outer-loop guard above.
      if (j > 0) while (k < hLines.length && hSig[k] === '') k++;
      if (k + (nSig.length - j) > hLines.length) { ok = false; break; }
      if (hSig[k] !== nSig[j]) { ok = false; break; }
    }
    if (!ok) continue;
    matches++;
    if (!first) {
      // Span from the first signature line's start (INCLUDING leading
      // whitespace) to the last signature line's end — EXCLUDING a trailing
      // \r, which belongs to the terminator: keeping it inside the span
      // would consume the CR and leave an orphan LF behind after splice.
      const last = hLines[k - 1];
      first = { start: hLines[i].start, end: last.text.endsWith('\r') ? last.end - 1 : last.end };
    }
  }
  if (matches === 0) return null;
  return { start: first.start, end: first.end, matches };
}

/**
 * Leading-whitespace prefix of a line (spaces and tabs).
 */
function indentPrefix(s) {
  const m = /^[ \t]*/.exec(s);
  return m ? m[0] : '';
}

/**
 * Display width of an indent prefix (tab = 2 columns, hardcoded like the
 * rest of the family).
 */
function indentWidth(ind) {
  let w = 0;
  for (const ch of ind) w += ch === '\t' ? 2 : 1;
  return w;
}

/**
 * Learn the indent transformation between the model's oldText and the
 * file's actual bytes over their matched region. Only INTERIOR lines vote
 * (old indent > 0 and file indent > 0 — top-level 0→0 lines carry no signal
 * and pollute both detectors):
 *   - prefer a uniform RATIO (fileW = oldW × r, r ≥ 1.5 or ≤ 0.67);
 *   - else a uniform SHIFT (fileW = oldW + d, d ≠ 0);
 *   - else null (no learnable transformation).
 */
function learnIndentTransform(oldText, fileText) {
  const oLines = oldText.split('\n');
  const fLines = fileText.split('\n');
  // Pair signature-equal lines sequentially (needle blanks skipped).
  const pairs = [];
  let fi = 0;
  for (const ol of oLines) {
    const sig = lineSignature(ol);
    if (sig === '') continue;
    while (fi < fLines.length && lineSignature(fLines[fi]) !== sig) fi++;
    if (fi >= fLines.length) break;
    const a = indentWidth(indentPrefix(ol));
    const b = indentWidth(indentPrefix(fLines[fi]));
    if (a > 0 && b > 0) pairs.push([a, b]);
    fi++;
  }
  if (pairs.length < 1) return null; // no interior signal at all
  // Uniform ratio needs CORROBORATION (2+ agreeing pairs): from a single
  // pair any b/a is "uniform", and scaling on one observation is a guess.
  if (pairs.length >= 2) {
    const r0 = pairs[0][1] / pairs[0][0];
    if (r0 !== 1 && (r0 >= 1.5 || r0 <= 0.67) && pairs.every(([a, b]) => Math.abs(b / a - r0) < 0.01)) {
      return { kind: 'ratio', value: r0 };
    }
  }
  // Uniform shift — a single pair suffices (it IS the observed drift, e.g.
  // a single-line oldText copied with wider indent).
  const d0 = pairs[0][1] - pairs[0][0];
  if (d0 !== 0 && pairs.every(([a, b]) => b - a === d0)) {
    return { kind: 'shift', value: d0 };
  }
  return null;
}

/**
 * Emit `w` columns of indent in the file's own style: tabs when the file
 * region uses tabs, else spaces (one column per space).
 */
function emitIndent(w, useTabs) {
  if (w <= 0) return '';
  if (!useTabs) return ' '.repeat(w);
  return '\t'.repeat(Math.floor(w / 2)) + ' '.repeat(w % 2);
}

/**
 * Reassemble newText for a fuzzy-anchored replacement, preserving the file's
 * indentation style via SEQUENTIAL ALIGNMENT (not a signature→indent map —
 * short lines like '}' collide and a map keeps only the first occurrence):
 *   1. Align oldText lines to file lines in order (signature-equal, blanks
 *      skipped) recording each old line's file indent BYTES and file line
 *      INDEX (for blank preservation positioning).
 *   2. Walk newText lines against oldText lines greedily forward: a new
 *      line whose signature matches a remaining old line reuses that old
 *      line's FILE indent bytes verbatim (unchanged content, zero drift).
 *   3. A genuinely new/changed line re-emits its indent with the learned
 *      transformation (ratio, then shift) — or unchanged width — in the
 *      file's own unit (tab file → tabs).
 * With `preserveUnmatched` (oldText had no blanks), file lines inside the
 * span that no old line claims — i.e. blanks the model never saw — are
 * emitted verbatim at their position instead of being dropped.
 */
function reassembleFuzzyReplacement(oldText, newText, fileText, preserveUnmatched) {
  const oLines = oldText.split('\n');
  const nLines = newText.split('\n');
  const fLines = fileText.split('\n');
  const usesTabs = fLines.some((l) => indentPrefix(l).includes('\t'));
  // 1. old-line index → the file's actual indent bytes AND file line index.
  const fileIndentAt = new Array(oLines.length).fill(null);
  const fileLineAt = new Array(oLines.length).fill(-1);
  let fi = 0;
  for (let oi = 0; oi < oLines.length; oi++) {
    const sig = lineSignature(oLines[oi]);
    if (sig === '') continue;
    while (fi < fLines.length && lineSignature(fLines[fi]) !== sig) fi++;
    if (fi >= fLines.length) break;
    fileIndentAt[oi] = indentPrefix(fLines[fi]);
    fileLineAt[oi] = fi;
    fi++;
  }
  const transform = learnIndentTransform(oldText, fileText);
  // 2/3. Greedy forward walk of newText against oldText. Changed lines (no
  // signature match) anchor positionally: when both sides have the SAME line
  // count, new line i corresponds to old line i — the ubiquitous "same
  // block, a couple of lines changed" shape — so the changed line inherits
  // that old line's file position and indent bytes, and unclaimed file lines
  // (blanks the collapsed oldText never mentioned) flush in the right order
  // on both sides of it.
  const equalCount = nLines.length === oLines.length;
  const claimed = new Set(fileLineAt.filter((v) => v >= 0));
  const out = [];
  let oi = 0;
  let lastFileLine = -1; // highest file line claimed so far
    const emitUnclaimedUpTo = (target) => {
    if (!preserveUnmatched) return;
    const from = lastFileLine + 1;
    const to = Math.min(target === -1 ? fLines.length : target, fLines.length);
    for (let k = from; k < to; k++) {
      // Strip a trailing CR: the caller re-adds line endings uniformly via
      // join('\n') + replace(/\n/g, '\r\n') on CRLF files — keeping the raw
      // CR here would yield '\r\r\n' after that conversion.
      if (!claimed.has(k)) out.push(fLines[k].replace(/\r$/, ''));
    }
  };
  for (let ni = 0; ni < nLines.length; ni++) {
    const nl = nLines[ni];
    const sig = lineSignature(nl);
    if (sig === '') { out.push(nl); continue; }
    let matched = -1;
    for (let oj = oi; oj < oLines.length; oj++) {
      if (lineSignature(oLines[oj]) === sig) { matched = oj; break; }
    }
    const content = nl.slice(indentPrefix(nl).length);
    if (matched !== -1 && fileIndentAt[matched] !== null) {
      emitUnclaimedUpTo(fileLineAt[matched]);
      out.push(fileIndentAt[matched] + content); // unchanged content: file bytes verbatim
      lastFileLine = fileLineAt[matched];
      oi = matched + 1;
      continue;
    }
    // Changed line. Positional correspondence (equal counts) first.
    if (equalCount && ni < oLines.length && fileLineAt[ni] !== undefined && fileLineAt[ni] >= 0 && ni >= oi - 1) {
      const fpos = fileLineAt[ni];
      emitUnclaimedUpTo(fpos);
      out.push(fileIndentAt[ni] + content); // inherit the replaced line's file indent
      lastFileLine = fpos;
      oi = Math.max(oi, ni + 1);
      continue;
    }
    // Insertion (or unaligned change): emit after the previous claim with
    // the learned indent transformation in the file's own unit.
    const w = indentWidth(indentPrefix(nl));
    let target = w;
    if (transform) {
      target = transform.kind === 'ratio'
        ? Math.max(0, Math.round((w * transform.value) / 2) * 2)
        : Math.max(0, w + transform.value);
    }
    out.push(emitIndent(target, usesTabs) + content);
  }
  // Trailing unclaimed file lines inside the span (blanks after the last claim).
  emitUnclaimedUpTo(-1);
  return out.join('\n');
}

/**
 * Build a diagnostic-rich "oldText not found" message: locate the oldText's
 * first line (by trimmed prefix) in the current file so the model can read
 * the exact region and retry, plus batch position when part of edits[].
 */
function buildEditFailureMessage(text, oldText, failedIndex, appliedBefore, total) {
  const firstLine = oldText.split('\n').map((l) => l.trim()).find((l) => l !== '') || '';
  let near = null;
  if (firstLine) {
    const lines = text.split('\n');
    // Tiered prefix scan: the full trimmed line first (precise), then
    // progressively shorter prefixes — a paraphrased first line shares its
    // opening words with the real one ("function compute…"), and a short
    // prefix still localizes the region well enough to re-read.
    for (const width of [40, 20, 10, 5]) {
      const prefix = firstLine.slice(0, width);
      if (prefix.length < 4) break; // too short to be meaningful
      for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes(prefix)) { near = i + 1; break; }
      }
      if (near !== null) break;
    }
  }
  const parts = ['oldText not found'];
  if (near !== null) parts.push(`first line matches near line ${near} — read the file around there (offset=${Math.max(1, near - 2)}) and match the exact text incl. indentation`);
  else parts.push('no line in the file resembles the oldText first line — re-read the file and copy the exact bytes');
...
async function toolGrep({ pattern, path: cwd, glob: globPat, ignoreCase, literal, context, limit }, guard) {
  ...
  return {
    ...
    matches,
    kbHint: guard?.shouldHintSearchTool?.()
      ? guard.searchToolHint('grep', 'grep performs textual regex search by default; set literal=true for literal matching')
      : undefined,
  };
}
...
async function toolAstGrep({ pat, path: cwd, glob: globPat, limit }, guard, rt) {
  ...
  for (const f of files) {
    ...
    if (!ext || !SOURCE_EXT_RE.test('.' + ext)) continue;   // SOURCE_EXT_RE heuristic only
 ...
 * preview + proposalId, and stashes the writes for `toolResolve`. It reads
 * and walks the filesystem to build the preview, but never writes target
 * files itself; resolve(action="apply") performs the writes.
 *
...
async function toolAstEdit({ ops, paths, tag }, _guard) {
  ...
  for (const abs of allFiles) {
    ...
    if (tag && tag !== currentTag) {
      return { error: `stale tag for ${path.relative(process.cwd(), abs)}: the current file hash differs from the supplied KB-index snapshot tag (expected ${tag}, got ${currentTag})` };
    }
    let next = text;
    ...
    for (const op of ops) {
      ...
      // Every match is relative to the same input snapshot. Reject ambiguous
      // overlap, then splice from right to left so offsets remain valid and
      // replacement text can never be matched again by this operation.
      const sorted = matches.slice().sort((a, b) => a.startOffset - b.startOffset || a.endOffset - b.endOffset);
      for (let i = 1; i < sorted.length; i++) {
        if (sorted[i].startOffset < sorted[i - 1].endOffset) {
          return { error: `ast_edit pattern produced overlapping matches in ${path.relative(process.cwd(), abs)}; narrow the pattern` };
        }
      }
      for (const m of sorted.reverse()) {
        if (!Number.isInteger(m.startOffset) || !Number.isInteger(m.endOffset)
            || m.startOffset < 0 || m.endOffset < m.startOffset
            || next.slice(m.startOffset, m.endOffset) !== m.text) {
          return { error: `ast_edit received invalid match offsets for ${path.relative(process.cwd(), abs)}` };
        }
        const replacement = substituteTemplate(op.out, m.meta, m.anonymousCaptures);
        next = next.slice(0, m.startOffset) + replacement + next.slice(m.endOffset);
        fileChanged = true;
      }
    }
    ...
    if (!hunks) {
      ...
      return {
        error: `ast_edit diff preview for "${rel}" exceeds the full-file LCS budget; reducing replacement scope will not help—exclude this file or use a non-LCS edit tool`,
      };
    }
...
async function toolResolve({ proposal_id, action }) {
  ...
  for (const f of entry.files) {
    ...
    if (f.tag && shortHash(current) !== f.tag) {
      ...
      proposals.drop(proposal_id);
      return {
        error: `stale tag for ${f.path}: the current file hash differs from the supplied KB-index snapshot tag`,
...
/** Replace named captures and anonymous wildcards in pattern order. */
function substituteTemplate(tmpl, meta, anonymousCaptures = []) {
  let out = '';
  let i = 0;
  let anonymousIdx = 0;
  while (i < tmpl.length) {
    if (tmpl[i] === '$') {
      if (tmpl[i + 1] === '$' && tmpl[i + 2] === '$') {
        ...
        // Anonymous wildcards are paired by occurrence with those in `pat`.
        out += anonymousCaptures[anonymousIdx++] ?? '';
        i = j; continue;
      }
      if (tmpl[i + 1] === '_') {
        out += anonymousCaptures[anonymousIdx++] ?? '';
...
function makeDiffHunks(prev, next) {
  const a = prev.split('\n');
  const b = next.split('\n');
  // Full-file LCS allocates one DP cell per line pair. Check before any
  // matrix allocation; replacement count and match scope do not reduce it.
...
async function toolBash({ command, timeout }, guard) {
  ...
  // tool in this LLM call, prepend a one-time hint to stderr nudging toward
...
function toolKbSearch(rt, llm) {
  return async ({ query, top_k, skip_rewrite, with_slice = true }, guard) => {
    ...
    // The end-of-turn KB-update offer catches the
    // "KB is stale" case, not the per-call hint; it also syncs parser-owned
    // doc:<relpath> Eden entries, not only newly discovered symbols.
    guard?.noteKbUsage?.();
    ...
    const slices = sliced.length > 0 ? await loadSlices(sliced, rt.sourceConfig) : null;
    ...
    // must suppress them exactly as it suppresses a read(). Index-derived
    // fields remain visible. Signatures are source-derived and may contain
    // parameter/type/default details; this path does not re-filter them as
    // source content.
    const deniedPaths = [];
    ...
    if (results.length > 0) {
      ...
      allowed = await resolveKbContentPaths(filePaths, rt.sourceConfig);
 ...
 */
async function loadSlices(results, sourceConfig) {
  ...
  await Promise.all(results.map(async (r, i) => {
    ...
    for (const abs of kbPathCandidates(r.filePath, sourceConfig)) {
...
function toolKbSymbol(rt) {
  return async ({ name }, guard) => {
    ...
    // Index-derived fields remain visible. Signatures are source-derived and
    // may contain parameter/type/default details; this path does not
    // re-filter them as source content.
    const fps = [...new Set(syms.map(s => rt.getFilePath(s.fileId)).filter(Boolean))];
    const absByFp = await resolveKbContentPaths(fps, rt.sourceConfig);
...
/**
 * File outline metadata comes from the loaded KB index without reading source
 * content. Permission filtering may still resolve paths and inspect
 * filesystem metadata. Kb-first alternative to `read` for "what's in this
 * file?" questions.
 */
function toolKbOutline(rt) {
  return async ({ path: p }, guard) => {
    ...
    const contentAbs = await resolveKbContentPath(p, rt.sourceConfig);
...
function toolKbClass(rt) {
  return async ({ name, qual_name }, guard) => {
    ...
    // Index-derived fields remain visible. Signatures are source-derived and
    // may contain parameter/type/default details; this path does not
    // re-filter them as source content. Same pattern as kb_symbol/kb_outline;
    // this was the last content outlet not routing through filterAllowedPaths.
    const contentAbs = await resolveKbContentPath(node.filePath, rt.sourceConfig);
    const allowedSet = await filterAllowedPaths(contentAbs ? [contentAbs] : []);
    const contentAllowed = Boolean(contentAbs && allowedSet.has(contentAbs));
 ...
 * Flat token-overlap match over one combined haystack (id/title/intro/keywords)
 * with no field weighting; the heavy BM25 path stays over code symbols.
 */
 ...
 * Recommended model shape is 2-5 steps with 2-4 strategies and one recommendation.
 * Runtime normalization only requires at least 2 usable steps and at least 2 named
 * strategies per step; it accepts larger arrays and normalizes zero or many
 * recommendations by selecting the first. Returns null below those minimums.
 */
 ...
 * each step with 2-4 candidate strategies, one marked recommended — the prompt
 * recommended shape; normalization enforces only minimums and no maximum) when
 * it decides a task benefits from explicit strategy decomposition.
 *
...
function toolPlan(confirmFn) {
  return async ({ summary, steps }, _guard) => {
    ...
    if (!plan) {
      return {
        error: 'Invalid plan. Provide at least 2 usable steps; each step needs a non-empty goal and at least 2 named strategies.',
      };
    }
 ...
 *
 * After a plan is accepted, the agent calls `plan_step`. With a `stepFn`, the
 * live progress panel above the status bar advances. The supplied index is a
 * reporting hint only — the state machine deliberately ignores it for the
 * mutation and always advances the current in-progress step. When no `stepFn`
 * is supplied (non-interactive mode) no real progress state is kept; the call
 * only acknowledges the reported step.
 *
 *   stepFn(stepIndex, note) -> Promise<void>
 *     stepIndex is passed through as a reporting hint; the interactive callback
 *     advances the current in-progress step rather than selecting by index.
 */
/**
 ...
 * when absent (non-interactive) the call returns ok:false — the model is
 * told to state the fact in its reply instead of relying on persistence.
 * Content-agnostic by design: any short string is a valid fact.
 */
  ...
  return async ({ fact } = {}, _guard) => {
    ...
    try {
      ...
      return {
        ...
        hint: 'After successful persistence it is in scope via the "## Session facts" system message and survives context compaction by design. Record FACTS ONLY (addresses, versions, constraints, preferences) — code-discovery knowledge belongs in the KB via kb_save_knowledge.',
      };
    } catch (err) {
...
function toolPlanStep(stepFn) {
  return async ({ step, note } = {}, _guard) => {
    // `marked` is the 1-based current step the callback actually advanced
    // (or null when there is no active plan). The supplied step is only a
    // reporting hint; the callback/state machine ignores it for selection.
    let marked = null;
    ...
    if (marked) {
      return {
        ...
        hint: 'Call plan_step again after finishing the next step. In interactive mode, when all steps are done the progress panel clears automatically.',
      };
    }
    if (typeof stepFn === "function") {
    ...
    return {
      ok: true,
      message: idx
        ? `No interactive progress state is available; reported completion of plan step ${idx}.`
        : "No interactive progress state is available; completion was acknowledged only.",
      hint: 'No interactive progress state is kept in this mode.',
    };
  };
}
 ...
 * @param {{allowWrite?: boolean, llm?: object, multimodal?: boolean}} opts
 ...
 *   - multimodal: when true (session model has --multimodal=on), media reads
 *          return attach markers instead of binary-rejection errors.
 */
export function buildTools(rt, opts = {}) {
  ...
  // to agent-proposed Holy entries (always) or Eden (when auto-learn is off).
  // Without it, Holy writes are REFUSED rather than silently persisted - the source of truth
  ...
  // Multimodal input flag of the session's active model (--multimodal=on).
  // When true, the `read` tool returns attach markers for media files so the
  // turn pipeline can inject real content blocks into the conversation.
  const multimodal = opts.multimodal === true;
  // Multimodal vision runtime for the vision tool suite (see
  // lib/agent/vision_tools.js): resolved per turn by the caller (turn.js) as
  //   tools.json visionModelRef → session model (when multimodal) → none.
  // When provided AND a vision model resolves, the ten vision tools are
  // appended — they internally call the DEDICATED multimodal model, so the
  // session model itself does NOT need multimodal input to use them.
  // Pre-built by the caller (buildVisionTools is async) and passed as an
  // array of already-shaped tool objects.
  const visionTools = Array.isArray(opts.visionTools) ? opts.visionTools : [];

  const tools = [
    {
      name: 'read',
      snippet: 'Read a UTF-8 text file (KB outline may be prepended for eligible indexed source or documentation files); image/video/audio files attach as multimodal content blocks on capable models',
      guidelines: [
        'Use read to examine files instead of cat or sed. It reads text files; image/video/audio files are never decoded as text — on multimodal models they attach automatically (see below), otherwise the read is rejected with a fix hint.',
        'When the active model has multimodal input on, reading an image/video/audio file returns an `attach` marker: the media is automatically injected into the conversation as content blocks — analyze the real content directly.',
        'For eligible indexed source or documentation files, read prepends a structural Outline (from KB) section. Use kb_outline first when you only need the structure — it is cheaper.',
      ...
      description: `Read a UTF-8 text file. Files larger than 5 MiB are rejected before reading (offset/limit cannot bypass this). Files whose first 8192 decoded characters contain a NUL byte are rejected as binary (a NUL-scan heuristic, not full binary-format detection). Image (png/jpg/jpeg/gif/webp/bmp), video (mp4/mov/mkv/avi/webm/flv/m4v) and audio (wav/mp3/m4a/aac/ogg/flac/opus) files are never read as text: with multimodal input enabled the read returns an attach marker and the media is automatically sent to the model as content blocks; without multimodal input the read is rejected with a fix hint. Output is capped at ${MAX_READ_LINES} lines or roughly ${Math.floor(MAX_READ_BYTES / 1024)}KB at line boundaries, except that the first requested line is still emitted even when that one line exceeds the nominal byte cap; use offset/limit to continue reading eligible text files. For eligible indexed source or documentation files, an outline from the KB may be prepended unless outline=false.`,
      parameters: {
        type: 'object',
        properties: {
          ...
          outline: { type: 'boolean', description: 'Prepend KB outline for eligible indexed files (default true)' },
        },
        required: ['path'],
      },
      execute: (args, guard) => toolRead(args, guard, rt, multimodal),
    },
    {
      name: 'bash',
    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          path: { type: 'string', description: 'Directory OR single file to search (default: current directory)' },
    ...
    },
    {
      ...
      guidelines: [],
      description: `Search file contents for a pattern. Returns matching lines with file paths and line numbers. Output is truncated to 100 matches. Long lines are truncated to 240 chars. The search covers up to 2000 files per call; when the cap is hit the result carries filesTruncated + a note. Prefer KB search for semantic concepts rather than plain textual pattern matching.`,
      parameters: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: `Structural rewrite across one or more files. Computes proposed writes (never writes to disk itself), returns a unified-diff preview plus a proposalId, then stashes the writes for resolve. The optional tag parameter is checked against every target file, so it is intended for single-file rewrites. Each op is {pat, out}: pat uses ast-grep metavariables ($$$IDENT, $IDENT, $_), out uses the same syntax to substitute captures. Directory walks are capped at 2000 files; hitting the cap fails without staging a partial proposal. Returns { proposed, proposalId, diffs[], summary }. If no matches, returns proposed:false with summary zeroed.`,
      parameters: {
        type: 'object',
        properties: {
          ops: {
          ...
          tag: { type: 'string', description: 'Optional shortHash tag from prior read/kb_outline; rejects if any target hash differs from the supplied snapshot' },
        },
        required: ['ops', 'paths'],
      },
      execute: toolAstEdit,
    ...
    {
      name: 'plan',
      snippet: 'Propose an execution plan when a complex task benefits from an explicit strategy decomposition',
      guidelines: [
        'Call `plan` ONLY when you decide a complex task benefits from an explicit strategy decomposition before execution. With an interactive confirmation callback, the user chooses strategies; without one, recommended strategies are auto-accepted and the result includes `autoAccepted:true`.',
        ...
        'In interactive mode the tool returns the plan after the user chooses strategies; without a confirmation callback it returns `confirmed:true` with the recommended strategies auto-accepted and `autoAccepted:true`. If the user cancels, stop and ask how to proceed instead of executing the cancelled plan.',
        'Do NOT start editing files before the `plan` tool returns; do not ask the user to confirm a plan in prose — an interactive `plan` callback is the confirmation interface when one exists.',
      ],
      description: 'Propose an execution plan when a complex task benefits from explicit strategy decomposition. The model-visible schema requires `summary` and `steps`; the prompt-recommended shape is 2-5 ordered steps, each with 2-4 strategies and exactly one recommended strategy. With an interactive confirmation callback, the user chooses strategies and the result is `{ confirmed:true, plan }`; without one, recommended strategies are auto-accepted and the result is `{ confirmed:true, plan, autoAccepted:true }`. A cancellation returns `{ cancelled:true }`. The recommendation is not a runtime hard limit: defensive normalization for direct/internal calls that bypass the schema requires at least 2 usable steps and 2 named strategies per step, has no maximum, normalizes a missing/non-string summary to an empty string, and makes the first strategy recommended when the count is not exactly one. An unusable shape returns `{ error }`.',
      parameters: {
        type: 'object',
        properties: {
          summary: { type: 'string', description: 'Required by the tool schema; recommended one-line summary of the whole plan. Direct executor calls normalize a missing or non-string value to an empty string.' },
          steps: {
            type: 'array',
            description: 'Recommended shape: 2-5 ordered steps. Internal normalization enforces a minimum of 2 usable steps and no maximum.',
            minItems: 2,
            items: {
              type: 'object',
              properties: {
                ...
                strategies: {
                  type: 'array',
                  description: 'Recommended shape: 2-4 candidate strategies. Normalization enforces a minimum of 2 named strategies and no maximum.',
                  minItems: 2,
                  items: {
                    type: 'object',
                    properties: {
    ...
    {
      name: 'plan_step',
      snippet: 'Report completion of a plan step; interactive mode advances the live progress panel',
      guidelines: [
        "In interactive mode, call `plan_step` exactly once after finishing a confirmed plan step to advance the live progress panel; without a progress callback it only acknowledges the report and maintains no progress state.",
        "With an interactive progress callback, each call advances the CURRENT in-progress step — the `step` argument is accepted for compatibility but the state machine deliberately ignores it for the mutation (no jumping to arbitrary steps). Without that callback there is no current-step mutation; the call only acknowledges the report. Call once per finished step.",
        ...
        "In interactive mode, when the last step is marked done the progress panel clears automatically - no separate finish call is needed. Without a progress callback there is no panel to clear.",
      ],
      description: "Report completion of the CURRENT in-progress step of a confirmed plan. With an interactive progress callback this advances the real progress panel; without a progress callback it only acknowledges the reported completion and maintains no progress state. The `step` argument is a reporting hint and is deliberately ignored for mutation. Only interactive mode has a panel that can clear after the last step; a normal turn end finalizes any leftover interactive panel as a backstop.",
      parameters: {
        type: 'object',
        properties: {
    ...
    {
      name: 'remember',
      snippet: 'Persist a session fact after successful storage (then it stays in scope and survives compaction)',
      guidelines: [
        ...
        "One fact per call, phrased self-contained ('staging endpoint 192.0.2.10', 'PostgreSQL 16.2', '用 npm 不用 yarn').",
        "When a later turn seems to have lost an earlier fact, check the \"## Session facts\" system message before asking the user again.",
      ],
      description: 'After successful persistence, a short self-contained fact is injected into subsequent turns via a standing "## Session facts" system message and survives context compaction by design. A missing callback or failed write returns failure instead of recording the fact. Use it for user-stated environment facts, constraints, and preferences — not for code knowledge (that belongs to kb_save_knowledge).',
      parameters: {
        type: 'object',
        properties: {
  ...
  if (allowWrite) {
    tools.push({
      ...
      guidelines: [
        'Use edit for precise changes; copy oldText exactly when possible. EOL and whitespace fallback require a unique matching anchor.',
      ...
      description: `Edit a single file using exact text replacement. Multi-edit entries are applied sequentially in array order against the evolving in-memory content; each oldText must be unique when its entry is processed unless replaceAll is true, and later entries may observe text produced by earlier ones. If a later entry fails, nothing is written to disk. Accepts either {edits:[{oldText,newText}]} (preferred, multiple edits in one call) or {old_string, new_string} (single edit). Optional \`tag\` (shortHash from a prior read/kb_outline) rejects the edit if the file's current content hash differs from the supplied KB-index snapshot tag. Exact matching falls back to EOL adaptation and then a unique whitespace-insensitive line sequence, reported in fuzzyAdapted. replaceAll replaces repeated exact or EOL-adapted anchors, but ambiguous fuzzy anchors still fail.`,
      parameters: {
        type: 'object',
        properties: {
          ...
          edits: {
            ...
            items: {
              type: 'object',
              properties: {
                oldText: { type: 'string', description: 'Text to find. Must be unique unless replaceAll is true; fuzzy matching always requires uniqueness.' },
    ...
    tools.push(
      {
        ...
        guidelines: ['Use kb_search instead of grep when looking for semantic concepts; grep performs textual regex search by default and literal search when literal=true. When an LLM is attached and skip_rewrite is not true, the query is rewritten before retrieval; pass skip_rewrite=true if you already have identifier-style keywords.'],
        ...
        parameters: {
          ...
          properties: {
            ...
            top_k: { type: 'number', description: 'Requested result budget: falsy values including 0 default to 10; other numeric values are normalized to 5-50. Actual results may be fewer when fewer matches exist.' },
      ...
      {
        name: 'kb_outline',
        snippet: 'File outline from the loaded KB index (no source-content read) — name/kind/lines/signature per symbol',
        guidelines: [
        ...
        description: `Get the symbol outline of a file FROM THE LOADED KB INDEX (no source-content read). Returns each symbol's name, qualified name, kind, line range, signature, parent class, and child count. Filesystem metadata/real-path checks may still occur for permission filtering. Use this when you need to know "what's in this file?" or "where is X defined in this file?" — much cheaper than read and stays in sync with the indexed codebase. Direct kb_outline does not use SOURCE_EXT_RE; any indexed file can be queried, including files with no symbols (which may still return a tag). If the file isn't in the KB, the call returns an explicit error pointing at /kb update.`,
        parameters: {
          type: 'object',
      ...
      },
      {
        ...
        guidelines: [],
        description: 'Search across both Holy and Eden knowledge spaces by natural-language query. Each whitespace token occurrence contributes at most one equal-weight point across one combined id/title/intro/keywords haystack; duplicate tokens can contribute again, ties preserve allKnowledge() order, and results are not filtered for superseded Eden entries. Falsy top_k values including 0 default to 5; other numeric values are bounded to 1-20. Use this to discover if the project KB already documents a concept before exploring code.',
        parameters: {
          type: 'object',
          properties: {
            query: { type: 'string', description: 'Natural-language query' },
            top_k: { type: 'number', description: 'Max results: falsy values including 0 default to 5; other numeric values are bounded to 1-20' },
          },
          required: ['query'],
        },
    ...
    if (opts.projectId) {
      tools.push({
        ...
        guidelines: [
          ...
          'Saving an agent-proposed entry to Holy prompts the user before commit, even with HK2_ENABLE_AUTO_LEARN=1; direct user commands have their own explicit-intent semantics. For NEW Holy entries the prompt is (y/N/E): y writes to Holy, N cancels, E saves to Eden instead. Updating an existing Holy entry prompts plain (y/N).',
        ...
        description: 'Persist a knowledge entry into Holy Space (stable, requires user approval) or Eden Space (frequently-updated, auto-learn eligible). Reloads into the in-memory KB runtime immediately, but does not clear the runLoop read-only cache: an identical kb_knowledge / kb_search_knowledge call that already produced a successful cached result earlier in this same runLoop may still return that cached result until a cache-busting call (bash/edit/write/ast_edit/resolve) or a new loop begins. Failed results are not cached.',
        parameters: {
          type: 'object',
          properties: {
  ...
  // Multimodal vision tools (ui_to_artifact, extract_text_from_screenshot,
  // ...) — appended AFTER the built-ins/KB tools, mirroring how MCP tools
  // extend the registry. Pre-built upstream (async) and passed whole.
