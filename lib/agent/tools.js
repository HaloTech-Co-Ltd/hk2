 ...
 * recommended shape; normalization enforces only minimums and no maximum) when it
 ...
 *
 * After a plan is confirmed, the agent calls `plan_step` so the live progress
 * panel above the status bar advances. The supplied index is a reporting
 * hint only — the state machine deliberately ignores it for the mutation and
 * always advances the current in-progress step. When no `stepFn` is supplied
 * (non-interactive mode) no real progress state is kept; the call only
 * echoes the reported step back.
 *
  ...
  const tools = [
    ...
    },
    {
      ...
      guidelines: [
        ...
        'In interactive mode the tool returns the user-confirmed plan (chosen strategy per step) which you must follow; without a confirmation callback it auto-accepts the recommended strategies and returns autoAccepted:true. If the user cancels, stop and ask how to proceed instead of executing the cancelled plan.',
  ...
  if (rt) {
    tools.push(
      ...
      },
      {
        ...
        guidelines: [],
        description: 'Search across both Holy and Eden knowledge spaces by natural-language query. Each whitespace token contributes at most one equal-weight point across one combined id/title/intro/keywords haystack; results are ranked by this flat overlap and are not filtered for superseded Eden entries. Use this to discover if the project KB already documents a concept before exploring code.',
        parameters: {
          type: 'object',
          properties: {
