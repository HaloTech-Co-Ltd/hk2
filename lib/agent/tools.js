  ...
  const tools = [
    ...
    },
    {
      ...
      guidelines: [
        ...
        "`step` is 1-based and matches the step numbering in the confirmed plan. Omit it to advance the current step.",
      ...
      description: "Mark a step of the currently confirmed plan as complete and advance the live progress panel. Use this AFTER finishing each step of a confirmed plan (the plan returned by the `plan` tool). `step` is 1-based; omit it to mark the current step. Returns a confirmation with a hint to call again for the next step. No-op when no plan is active.",
      parameters: {
        type: 'object',
        properties: {
          step: { type: "number", description: "1-based step number to mark done (defaults to the current step)" },
  ...
  if (allowWrite) {
    tools.push({
      ...
      guidelines: [
        ...
        'Set replaceAll:true only when you deliberately want every occurrence of a non-unique anchor replaced; keep anchors unique otherwise.',
      ],
      description: `Edit a single file using exact text replacement. Every edit's oldText must match a unique, non-overlapping region of the original file. If two changes affect the same block or nearby lines, merge them into one edit instead of emitting overlapping edits. Do not include large unchanged regions just to connect distant changes. Accepts either {edits:[{oldText,newText}]} (preferred, multiple disjoint edits in one call) or {old_string, new_string} (single edit). Optional \`tag\` (shortHash from a prior read/kb_outline) rejects the edit if the file's current content hash differs — protects against silent overwrites when the file changed since the read. When byte-exact matching fails, a whitespace-insensitive fallback matches the trimmed-content line sequence (indent width, tab↔space, trailing whitespace, EOL and blank-line drift are absorbed; the file's own indentation and line endings are preserved) — surfaced as fuzzyAdapted[]. Optional \`replaceAll\` replaces every occurrence of a non-unique anchor.`,
      parameters: {
        type: 'object',
        properties: {
    ...
    tools.push(
      {
        ...
        description: 'Search the project knowledge base by natural-language / keyword query. Returns symbols ranked by BM25 + name-match reranking, with file paths, line ranges, and snippets. By default the query is rewritten through the LLM into English function names + keywords before retrieval; pass skip_rewrite=true to skip that step (useful when you already have identifiers). By default the top 3 results also carry a ±15-line source slice (with_slice=false to disable) so you can usually judge or use the implementation without a follow-up read.',
        parameters: {
          type: 'object',
          properties: {
