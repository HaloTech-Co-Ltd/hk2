  ...
  return async ({ query, top_k, skip_rewrite, with_slice = true }, guard) => {
    ...
    const out = {
      ...
      results: results.map((r, i) => ({
        ...
        // Snippets trimmed to the first 200 chars — enough to judge relevance,
        // without letting 10 snippets dominate the payload.
...
async function loadSlices(results) {
  ...
  const root = process.env.HK2_PROJECT_SOURCE
  ...
  await Promise.all(results.map(async (r, i) => {
    ...
    for (const base of root) {
      const abs = path.resolve(base, r.filePath);
      let text;
      try {
  ...
  return async ({ name }, guard) => {
    ...
    return {
      ...
      symbols: syms.map(s => ({
        ...
        // often be answered without a follow-up read.
...
function toolKbSaveKnowledge(rt, projectId, confirm) {
  return async ({ space, id, title, intro, key_files, key_symbols, keywords }, guard) => {
    ...
    if (needsConfirm) {
      ...
      let approved = false;
      try { approved = !!(await confirm(targetSpace, { id, title, intro, keyFiles: key_files, keySymbols: key_symbols, keywords })); }
...
export function buildTools(rt, opts = {}) {
  ...
  const tools = [
    ...
    },
    {
      ...
      guidelines: [],
      description: `Search file contents for a pattern. Returns matching lines with file paths and line numbers. Output is truncated to 100 matches. Long lines are truncated to 240 chars. Prefer the KB tools (kb_search for semantic concepts, kb_symbol for exact names) over grep — grep is only for literal text the KB index can't answer.`,
      parameters: {
        type: 'object',
        properties: {
