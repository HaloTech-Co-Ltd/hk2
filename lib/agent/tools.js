...
async function walk(root, dir, segments, idx, out, limit) {
  ...
  if (dir !== root) {
    const svc = getPermissionService();
    const res = await svc.checkReal(dir, 'r');
    if (!res.ok) return; // prune the whole subtree
  }
  let entries;
  ...
  for (const ent of entries) {
    ...
    if (ent.isDirectory()) {
      if (seg === '**') {
        await walk(root, entPath, segments, idx, out, limit);
        await walk(root, entPath, segments, idx + 1, out, limit);
      } else if (matchSeg(seg, ent.name)) {
        await walk(root, entPath, segments, idx + 1, out, limit);
      }
    } else if (idx === segments.length - 1 && matchSeg(seg, ent.name)) {
      // Per-file 'r' check: a deny rule placed on the FILE itself (not its
      // parent dir) must suppress it from recursive listings too.
      const svc = getPermissionService();
      const fres = await svc.checkReal(entPath, 'r');
      if (!fres.ok) continue;
      out.push(entPath);
    }
  }
