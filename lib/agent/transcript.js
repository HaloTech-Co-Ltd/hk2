 ...
 *   - assistant_message {round, content, tool_calls} (exact loop round)
...
export class Transcript {
  ...
  append(event) {
    ...
    //
    // The job attaches to _tail SYNCHRONOUSLY, with _init() INSIDE the chain:
    // awaiting _init() before attaching left a window where a fire-and-forget
    // append followed by flush() resolved the OLD tail — the very last
    // transcript event could still be lost at process exit (review round 7).
    const job = this._tail.catch(() => {})
      .then(() => this._init())
  ...
  async logUser(text, opts = {}) {
    // Multimodal user turns: `content` may be an ARRAY of content blocks
    // (text + image_url / video_url / input_audio — see attachments.js).
    // Store the blocks as `content` (replay re-materializes them verbatim)
    // and keep `attachments` as the compact human-readable source list for
    // display tooling that does not want to touch the Base64 payloads.
    ...
    await this.append({ type: 'user', text, attachments: Array.isArray(opts.attachments) ? opts.attachments : undefined });
  }
  ...
  async logAssistantMessage(message, round) {
    await this.append({
  ...
  async logToolCall(call, result) {
    await this.append({
      ...
      ok: result.ok,
      // Loop-round index this call belongs to (issue #4): replayTranscript
      // groups consecutive tool_call events by round so sequential rounds
      // reconstruct as SEPARATE assistant.tool_calls messages instead of one
      // coalesced parallel-looking message. Older transcripts without this
      // field fall back to the coalescing behavior (backward compatible).
      round: typeof call.round === 'number' ? call.round : undefined,
 ...
 *
 * New transcripts store each exact assistant message before its tool_call
 * result events, so replay preserves round content and ordering. Legacy
 * transcripts stored only flat assistant / tool_call events; this function
 * retains their best-effort reconstruction path:
 *
 *   user "..."
 ...
 *
 * Consecutive tool_call events are grouped by their `round` field when
 * present (stamped by runLoop): calls sharing a round were issued as ONE
 * parallel batch → one assistant.tool_calls message per round; consecutive
 * rounds become SEPARATE messages, preserving the sequential causality of
 * the original session (issue #4). Older transcripts without `round` fall
 * back to coalescing all consecutive tool_call events into ONE synthesized
 * assistant message — the documented legacy trade-off: the flat event stream
 * cannot tell the rounds apart, and the grouped shape is still valid
 * OpenAI/Anthropic tool protocol.
 *
...
export function replayTranscript(text) {
  ...
  let structuredToolIds = null; // ids declared by the latest assistant_message
  ...
  const flushTools = () => {
    ...
    // Group by `round` when any event carries one: each round becomes its own
    // assistant.tool_calls message followed by that round's tool results —
    // mirroring exactly what runLoop pushed per iteration. Events without a
    // round (legacy lines mixed in) group with the previous batch.
    const groups = [];
    for (const c of pending) {
      const g = groups[groups.length - 1];
      if (g && (c.round === undefined || g.round === c.round)) g.calls.push(c);
      else groups.push({ round: c.round, calls: [c] });
    }
    for (const g of groups) {
      messages.push({
        role: 'assistant',
        content: '',
        tool_calls: g.calls.map(c => ({
          id: c.id,
          type: 'function',
          function: {
            name: c.name,
  ...
  for (const line of String(text || '').split('\n')) {
    ...
    switch (evt.type) {
      case 'user': {
        flushTools();
        structuredToolIds = null;
        // Multimodal user turns store `content` as an array of content
        // blocks (text + image_url / video_url / input_audio). Replay them
        // VERBATIM — the blocks are exactly what the multimodal API expects
        // in messages[].content, so the resumed conversation keeps its
        // images / video / audio. Legacy (and plain) turns carry `text`.
        if (Array.isArray(evt.content)) {
          ...
          const txt = evt.content
            ...
            .map(b => b.text)
            .join('\n');
          lastUserText = txt || lastUserText || null;
          break;
        }
      ...
      case 'assistant_message': {
        flushTools();
        const toolCalls = Array.isArray(evt.tool_calls) ? evt.tool_calls : [];
        const message = { role: 'assistant', content: String(evt.content ?? '') };
        if (toolCalls.length > 0) message.tool_calls = toolCalls;
        messages.push(message);
        structuredToolIds = new Set(toolCalls.map(call => call?.id).filter(Boolean));
        break;
      }
      case 'tool_call': {
        if (structuredToolIds?.has(evt.id)) {
          messages.push({
            role: 'tool',
            tool_call_id: evt.id,
            name: evt.name,
            content: JSON.stringify(evt.result ?? { error: 'result missing from transcript' }),
          });
          structuredToolIds.delete(evt.id);
          if (structuredToolIds.size === 0) structuredToolIds = null;
        } else {
          pending.push(evt);
        }
        break;
