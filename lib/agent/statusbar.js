...
export class StatusBar {
  ...
  update() {
    ...
    if (grew || shrank) {
      ...
      if (park) {
        // The workspace geometry just changed (plan grew/shrank mid-turn), so
        // the saved slot is stale: reset it to the NEW workspace bottom, then
        // re-dock the real cursor into the box.
        this._docked = true;
        seq += `\x1b7${park}`;
      }
    } else if (park) {
