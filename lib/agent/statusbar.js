...
// DECTCEM cursor hide/show. ONE tty write is not guaranteed to be ONE rendered
// frame: pty drivers, ConPTY and terminal-side read chunking can split a single
// write, and terminals render per chunk. Every periodic multi-motion sequence
// (probe, heal, resize, diff repaint) passes the visible cursor through BLANK
// positions — DECSTBM homes it to (1,1), DECRC jumps to the (often blank)
// workspace continuation row, the heal tail parks it on a fresh bottom row —
// so a split at any of those points rendered "a cursor flashing by in blank
// space". Bracketing with hide/show makes every intermediate frame CURSOR-LESS
// instead (same technique as src/tui/frame.js reflow). Kept inside the same
// single write as the sequence it guards, so the pairing can never tear.
...
export class StatusBar {
  constructor(stream = process.stderr, opts = {}) {
    ...
    // One-frame atomicity buffer: _resizePass stashes its DECSTBM region
    // assertion here so update() can emit prefix + repaint body as a SINGLE
    // tty write (see _resizePass — the periodic input-box cursor-blink law:
    // any multi-part control sequence that must be perceived as one frame
    // must leave as one write, exactly like the slot-probe sequence).
    this._pendingPrefix = '';
    ...
    rl._refreshLine = (...args) => {
      ...
      if (gateFn && gateFn()) {
        ...
        if (menu && typeof menu.prompt === 'string' && menu.prompt) {
          ...
          this._write(`${CURSOR_HIDE}\r\x1b[2K${menu.prompt}${line}${this._menuCursorBack(line, cursor)}${CURSOR_SHOW}`);
        }
        return;
      }
      const r = orig(...args);
  ...
  refreshInputLine() {
    ...
    // DECTCEM bracket (same law as update()/probe): the rewrite erases the
    // input row (\x1b[2K) before drawing it back, and a transport split in
    // that window rendered the caret standing on a BLANK row for one frame —
    // the per-keystroke variant of the blank-space flash.
    this._write(`${CURSOR_HIDE}${park ? '' : '\x1b7'}\x1b[${r};1H\x1b[2K${inputLines[0]}${park || '\x1b8'}${CURSOR_SHOW}`);
  ...
  _resizePass({ heal = false } = {}) {
    ...
    const park = this.parkSeq();
    // ATOMICITY: the region assertion and the repaint body must reach the
    // terminal as ONE write. DECSTBM homes the cursor per VT spec; emitting
    // it as its own write let the tty driver flush between the two writes,
    // and terminals render per flush — the frame between them showed the
    // cursor parked in blank space (top-left, or wherever DECSTBM homed it)
    // for a few ms before the repaint re-docked it into the input box. With
    // the slot watchdog healing on every probe cycle (~5s base cadence),
    // that intermediate frame WAS the periodic "add instruction" input-box
    // cursor blink. The prefix is therefore stashed in _pendingPrefix and
    // prepended to update()'s single write instead of written separately
    // (same law as the slot-probe sequence: lib/agent/slot_probe.js).
    this._pendingPrefix = `${park ? '' : '\x1b7'}\x1b[1;${scrollBottom}r`;
    try {
      ...
      // update() bailed before writing (bar disabled / implausible
      // geometry): flush the region assertion alone so the terminal state
      // matches what the previous two-write path produced in that case.
      if (this._pendingPrefix) {
        const p = this._pendingPrefix;
        this._pendingPrefix = '';
        this._write(CURSOR_HIDE + p + CURSOR_SHOW);
      }
    }
  }
  ...
  update({ skipCursorSave = false, force = false, heal = false } = {}) {
    ...
    if (diffable) {
      ...
      this._lastRows = rows;
      // Bracket with DECTCEM only when rows are actually being repainted: the
      // repaint jumps the cursor through the homed (1,1) position on its way to
      // the changed rows and back into the box. When nothing changed (steady
      // idle ticks) the write is region+tail only and the caret keeps its
      // native blink — a hide/show pair every 200ms would restart the blink
      // phase and freeze the caret visually solid.
      const body = this._takePendingPrefix(dseq);
      this._write(changed.length > 0 ? CURSOR_HIDE + body + CURSOR_SHOW : body);
      return;
    }
    ...
    // Full repaints (heal / resize / grow-shrink reflow / force) move the
    // cursor the furthest — through (1,1), across cleared blank rows, and (on
    // heal) onto a fresh bottom row — so they take the DECTCEM bracket too.
    this._write(CURSOR_HIDE + this._takePendingPrefix(seq) + CURSOR_SHOW);
  }
  ...
  poll(ms) {
    ...
    if (ms > 0 && this.enabled && this._started) {
      ...
      const slotTick = () => {
        if (this._docked && this._slotProbeFn) {
          // Silent-terminal backoff: a terminal that never answers DSR can
          // never prove health, and every probe conservatively heals (full
          // repaint + cursor re-dock) — perceived as the input-box cursor
          // blinking on a fixed cadence. Each silent round doubles the probe
          // period (25 → 50 → 100 → 200 → 400 ticks, i.e. ~5s → ~80s, capped)
          // so a DSR-deaf terminal settles into a quiet rhythm; any ANSWER
          // (healthy or drift-proven) restores the base cadence. The heal
          // itself still fires on every silent probe (conservative safety).
          const period = 25 << Math.min(this._slotSilentStreak || 0, 4);
          if ((this._slotCheckCounter = (this._slotCheckCounter || 0) + 1) % period === 0) {
   ...
   *
   * opts.silent marks a NO-ANSWER verdict: the probe cadence backs off (see
   * slotTick) so a DSR-deaf terminal stops full-healing every ~5s, while an
   * answer-backed verdict (healthy or drift) resets the cadence to base.
   */
  slotProbeResult(healthy, rowsArg, opts = {}) {
    if (!this.enabled || !this._started) return;
    // Every verdict restarts the cadence clock — the next probe is a full
    // period away, so the backoff arithmetic stays independent of where in
    // the old period the verdict landed.
    this._slotCheckCounter = 0;
    if (healthy) {
      this._slotSilentStreak = 0;
      return;
    }
    this._slotSilentStreak = opts?.silent ? (this._slotSilentStreak || 0) + 1 : 0;
  ...
  /**
   * Prepend (and clear) the write prefix stashed by _resizePass so the
   * region assertion and this repaint leave as ONE tty write; returns `seq`
   * unchanged when no prefix is pending (the common path — direct update()
   * callers never set one).
   */
  _takePendingPrefix(seq) {
    const p = this._pendingPrefix;
    this._pendingPrefix = '';
    return p ? p + seq : seq;
  }
