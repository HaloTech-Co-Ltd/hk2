...
export class StatusBar {
    ...
    rl._refreshLine = (...args) => {
      ...
      if (gateFn && gateFn()) {
        ...
        if (menu && typeof menu.prompt === 'string' && menu.prompt) {
          ...
          this._write(`\r\x1b[2K${menu.prompt}${line}`);
        }
        return;
      }
      const r = orig(...args);
      try { this.update(); } catch { /* repaint is best-effort */ }
      return r;
    };
  ...
  _resizePass() {
    ...
    this.update({ skipCursorSave: true });
  }
  ...
  update({ skipCursorSave = false } = {}) {
    ...
    // Cache the new counts; handleResize() uses them to size the region.
    ...
    // Real cursor docked in the input box? Then this repaint must NOT emit a
    // leading \x1b7: the DECSC slot currently holds the workspace continuation
    // position (saved by armInputBox / the write-router), and saving here would
    // clobber it with the dock position — the next workspace write would then
    // land ON the input row. Legacy path (no dock) keeps the save/restore pair.
    const park = this.parkSeq();
    ...
    if (grew || shrank || heightChanged) {
      if (park) {
        ...
        seq += `\x1b[${scrollBottom};1H\x1b7\r\x1b[2K${prompt}${line}`;
  ...
  _rows() {
    ...
    // row on the NEW screen (the "status bar mid-screen" bug). When the draw
    // stream carries no rows (tests, exotic redirections) fall back to
    // stdout, then stderr, then 24.
    return this.stream?.rows || process.stdout.rows || process.stderr.rows || 24;
  }
