...
export class StatusBar {
  ...
  /**
   * Install the continuation-slot probe used by the poll watchdog. fn(rows)
   * queries the terminal cursor (Device Status Report) and reports back via
   * slotProbeResult(healthy). interactive.js supplies the stdin listener
   * that decodes ESC[r;cR answers; tests can inject a synchronous fake.
   */
  setSlotProbeFn(fn) {
  ...
  /**
   * Receive a decoded DSR row from interactive.js's stdin listener and
   * judge it against the current geometry: rows inside the scroll workspace
   * are healthy; rows inside the reserved block prove slot drift (heal).
   */
  slotProbeReport(row) {
    const rows = this._rows();
    if (rows <= 1) return;
    const scrollBottom = Math.max(1, rows - 1 - this._planLineCount - this._inputLineCount);
    const healthy = Number.isFinite(row) && row >= 1 && row <= scrollBottom;
