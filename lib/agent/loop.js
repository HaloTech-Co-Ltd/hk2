...
export async function runLoop(opts) {
  ...
  while (turns < maxTurns) {
    ...
    const stream = llm.stream(messages, {
      ...
      tools: toolSpec,
      signal,
      // Transient-error retry feedback (fetchWithRetry): lets the UI say
      // 'retrying model request · attempt 2/3' instead of looking stuck.
      onRetry: (attempt, delayMs, err, attempts) => callbacks.onRetry?.(attempt, delayMs, err, attempts),
    });
    ...
    if (thisKey === lastToolKey && thisFingerprint === lastResultFingerprint) {
      ...
      if (identicalRepeatCount >= STUCK_REPEAT_LIMIT) {
        throw new Error(`agent stuck: ${STUCK_REPEAT_LIMIT} identical tool-call rounds with identical results (no progress). Last signature: ${thisKey}`);
      }
    } else {
