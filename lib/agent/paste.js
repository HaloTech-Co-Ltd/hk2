  ...
  start() {
    ...
    this._kp = (str, key) => {
      ...
      if (key.name === 'paste-start') {
        this.pasting = true;
        this.buf = [];
        // Snapshot the edit state the paste started from. Some Node readline
        // versions append bracket-pasted chars at EOL, while others splice
        // them at the cursor. Paste-end handles both shapes below.
        ...
        if (isMultiLine) {
          ...
          // cursor.
          //
          // Node's bracketed-paste behavior varies by version: some append
          // the pasted chars at EOL, while others already splice them at the
          // cursor. Forcing `cursor = line.length` broke the latter behavior;
          // leaving the former unrepaired broke mid-line edits. Terminals
          // that wrap IME commits as bracketed pastes (Windows Terminal with
          // a CJK IME, several SSH clients) exposed both failure modes.
          //
          ...
          // user actually pasted into. If neither known shape matches, leave
          // readline's line and cursor untouched.
          if (this.rl) {
            const from = this._pasteFrom;
            if (from) {
              const inserted = from.line.slice(0, from.cursor) + text + from.line.slice(from.cursor);
              if (this.rl.line === from.line + text) {
                // Older Node: readline appended the paste at EOL; repair it.
                this.rl.line = inserted;
                this.rl.cursor = from.cursor + text.length;
              } else if (this.rl.line === inserted) {
                // Newer Node: readline already spliced correctly. Preserve
                // that line and its logical post-paste cursor.
                this.rl.cursor = from.cursor + text.length;
              }
            }
