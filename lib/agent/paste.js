    ...
    this._kp = (str, key) => {
      ...
      if (key.name === 'paste-start') {
        ...
        if (isMultiLine) {
          ...
          // user actually pasted into. Some supported Node releases already
          // splice the text correctly but still leave the cursor at the end;
          // recognize that shape too and restore the intended cursor. Anything
          // unexpected falls back to the legacy end-of-line behavior.
          if (this.rl) {
            const from = this._pasteFrom;
            const inserted = from
              ? from.line.slice(0, from.cursor) + text + from.line.slice(from.cursor)
              : null;
            if (from && (this.rl.line === from.line + text || this.rl.line === inserted)) {
