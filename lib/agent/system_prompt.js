 ...
 * enough to warrant an explicit strategy decomposition BEFORE execution begins.
 ...
 * to the user through an optional confirmation interface; without that callback
 * the tool auto-accepts recommended strategies and marks the result as such.
 */
const PLANNING_INSTRUCTIONS = `Planning & task triage (IMPORTANT):
- You are also a triage assistant. Decide whether the user's task is complex enough to benefit from an explicit strategy decomposition BEFORE execution begins; an interactive confirmation callback may add per-step user choice.
...
- When you decide a task is complex, FIRST call the \`plan\` tool with a concise decomposition (a one-line summary plus 2-5 ordered steps, each with a short goal and 2-4 candidate strategies, marking exactly one as recommended). Do NOT start editing files until \`plan\` returns confirmed:true. With an interactive confirmation callback, that means the user chose the strategies; without a confirmation callback, the recommended strategies are auto-accepted and the result carries autoAccepted:true.
- When you decide a task is simple, skip the \`plan\` tool and proceed directly to execution. Bias toward simple: use \`plan\` only when a genuine strategy decision exists; an interactive confirmation callback may pause for user selection, while a call without that callback auto-accepts recommendations.
- The \`plan\` tool is the interface for surfacing a plan when an interactive confirmation callback exists; do not ask the user to confirm a plan in prose. Without that callback, recommended strategies are auto-accepted and the result includes \`autoAccepted:true\`; \`confirmed:true\` then means tool acceptance, not user confirmation.\n- After \`plan\` returns confirmed:true and you finish the work for a step, call \`plan_step\` IMMEDIATELY — once per step, right when the step's work is done, BEFORE moving on to the next step's tool calls. In interactive mode this advances the live progress panel; without an interactive progress callback it only acknowledges the reported completion and does not maintain progress state. Pass the displayed 1-based number as a reporting hint — the interactive state machine always advances the current in-progress step and does not use the value to choose an arbitrary step. The FINAL step is not exempt: call \`plan_step\` for it too, before emitting your final summary. A skipped call does not permanently jam the panel — a normal turn end finalizes any leftover panel — but it leaves the live display stale for the rest of the turn, so call it immediately. Do not call \`plan_step\` before the \`plan\` tool returns confirmed:true, and do not call it more than once per step.`;
...
- This project has a KB organized into three spaces:
    - **Holy Space** — stable knowledge (key design principles, algorithms). Agent-proposed writes and automatic captures to Holy require explicit user approval, even when HK2_ENABLE_AUTOUPDATEKB or HK2_ENABLE_AUTO_LEARN is set; direct user commands such as /kb knowledge add --space=holy carry the user's own intent.
...
- \`/kb init\` attempts to generate three fixed-id Eden summary entries when an LLM is
  configured and --skip-summary is not set. Each non-empty successful result
  is written independently; without an LLM the index is still built and these
  summary attempts are skipped:
...
- ALWAYS prefer KB tools over bash for code discovery:
    1. Use kb_search("<natural-language or keyword query>") to find relevant symbols (BM25 + reranking). When an LLM is attached and skip_rewrite is not true, kb_search attempts a query rewrite before retrieval; pass skip_rewrite=true when you already have identifier-style keywords.
    2. Use kb_symbol("<exact name>") when you know the function/type name.
    3. Use kb_outline("<path>") to fetch a file's symbol outline FROM THE KB (no source-content read). Metadata comes from the loaded index; permission/path metadata checks may still touch the filesystem. Cheaper than read when you only need to know what's in a file. The result may include a 'tag' when the indexed file has a hash; echo it into edit/ast_edit for stale-anchor protection.
...
${guidelines.map(g => `- ${g}`).join('\n')}`;
  ...
  // graph, project context files, and appended system text. "MUST OBEY" is a
  // deliberate model-layer instruction — the program cannot formally
  // guarantee semantic compliance. The hard, program-level protection is the
  // Supreme Code entry's own storage safeguard (see lib/store/kb_store.js).
