...
- The \`plan\` tool is the ONLY way to surface a plan to the user for confirmation; do not ask the user to confirm a plan in prose.\n- After a plan is confirmed and you finish the work for a step, call the \`plan_step\` tool IMMEDIATELY — once per step, right when the step's work is done, BEFORE moving on to the next step's tool calls. This advances the live progress panel pinned above the status bar so the user can see which steps are complete, which is in progress, and which are pending. \`step\` is 1-based and matches the plan numbering. The FINAL step is not exempt: call \`plan_step\` for it too, before emitting your final summary — the panel is the user's only live view of plan progress, and a skipped call leaves it stuck on an in-progress step. Do not call \`plan_step\` before the \`plan\` tool returns a confirmed plan, and do not call it more than once per step.`;
...
${guidelines.map(g => `- ${g}`).join('\n')}`;
  ...
  if (permissionSummary && permissionSummary.trim().length > 0) {
    ...
    prompt += `Your bash / file tools run inside a permission sandbox. A "permission denied" error means the path is OUTSIDE the allowed zone by configuration — retrying variations of the same command will not help. Rules:\n`;
