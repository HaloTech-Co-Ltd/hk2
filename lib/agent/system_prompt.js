...
${guidelines.map(g => `- ${g}`).join('\n')}`;
  ...
  if (permissionSummary && permissionSummary.trim().length > 0) {
    ...
    prompt += `- If a task truly needs a path outside the allowed zone (or a denied mode), STOP and tell the user which path/mode is needed — they can grant it via an \`allow\` rule in setting.json. Do NOT try to bypass (symlinks, cd tricks, command substitution — these are checked too).\n`;
