/**
 * Code Review: after a normal agent return whose plan-related completion
 * signal qualifies, ask an LLM to review the completed work for correctness,
 * completeness, and quality. Mirrors the
 ...
 * task from the OUTSIDE: the original user request, queued mid-task additions,
 * and the assistant's claimed result are provided - NO execution process, tool
 * outputs, reasoning, or intermediate turns. That deliberate isolation keeps the reviewer from being
...
requirement below. You are given the original requirement, any mid-task additions
that extended it, the assistant's claimed result/summary, the list of changed
files, and the working-tree diff.
 ...
 * result text only.
 *
 * additionalInstructions: instructions the user queued while the task ran
 * (mid-task additions). They extend the original requirement, so they get
 * their own section right after it — the reviewer must check them too, never
 * treat them as THE requirement.
 *
 * @param {{requestText?: string, additionalInstructions?: string[], changedFiles?: string[], diffText?: string, answerText?: string}} [input]
 * @returns {string}
 */
export function buildManualCodeReviewContent({ requestText = '', additionalInstructions = [], changedFiles = [], diffText = '', answerText = '' } = {}) {
  const sections = [
  ...
  const additions = (Array.isArray(additionalInstructions) ? additionalInstructions : [])
    .map(s => String(s).trim()).filter(Boolean);
  if (additions.length > 0) {
    sections.push('=== ADDITIONAL USER INSTRUCTIONS (given mid-task, they extend the original requirement) (begin) ===');
    sections.push(additions.map(s => `- ${s}`).join('\n'));
    sections.push('=== ADDITIONAL USER INSTRUCTIONS (end) ===');
    sections.push('');
  }
...
async function callLlm(llm, messages, opts, onDelta, onReasoning) {
  let out = '';
  for await (const evt of llm.stream(messages, opts)) {
    if (evt.type === 'delta') {
      ...
      // Forward the thinking stream (reasoning_content / thinking_delta) so
      // callers can surface the reviewer's reasoning window live, exactly
      // like the main agent loop does. Review phases hardcode
      // enableReasoning: true, so without this the whole thinking phase is
      // silently discarded and the user stares at a spinner.
      if (onReasoning && evt.text) onReasoning(evt.text);
    } else if (evt.type === 'retry') {
      // Transient failure — the client is re-issuing the call from scratch.
      // The partial report text streamed by the failed attempt is void; the
      // retried attempt re-generates the whole two-part reply. Without this
      // reset the verdict parser would see attempt-1's half report glued in
      // front of attempt-2's text.
      out = '';
    }
  }
  return out;
 ...
 * @param {{signal?: AbortSignal, systemPrompt?: string, onDelta?: (text: string) => void, onReasoning?: (text: string) => void}} [opts]
 ...
 *        onReasoning receives every reasoning delta (thinking stream) so
 *        callers can render it live like the main agent loop.
 *        Note: there is deliberately NO timeoutMs option — the review always
 *        waits for the LLM to finish (see the callLlm options below).
  ...
  try {
    ...
    const raw = await callLlm(
      ...
      messages,
      {
        ...
        //
        // Reasoning is ALWAYS on for code review: judging a completed change
        // is a deep reasoning task, and a fast no-thinking pass is exactly the
        // shallow rubber-stamp review this phase exists to prevent.
        enableReasoning: true,
        // NO timeout: the review MUST wait for the LLM to finish naturally.
        // A mid-reply abort loses the verdict JSON (and its reasoning), which
        // degrades to a fake "no issues found". timeoutMs: 0 is the adapters'
        // explicit no-timeout value (see lib/llm/*_adapter.js).
        timeoutMs: 0,
        signal: opts.signal,
      },
      opts.onDelta,
