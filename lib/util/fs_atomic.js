...
export async function writeFileAtomic(absPath, content, { mode = 0o600 } = {}) {
  ...
  try {
    try {
      ...
      // filesystems. The copy itself is non-atomic, but writing to the tmp
      // path and swapping with a rename keeps readers from ever observing a
      // torn file (issue #7 note: the old direct copyFile(tmp, absPath) could
      // expose a half-written target during the race window).
      if (err.code !== 'EXDEV') throw err;
      const tmp2 = `${tmp}.copy`;
      try {
        await fs.copyFile(tmp, tmp2);
        await fs.rename(tmp2, absPath);
      } finally {
