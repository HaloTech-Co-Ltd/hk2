...
/**
 * Process start identity, used to distinguish a live lock owner from PID
 * reuse: two processes with the same pid but different start times are
 * different processes, so a lock held by "this pid, older start" is stale.
 * Linux: /proc/<pid>/stat field 22 (starttime ticks). macOS: `ps -o lstart=`
 * (no /proc there; without this fallback a reused pid kept orphaned locks
 * until the age-based staleMs elapsed). Other platforms: null → the pid +
 * age fallback in isMetadataStale applies.
 */
async function processStartIdentity(pid) {
  ...
  if (process.platform === 'darwin') {
    try {
      // lstart = absolute start time of the process, to the second; stable
      // for the life of the process and different across pid reuse.
