 ...
 *
 * Returns an array of matches: { startOffset, endOffset, startLine, endLine,
 * startCol, endCol, text, meta, anonymousCaptures }. `meta` contains named
 * captures and `anonymousCaptures` contains anonymous wildcards in pattern
 * order. Returns null on unsupported input (empty pattern, regex compile
 * failure).
 */
export function queryPattern(src, pattern) {
  ...
  let reSrc = '';
  const captureNames = [];   // internal group name → user name
  const anonymousGroups = []; // internal group names in pattern order
  let captureIdx = 0;
  for (const tok of tokens) {
    if (tok.kind === 'lit') {
      ...
      captureIdx++;
      const groupName = tok.name ? `m${captureIdx}` : `a${captureIdx}`;
      reSrc += `(?<${groupName}>[\\s\\S]*?)`;
      if (tok.name) {
        captureNames.push([groupName, tok.name]);
      } else {
        anonymousGroups.push(groupName);
      }
