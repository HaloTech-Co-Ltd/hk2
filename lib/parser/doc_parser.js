...
/**
 * Extract cross-reference links from Markdown source.
 * Recognizes inline links `[text](target)`, reference links `[text][ref]` with
 * `[ref]: target` definitions, and autolinks `<http://...>` / bare URLs.
 *
 * Returns [{ text, target, anchor }] — `target` is the raw link destination,
 * `anchor` the #fragment when present. Code blocks/inline code are skipped.
 */
const MD_CODE_FENCE_RE = /^\s*(```|~~~)/;
const MD_LINK_RE = /(?<!\!)\[([^\]\n]*)\]\(([^)\s]+)(?:\s+"[^"]*")?\)/g;
const MD_LINK_DEF_RE = /^\s{0,3}\[([^\]]+)\]:\s+(\S+)(?:\s+"[^"]*")?\s*$/;
const MD_WIKI_LINK_RE = /\[\[([^\]|]+)(?:\|([^\]]+))?\]\]/g;

export function extractMarkdownLinks(src) {
  const links = [];
  const seen = new Set();
  const push = (text, target) => {
    if (!target) return;
    ...
    links.push({ text: (text || '').trim(), target, anchor });
  };

  const lines = src.split('\n');
  const refDefs = new Map();  // ref id (lowercase) -> target
  let inFence = false;

  // Pass 1: collect reference definitions `[ref]: target`
  for (const line of lines) {
    if (MD_CODE_FENCE_RE.test(line)) inFence = !inFence;
    if (inFence) continue;
    const m = MD_LINK_DEF_RE.exec(line);
    if (m) refDefs.set(m[1].trim().toLowerCase(), m[2]);
  }

  // Pass 2: extract links outside code fences / inline code
  inFence = false;
  for (const line of lines) {
    if (MD_CODE_FENCE_RE.test(line)) { inFence = !inFence; continue; }
    if (inFence) continue;
    // Definition lines (`[ref]: target`) are consumed by pass 1 — don't emit
    // them again as shortcut-style usages.
    if (MD_LINK_DEF_RE.test(line)) continue;
    // Strip inline code spans `...` so links inside them are ignored
    const clean = line.replace(/`[^`]*`/g, ' ');

    let m;
    MD_LINK_RE.lastIndex = 0;
    while ((m = MD_LINK_RE.exec(clean)) !== null) {
      push(m[1], m[2]);
    }
    MD_WIKI_LINK_RE.lastIndex = 0;
    while ((m = MD_WIKI_LINK_RE.exec(clean)) !== null) {
      // [[page]] or [[page|label]] — wiki-style internal reference
      push(m[2] || m[1], m[1]);
    }
    // Reference-style usage [text][ref] (or collapsed [ref][] / shortcut [ref])
    const refUse = /\[([^\]\n]+)\](?:\[([^\]\n]*)\])?/g;
    while ((m = refUse.exec(clean)) !== null) {
      const refId = (m[2] || m[1]).trim().toLowerCase();
      const target = refDefs.get(refId);
      if (target) push(m[1], target);
    }
  }
  return links;
}
...
/**
 * Extract GFM pipe tables from Markdown source.
 * Returns [{ title, headers, align, rows }] — `title` is the nearest preceding
 * heading (or ''), `align` per column is 'left' | 'center' | 'right' | null.
 *
 * Single-column tables (`| a |` / `| --- |`) are recognized; a delimiter row
 * whose cell count differs from the header is NOT a GFM table (setext/hr
 * false positives are rejected by that same count check).
 */
const MD_TABLE_SEP_RE = /^\s*\|?\s*:?-{3,}:?\s*(?:\|\s*:?-{3,}:?\s*)*\|?\s*$/;
...
export function extractMarkdownTables(src) {
  const lines = src.split('\n');
  const tables = [];
  let lastHeading = '';
  let inFence = false;
  ...
  while (i < lines.length) {
    const line = lines[i];
    if (MD_CODE_FENCE_RE.test(line)) { inFence = !inFence; i++; continue; }
    if (inFence) { i++; continue; }

    const h = MD_SECTION_RE.exec(line);
    if (h) { lastHeading = h[2].trim(); i++; continue; }

    const isDataRow = line.includes('|') && line.trim().length > 0;
    if (isDataRow && i + 1 < lines.length && MD_TABLE_SEP_RE.test(lines[i + 1])) {
      const headers = splitTableRow(line);
      const sepCells = splitTableRow(lines[i + 1]);
      // GFM: the delimiter row must match the header row in cell count —
      // otherwise it's a setext underline / hr, not a table.
      if (sepCells.length !== headers.length) { i++; continue; }
      ...
      tables.push({ title: lastHeading, headers, align, rows });
      i = j;
      continue;
    }
    i++;
 ...
 * Returns [{ lang, code }] — `lang` is the info string (may be '').
 */
export function extractCodeBlocks(src) {
  const out = [];
  const lines = src.split('\n');
  let i = 0;
  while (i < lines.length) {
    // A fence is 3+ backticks or 3+ tildes; the info string follows the FULL
    // marker run (so ````js` and ~~~~python both parse correctly).
    ...
    if (fence) {
      ...
      while (j < lines.length) {
        if (new RegExp(`^\\s*${marker}`).test(lines[j])) { closed = true; break; }
        body.push(lines[j]);
        j++;
      }
      const code = body.join('\n');
      if (closed && code.trim()) out.push({ lang, code });
      i = closed ? j + 1 : j;
      continue;
    }
    i++;
 ...
 *
 * Deep mode (the default) additionally extracts cross-reference links
 * (`links`), GFM tables (`tables`), and fenced code blocks (`codeBlocks`)
 * so the indexer can build a doc reference graph, structured table index,
 * and doc↔code-symbol associations. Sections/text keep their flat shape for
 * backward compatibility with compactDoc / learn consumers.
 */
...
export function parseHtml(src, fileName) {
  let text = src;
  // Decode entities in heading/title strings (shared with the body paths).
  const decodeEntities = (s) => String(s).replace(/&[a-z#0-9]+;/gi, e => HTML_ENTITY[e] || e);
  ...
  while ((m = headingRe.exec(src)) !== null) {
    if (lastHeading !== null) {
      const body = src.slice(cursor, m.index).replace(HTML_TAG_RE, '').replace(/&[a-z#0-9]+;/g, e => HTML_ENTITY[e] || e).trim();
      sections.push({ title: lastHeading, body, level: lastLevel });
    }
    lastHeading = decodeEntities(m[2].replace(HTML_TAG_RE, '')).trim();
    lastLevel = parseInt(m[1], 10);
    cursor = m.index + m[0].length;
  }
  if (lastHeading !== null) {
    const body = src.slice(cursor).replace(HTML_TAG_RE, '').replace(/&[a-z#0-9]+;/g, e => HTML_ENTITY[e] || e).trim();
    sections.push({ title: lastHeading, body, level: lastLevel });
  }
  ...
  text = src.replace(/<script[\s\S]*?<\/script>/gi, '')
            ...
            .replace(HTML_TAG_RE, ' ')
            .replace(/&[a-z#0-9]+;/g, e => HTML_ENTITY[e] || e)
            .replace(/\s+/g, ' ')
            .trim();
