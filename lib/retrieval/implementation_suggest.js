...
export async function* suggestImplementation(rt, llm, query, opts = {}) {
  ...
  //    opts.rewriteLlm (when provided) is used ONLY for the rewrite phase, so a
  //    per-phase model override applies to retrieval without changing the model
  //    that streams the final answer (still `llm`).
  const rewriteLlm = opts.rewriteLlm || llm;
  const rewrite = opts.skipRewrite
    ? { fallback: true, rewrittenQuery: query, originalQuery: query, intent: '', functionNames: [], keywords: [] }
    : await rewriteQuery(rewriteLlm, query, { signal: opts.signal });
