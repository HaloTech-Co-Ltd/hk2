...
import {
  getMeta, readInverted, readFiles, readCallgraph, readStats,
  readSymbolsShard, listSymbolShards,
  listKnowledge, readKnowledge,
  migrateLegacyPrinciples,
} from '../store/kb_store.js';
import { ensureSupremeCode, SUPREME_CODE_ID, normalizeCodes } from '../store/supreme_code.js';
import { readGraph } from '../store/graph_store.js';
import { readDocIndex } from '../store/doc_index_store.js';
...
export class KBRuntime {
  constructor(name) {
    ...
    this.sourceConfig = null;       // scan roots used to build this KB
    ...
    // --- Doc cross-reference index (tables / links / symbol mentions) ---
    this.docIndex = null;            // { docs, links, referencedBy, tables, symbolMentions, meta }
    this._docTableTokens = null;     // precomputed: tables[i] -> Set<token> (lazy)
    // --- Holy / Eden Spaces ---
  ...
  async load() {
    ...
    const [invObj, files, graphObj, stats, graphData, docIndexData, meta] = await Promise.all([
      readInverted(this.name),
      readFiles(this.name),
      readCallgraph(this.name),
      readStats(this.name),
      readGraph(this.name),
      readDocIndex(this.name),
      getMeta(this.name),
    ]);
    if (!invObj) throw new Error(`KB ${this.name} inverted index missing`);
    this.bm = BM25Index.deserialize(invObj);
    this.files = files;
    this.sourceConfig = meta ? { ...meta, indexedPaths: files.sourcePaths } : null;
    ...
    // Load doc cross-reference index (missing/old KBs → null, all doc APIs no-op)
    this.docIndex = docIndexData && docIndexData.meta ? docIndexData : null;
    this._docTableTokens = null;
    ...
    // Self-heal the permanent supreme-code entry (hk2-supreme-code): projects
    // initialized before this feature lack it. ensureSupremeCode only creates
    // an EMPTY entry when missing — it never overwrites existing code items.
    try {
      const healed = await ensureSupremeCode(this.name, { createdVia: 'runtime-selfheal' });
      if (healed.created) {
        const entry = await readKnowledge(this.name, 'holy', SUPREME_CODE_ID);
        this.knowledgeBySpace.holy.push(entry);
        this.knowledge.set(entry.id, { entry, space: 'holy' });
        this.principles.set(entry.id, entry);
        this._principleList.push(entry);
        log.info(`KB ${this.name} self-healed missing supreme-code entry (${SUPREME_CODE_ID})`);
      }
    } catch (err) {
      log.warn(`KB ${this.name} supreme-code self-heal failed: ${err.message}`);
    }
    // Cached copy for the system-prompt injection (and cheap /kb code reads).
    const scEntry = this.knowledgeBySpace.holy.find(e => e.id === SUPREME_CODE_ID);
    this.supremeCodes = scEntry ? normalizeCodes(scEntry.codes) : [];
  ...
  /** Raw doc index snapshot (or null when the KB predates this feature). */
  getDocIndex() { return this.docIndex; }

  /** Doc-graph record for one document path, or null. */
  getDocRecord(docPath) {
    if (!this.docIndex) return null;
    return this.docIndex.docs[docPath] || null;
  }

  /** Docs that link TO the given document (reverse cross references). */
  getDocReferencedBy(docPath) {
    if (!this.docIndex) return [];
    return (this.docIndex.referencedBy[docPath] || []).slice();
  }

  /** Outgoing doc→doc links from the given document. */
  getDocLinks(docPath) {
    if (!this.docIndex) return [];
    return this.docIndex.links.filter(l => l.from === docPath);
  }

  /** Doc mentions of a code symbol: [{ doc, kind, lang }] across all docs. */
  getSymbolDocRefs(symbolName) {
    if (!this.docIndex) return [];
    return (this.docIndex.symbolMentions[symbolName] || []).map(m => ({ ...m }));
  }

  /**
   * Rank structured tables against a query by token overlap on headers,
   * section titles and row text. Exact column-name hits score highest.
   * @returns {Array<{doc, section, headers, align, rows, score}>}
   */
  findTables(query, limit = 5) {
    if (!this.docIndex || !Array.isArray(this.docIndex.tables)) return [];
    const qTokens = new Set(String(query || '').toLowerCase().split(/[\s\p{P}]+/u).filter(t => t.length >= 2));
    if (qTokens.size === 0) return [];

    if (!this._docTableTokens) {
      this._docTableTokens = this.docIndex.tables.map(t => ({
        headers: new Set(t.headers.flatMap(h => String(h).toLowerCase().split(/\W+/)).filter(Boolean)),
        body: new Set(t.rows.flat().flatMap(c => String(c).toLowerCase().split(/\W+/)).filter(Boolean)),
        section: new Set(String(t.section || '').toLowerCase().split(/\W+/).filter(Boolean)),
      }));
    }

    const scored = [];
    for (let i = 0; i < this.docIndex.tables.length; i++) {
      const t = this.docIndex.tables[i];
      const tk = this._docTableTokens[i];
      let headerHits = 0, otherHits = 0;
      for (const q of qTokens) {
        if (tk.headers.has(q)) headerHits += 2;
        if (tk.body.has(q) || tk.section.has(q)) otherHits += 1;
      }
      const score = headerHits + otherHits;
      if (score > 0) scored.push({ ...t, score, headerHits });
    }
