  ...
  try {
    ...
    const raw = await callLlm(
      ...
      messages,
      {
        temperature: 0.1,
        maxChars: 2048,
        // HK2_ASSESS_REASONING=1 lets the assessor think (deep reasoning)
        // before answering — pragmatic reference resolution ("执行下一步")
        // benefits from it on strong models. Default off: the assessment is a
        // latency-sensitive pre-turn phase. (Inlined flag parse — envFlag
        // lives in src/, importing it from lib/ would invert the layering.)
        enableReasoning: /^(1|yes|true|on)$/i.test(String(process.env.HK2_ASSESS_REASONING ?? '').trim()),
