 ...
 *   1. Longest-prefix rule from setting.json wins (allow OR deny). The
 *      per-project layer ($HK2_HOME/settings/<project-id>/setting.json)
 *      overlays the global $HK2_HOME/setting.json; the project file is
 *      authoritative on ties (same prefix length).
 ...
 *
 * setting.json format (identical for the global file and the per-project
 * managed file):
 *   {
 *     "permissions": [
 ...
 *   }
 *
 * Relative paths inside the per-project setting.json resolve against the
 * project root (NOT the directory containing the file). Every rule applies
 * to its target path AND everything below it (directory semantics); a
 * trailing "/**" is accepted but has no additional effect.
 *
 ...
 * prefix.
 *
 * SECURITY (privilege-escalation fix): BOTH layers live under HK2_HOME —
 * by default outside the workspace, though HK2_HOME itself can be pointed
 * anywhere. The structural guarantee is therefore NOT the location: it is
 * that checkPermission hard-denies agent writes into the HK2_HOME settings
 * area (below), so the agent cannot write the file that defines its own
 * sandbox regardless of where HK2_HOME sits. A setting.json inside the
 * project root is NOT loaded (it only produces a load-time migration
 * hint): the project root is agent-writable by default, so trusting a
 * config there would let a model neutralize any global deny with an
 * equal-prefix project allow. checkPermission additionally hard-denies
 * writes into the HK2_HOME settings area even when an allow rule covers
 * it (defense in depth).
 */
import os from 'node:os';
import path from 'node:path';
import fs from 'node:fs/promises';
import { readJsonSafe, exists } from '../util/fs_atomic.js';
...
/**
 * Resolve the project id used for the managed per-project settings dir
 * ($HK2_HOME/settings/<project-id>/setting.json). Sources, in order:
 *   1. HK2_PROJECT_ID env — set by the interactive session whenever it pins
 *      HK2_PROJECT_SOURCE (authoritative, no IO);
 *   2. reverse lookup of the project root in $HK2_HOME/projects.json
 *      (covers the non-interactive path, which only has cwd).
 * Returns null when the project cannot be identified → no project layer
 * (deny-by-default stays intact; only the global baseline applies).
 */
export async function getProjectId(projectRoot = null) {
  const envId = process.env.HK2_PROJECT_ID;
  // The id becomes a path segment — anything that is not a single safe
  // segment (empty, separators, leading dots, ...) is rejected so the
  // managed settings path can never escape $HK2_HOME/settings/.
  if (envId && /^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(envId)) return envId;
  const root = path.resolve(projectRoot || getProjectRoot());
  try {
    const raw = await readJsonSafe(path.join(hk2Home(), 'projects.json'), null);
    const projects = raw && typeof raw === 'object' && raw.projects && typeof raw.projects === 'object'
      ? raw.projects : {};
    for (const p of Object.values(projects)) {
      if (p && typeof p === 'object' && typeof p.sourcePath === 'string'
        && typeof p.id === 'string' && p.id
        && path.resolve(p.sourcePath) === root) {
        return p.id;
      }
    }
  } catch { /* registry unreadable → no project layer (deny-by-default) */ }
  return null;
}

/**
 * Paths whose WRITE would let the agent forge its own permission rules:
 * the global setting.json and everything under the managed settings/ dir.
 * Hard-denied in checkPermission regardless of any allow rule — the sandbox
 * config must only ever be edited by the human user.
 */
function isPermissionConfigPath(abs) {
  const home = hk2Home();
  if (abs === path.join(home, 'setting.json')) return true;
  const managed = path.join(home, 'settings');
  if (abs === managed || abs.startsWith(managed + path.sep)) return true;
  return false;
}
...
export async function loadPermissionRules({ projectRoot } = {}) {
  ...
  const pid = await getProjectId(root);

  // SECURITY (privilege-escalation fix): a setting.json in the project root
  // is deliberately NOT loaded — the agent has default write access to the
  // project root, so rules placed there could be self-granted (any global
  // deny neutralized by an equal-prefix project allow). Surface a migration
  // hint instead of silently ignoring the file.
  const legacy = path.join(root, 'setting.json');
  if (await exists(legacy)) {
    const managed = pid
      ? path.join(hk2Home(), 'settings', pid, 'setting.json')
      : `${path.join(hk2Home(), 'settings', '<project-id>', 'setting.json')} (project not registered — register it via /project init)`;
    errors.push(
      `setting.json (ignored): ${legacy} — a project-root setting.json is no longer loaded, because the agent can write the project root and must not be trusted as a permission source. ` +
      `Move these rules to ${managed}, or merge them into the global ${path.join(hk2Home(), 'setting.json')}.`
    );
  }
  ...
  if (pid) {
    files.push({ fp: path.join(hk2Home(), 'settings', pid, 'setting.json'), baseDir: root, layer: 'project' });
  }
 ...
 * on-disk absolute-path candidates. With KB metadata, resolve its scan roots
 * authoritatively (including sourceRoot and named extraRoots). Without metadata,
 * retain the legacy project/cwd lookup. An absolute
 ...
 * actually be read.
 */
export function kbPathCandidates(p, source = null) {
  if (typeof p !== 'string' || !p) return [];
  if (path.isAbsolute(p)) return [path.resolve(p)];
  // Indexed paths are relative to the scan root, not necessarily the project.
  // A known KB root is authoritative even when its file has since disappeared.
  if (source?.sourcePath) {
    if (source.indexedPaths && Object.hasOwn(source.indexedPaths, p)) {
      const actual = source.indexedPaths[p];
      return typeof actual === 'string' && path.isAbsolute(actual) ? [actual] : [];
    }
    const extras = source.extraRoots || [];
    for (const root of extras) {
      const prefix = root.name ? `${root.name}/` : '';
      // Old indexes did not record provenance: either root may have owned
      // this name (even if its file is now deleted). Fail closed until an
      // update records the source, rather than authorize a different file.
      if (!prefix || p.startsWith(prefix)) return [];
    }
    return [path.resolve(path.join(source.sourcePath, source.sourceRoot || ''), p)];
  }
 ...
 */
export async function resolveKbContentPath(p, source = null) {
  const candidates = kbPathCandidates(p, source);
...
export async function resolveKbContentPaths(paths, source = null) {
  const uniq = [...new Set((paths || []).filter(p => typeof p === 'string' && p))];
  const out = new Map();
  await Promise.all(uniq.map(async (p) => {
    out.set(p, await resolveKbContentPath(p, source));
  }));
  return out;
}
...
export function checkPermission(rules, target, mode, { projectRoot, canonicalRoots } = {}) {
  ...
  // Defense in depth: the permission config itself is READ-ONLY for the
  // agent. Even an allow rule covering HK2_HOME must not extend to the
  // rules store — otherwise a model could rewrite its own sandbox.
  if (mode === 'w' && isPermissionConfigPath(abs)) {
    return { ok: false, reason: `permission config is agent read-only: ${abs} (edit it manually — the sandbox may not rewrite its own rules)` };
  }
...
export class PermissionService {
  ...
  async checkReal(target, mode) {
    ...
    try {
      ...
      if (err.code !== 'ENOENT') {
        return { ok: false, reason: `cannot resolve permission target: ${abs}: ${err.message}` };
      }
      // Preserve every missing path component when resolving an ancestor.
      // Checking just that ancestor loses file/subtree rules on new targets.
      let probe = abs;
      let suffix = [];
      for (let hops = 0; hops < 128; hops++) {
        try {
          real = path.join(await fs.realpath(probe), ...suffix);
          break;
        } catch (resolveError) {
          if (resolveError.code !== 'ENOENT') break;
        }
        try {
          // realpath also fails for a dangling symlink. Follow its target
          // before walking upwards, otherwise a new external file could be
          // authorized using the link's in-project parent.
          if ((await fs.lstat(probe)).isSymbolicLink()) {
            probe = path.resolve(path.dirname(probe), await fs.readlink(probe), ...suffix);
            suffix = [];
            continue;
          }
        } catch (statError) {
          if (statError.code !== 'ENOENT') break;
        }
      ...
      if (!real) return { ok: false, reason: `cannot resolve permission target: ${abs}` };
    }
