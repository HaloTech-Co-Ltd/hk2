...
export async function resolveModelRef(ref) {
  ...
  return {
    ...
    modelType: typeof m.modelType === 'string' ? m.modelType : DEFAULT_MODEL_TYPE,
    // Model-specific feature options (--model-options), stored as a JSON
    // object with a free-form, model-specific key set. Defaults to an empty
    // object (no options) for legacy records that predate the field.
    modelOptions: (m.modelOptions && typeof m.modelOptions === 'object' && !Array.isArray(m.modelOptions))
      ? { ...m.modelOptions }
