...
import { llmApiTimeoutMs } from '../llm/timeout.js';
...
export const DEFAULT_INCLUDE_GLOBS = [
  ...
  '**/*.y', '**/*.l',
  // Documents (Markdown, plain text, JSON, YAML, HTML, SGML, PDF, Word, PPT)
  '**/*.md', '**/*.markdown', '**/*.txt', '**/*.rst', '**/*.adoc',
  '**/README*', '**/LICENSE*', '**/CHANGELOG*', '**/CONTRIBUTING*',
  '**/*.json', '**/*.yaml', '**/*.yml', '**/*.html', '**/*.htm',
  '**/*.sgml', '**/*.pdf', '**/*.doc', '**/*.docx', '**/*.ppt', '**/*.pptx',
];
...
export async function ensureHome() {
  // HK2_HOME holds models.json with API keys: owner-only, always — mkdir's
  // mode only applies at creation and is masked by umask, so chmod after.
  await fs.mkdir(HK2_HOME, { recursive: true, mode: 0o700 });
  await fs.chmod(HK2_HOME, 0o700).catch(() => {});
  ...
  // Migration: tighten files written by older versions under a permissive
  // umask (0644 let every local user read the API keys inside).
  await fs.chmod(MODELS_PATH, 0o600).catch(() => {});
  await fs.chmod(PROJECTS_PATH, 0o600).catch(() => {});
}
 ...
 *
 * glm-5.3 / glm-5.3-flash (BigModel): deep-reasoning model. Reasoning is on
 * by default and the effort level is selectable via the `reasoning_effort`
 * option:
 ...
 * glm-5.3-flash declares the same feature set as glm-5.3, PLUS multimodal
 * input (images / video / audio via content blocks — see `multimodal`
 * below and lib/agent/attachments.js).
 * deepseek-flash declares multimodal input as well, with no extra options.
 *
 * `multimodal: true` declares the model type CAN accept multimodal content
 * blocks (messages[].content[] with type image_url / video_url /
 * input_audio). It gates the per-model `--multimodal=on` flag: enabling
 * multimodal input on a type without this declaration is rejected by
 * /model add|set (see validateMultimodalForType). Wire mapping of the
 * content blocks themselves lives in the protocol adapters.
 *
 * `defaultBuiltinTools: true` declares the model type gets the official
 * server-side built-in tools (Anthropic ToolUnion: web_search / web_fetch /
 * code_execution / ...) declared BY DEFAULT — every current type does.
 * Built-in tool types documented by the Messages-API reference must work
 * out of the box, not require per-model configuration; modelOptions.tools
 * (see lib/llm/anthropic_adapter.js) stays the escape hatch for variants
 * hk2 does not yet know (forwarded verbatim; a same-family entry replaces
 * the default declaration). The per-model `builtinTools` flag (default on,
 * --built-in-tools=off) opts an entry out.
 */
export const MODEL_TYPE_FEATURES = {
  generic: {
    defaultBuiltinTools: true,
  },
  'glm-5.3': {
...
/**
 * Whether a model type CAN accept multimodal content blocks (image /
 * video / audio). This is a capability DECLARATION of the type; the
 * per-model `multimodal` flag (set via /model add|set --multimodal=on,
 * default off) actually enables it on an entry. Currently glm-5.3-flash
 * and deepseek-flash declare it.
 */
export function modelTypeMultimodal(type) {
  return modelTypeFeatures(type)?.multimodal === true;
}

/**
 * Whether a model type declares the official server-side built-in tools
 * (Anthropic ToolUnion) as declared-by-default. All current types do; the
 * per-model `builtinTools` flag (default on, --built-in-tools=off) opts an
 * entry out. Only the anthropic-dialect adapter acts on this.
 */
export function modelTypeDefaultBuiltinTools(type) {
  return modelTypeFeatures(type)?.defaultBuiltinTools === true;
}

/**
 * Parse a user-supplied --built-in-tools value. Accepts on/off
 * (case-insensitive, plus 1/0/true/false/yes/no) and returns the boolean,
 * or null when the value is not recognizable so the CLI can surface a
 * clear error.
 */
export function parseBuiltinToolsFlag(v) {
...
/**
 * Parse a user-supplied --multimodal value. Accepts on/off (case-insensitive,
 * plus 1/0/true/false/yes/no) and returns the boolean, or null when the
 * value is not recognizable so the CLI can surface a clear error.
 */
export function parseMultimodalFlag(v) {
  if (v === true) return true;
  if (v === false) return false;
  if (typeof v !== 'string') return null;
  const s = v.trim().toLowerCase();
...
/**
 * Validate a --multimodal=on request against the model type's capability
 * declaration. `off` is always valid (any model can decline input it will
 * never receive). Turning multimodal ON for a type that does not declare
 * the capability returns a human-readable error naming the capable types;
 * null means the request is valid.
 */
export function validateMultimodalForType(type, wantMultimodal) {
  if (wantMultimodal !== true) return null;
  if (modelTypeMultimodal(type)) return null;
  const capable = Object.entries(MODEL_TYPE_FEATURES)
    .filter(([, f]) => f?.multimodal === true)
    .map(([t]) => t);
  return `--multimodal=on requires a multimodal-capable model type (capable: ${
    capable.join(', ') || '(none)'
  }); ${type || DEFAULT_MODEL_TYPE} does not support image/video/audio input`;
}
...
/**
 * Transactional read-modify-write for models.json (issue #7).
 *
 * All writers used to do loadModels() → mutate the in-memory snapshot →
 * saveModels(snapshot). With no mutual exclusion, two concurrent hk2
 * processes silently dropped whichever write landed first (writeFileAtomic
 * prevents torn READS, not write-write races). withModels holds a
 * cross-process lockfile around load → fn → save, so the mutation runs on
 * the freshest state and the write is atomic with respect to other
 * withModels/withProjects callers.
 *
 * @param {(data: {providers: Object, default: string|null}) => any|Promise<any>} fn
 *   Receives the freshly-loaded snapshot; MUTATE IT IN PLACE. The return
 *   value (if any) is passed through to the caller but is NOT what gets
 *   saved — the (mutated) snapshot is always persisted. Returning a
 *   replacement object is NOT supported for models.json (unlike a generic
 *   RMW helper): callers like /model set need to return the touched entry
 *   for their own printing, and treating that as the file content would
 *   clobber the registry.
 */
export async function withModels(fn) {
  await ensureHome();
  return withLock(MODELS_PATH, async () => {
    const data = await loadModels();
    const out = await fn(data);
    await writeJsonAtomic(MODELS_PATH, data);
    return out !== undefined ? out : data;
  });
}
...
/**
 * Per-project default model ref (set via `/model set-default current <ref>`).
 * Returns null when the project has no override — callers then fall back to
 * the global default in models.json.
 *
 ...
 */
export function getProjectDefaultModelRef(project) {
  if (!project || typeof project !== 'object') return null;
  const ref = project.defaultModel;
  return typeof ref === 'string' && ref ? ref : null;
}

/**
 * Resolve the effective default model ref: the given project's defaultModel
 * override when set (and resolvable), otherwise the global models.json
 * default. Returns the resolved LLM config object, or null when nothing is
 * configured / resolvable.
 *
 * `project` semantics (v1.1.48+):
 *   - undefined        → resolve the project from the GLOBAL `current` pointer
 *                        (legacy / non-interactive callers; test helpers).
 *   - null             → NO project: skip the override entirely and fall
 *                        straight through to the global default. Used by the
 *                        interactive session (reloadAll) for a bare launch
 *                        with no project, so a global current owned by
 *                        ANOTHER process never leaks its override in here.
 *   - a project record → use THIS project's override (e.g. the session-pinned
 *                        project from `--project=<name>`), never the global
 *                        pointer. A record without defaultModel just falls
 *                        through to the global default.
 *
 * This is the single fallback point for every default-model consumer
 * (reloadAll, explain, search, serve), so project overrides apply uniformly.
 */
export async function resolveDefaultModel(project = undefined) {
  // Per-project override first: only used when it actually resolves (a stale
  // ref — e.g. the model was deleted or renamed — silently falls back to the
  // global default instead of leaving the session without a model).
  try {
    const p = project !== undefined ? project : await getCurrentProject();
    const ref = getProjectDefaultModelRef(p);
    if (ref) {
...
export async function resolveModelRef(ref) {
  ...
  return {
    ...
    // Explicit per-model --max-tokens, honored by LLMClient → adapters as
    // maxOutputTokens (issue #5: this used to be write-only config — shown by
    // /model list|show but never read, so the wire max_tokens was always
    // derived from contextWindow/4). Adapters fall back to that estimate
    // when unset.
    maxTokens: m.maxTokens || undefined,
    // UNSET temperature stays undefined (resolved per adapter: OpenAI falls
    // back to 0.2, Anthropic OMITS the field — the parameter is deprecated
    // and models after Claude Opus 4.6 reject any non-1.0 value). The
    // resolved display default below is presentation-only.
    ...
    // Multimodal input capability for THIS entry: on only when the entry's
    // `multimodal` flag is true AND the model type declares the capability
    // (hand-edited records that flip the flag on a non-capable type resolve
    // to false here; the CLI rejects that combination up front).
    multimodal: m.multimodal === true && modelTypeMultimodal(modelType) === true,
    // Server-side built-in tools (anthropic dialect) declared BY DEFAULT:
    // official-documented ToolUnion variants (web_search / web_fetch / ...)
    // are advertised in every /v1/messages request body without per-model
    // configuration. Default on for every type that declares the capability
    // (all current types); an explicit --built-in-tools=off opts this entry
    // out. modelOptions.tools remains the escape hatch for variants hk2 does
    // not yet know — see ANTHROPIC_DEFAULT_BUILTIN_TOOLS in the adapter.
    builtinTools: m.builtinTools === false ? false : modelTypeDefaultBuiltinTools(modelType) === true,
    // Stamped from the shared env-aware default (HK2_LLMAPI_TIMEOUT_MS,
    // fallback 3600000) so the per-call chain opts.timeoutMs > config.timeout
    // > env default always has a concrete last link. Note: this stamp is why
    // the adapters' `timeoutMs || <default>` fallback only fires for callers
    // that bypass LLMClient (e.g. direct completeOpenAI calls in tests).
    timeout: llmApiTimeoutMs(),
  };
}
...
/*
 * Remove MCP server(s) from a model entry (the counterpart of
 * /model add-mcpserver). Runs inside the models.json lock (issue #7):
 * load → remove → save is atomic, and the mutation only ever touches the
 * addressed model's `mcpServers` array.
 *
 * @param {string} ref "provider/model-id" of the target model.
 * @param {{name?: string, all?: boolean}} opts `name` removes the single
 *   server with that exact name; `all: true` (or name === 'all') removes
 *   every server on the model.
 * @returns {Promise<{removed: Array<{type: string, name: string, options: object}>}|{error: string}>}
 *   `removed` lists what was taken out (empty array when the model had no
 *   servers / nothing matched — still a success, nothing persisted unless
 *   a removal actually happened). Errors: unknown provider/model, or a
 *   missing/ambiguous removal selector. Failed calls leave models.json
 *   byte-identical.
 */
export async function removeModelMcpServer(ref, { name, all = false } = {}) {
  const split = splitModelRef(ref);
  if (!split) return { error: `invalid model ref: ${ref} (expected provider/model-id)` };
  const wantAll = all === true || name === 'all';
  if (!wantAll && (typeof name !== 'string' || !name.trim())) {
    return { error: `missing MCP server name (use --name=<NAME>, or --all to remove every server)` };
  }
  const target = wantAll ? null : name.trim();
  const removed = await withModels((data) => {
    const prov = data.providers[split.provider];
    const entry = (prov && Array.isArray(prov.models)) ? prov.models.find(m => m.id === split.model) : null;
    if (!entry) return null; // caller maps this to "model not found"
    const servers = Array.isArray(entry.mcpServers) ? entry.mcpServers : [];
    const out = wantAll ? servers.slice() : servers.filter(s => s && s.name === target);
    if (out.length === servers.length) {
      // Removing everything: drop the key so the stored entry goes back to
      // the pre-add shape instead of lingering as `"mcpServers": []`.
      delete entry.mcpServers;
    } else {
      entry.mcpServers = servers.filter(s => !out.includes(s));
    }
    return out;
  });
  if (removed === null) {
    return { error: `Model not found: ${ref}` };
  }
  return { removed };
}

/* ------------------------------------------------------------------ */
/* tools.json — multimodal tool settings (/tool)                      */
/* ------------------------------------------------------------------ */

/**
 * Settings for the multimodal vision tool suite (see lib/agent/vision_tools.js).
 *
 *   ~/.hk2/tools.json:
 *     {
 *       "visionModelRef": "bigmodel/glm-5.3-flash",   // dedicated vision model (suite default)
 *       "toolModels": {                                 // OPTIONAL per-tool overrides
 *         "ui_to_artifact": "bigmodel/glm-5.3-flash",
 *         "video_analysis": "other/qwen-vl"
 *       },
 *       "disabled": ["video_analysis"]                 // tools disabled via /tool disable
 *     }
 *
 * Kept in its OWN file (not models.json): loadModels() normalizes to
 * { providers, default } and withModels() persists exactly that snapshot,
 * so any sibling keys on models.json would be silently dropped. A dedicated
 * file with the same withLock discipline keeps writes race-free.
 *
 * visionModelRef must point at a model entry that resolves multimodal:true
 * (entry flag --multimodal=on AND a multimodal-capable model type — enforced
 * by /tool set-model, re-validated at every read). When unset, the vision
 * tools fall back to the session's active model when it is multimodal; with
 * neither available the tools are not registered at all.
 *
 * toolModels maps an individual vision tool name to its OWN model ref. The
 * per-tool ref wins over visionModelRef for that tool only (resolution:
 * toolModels[name] → visionModelRef → multimodal session model → not
 * registered). Each ref is re-validated at every read with the same
 * multimodal:true requirement; stale refs fall through silently to the next
 * tier. Configured via /tool set-model <tool> <provider>/<model-id>.
 */
export const TOOLS_PATH = path.join(HK2_HOME, 'tools.json');

/** Default tools.json content (all keys optional). */
function defaultTools() {
  return { visionModelRef: null, toolModels: {}, disabled: [] };
}

/** Read tools.json (tolerant of missing/legacy files). */
export async function loadToolSettings() {
  await ensureHome();
  const data = await readJsonSafe(TOOLS_PATH, null);
  if (!data || typeof data !== 'object') return defaultTools();
  const toolModels = {};
  if (data.toolModels && typeof data.toolModels === 'object' && !Array.isArray(data.toolModels)) {
    for (const [k, v] of Object.entries(data.toolModels)) {
      if (typeof v === 'string' && v) toolModels[k] = v;
    }
  }
  return {
    visionModelRef: (typeof data.visionModelRef === 'string' && data.visionModelRef) || null,
    toolModels,
    disabled: Array.isArray(data.disabled)
      ? data.disabled.filter((t) => typeof t === 'string' && t)
      : [],
  };
}

/** Transactional read-modify-write for tools.json (same discipline as withModels). */
export async function withToolSettings(fn) {
  await ensureHome();
  return withLock(TOOLS_PATH, async () => {
    const data = await loadToolSettings();
    const out = await fn(data);
    await writeJsonAtomic(TOOLS_PATH, data);
    return out !== undefined ? out : data;
  });
}

/**
 * Resolve the configured vision-model ref to a concrete model config.
 * Returns null when unset OR when the ref no longer resolves to a
 * multimodal-capable entry (stale ref, capability removed, model deleted) —
 * callers fall back to the session model. Resolution failures never throw.
 */
export async function resolveVisionModel() {
  let ref;
  try {
    ({ visionModelRef: ref } = await loadToolSettings());
  } catch { return null; }
  if (!ref) return null;
  try {
    const cfg = await resolveModelRef(ref);
    return (cfg && cfg.multimodal === true) ? cfg : null;
  } catch { return null; }
}

/** Whether a vision tool name is disabled (false for unknown names). */
export async function isToolDisabled(name) {
  try {
    const { disabled } = await loadToolSettings();
    return disabled.includes(name);
  } catch { return false; }
}

/**
 * Set the vision model ref. `ref` must resolve to an existing model entry
 * whose effective multimodal capability is on; callers pre-validate and pass
 * the resolved config so the write is the last step.
 */
export async function setToolVisionModelRef(ref, cfg) {
  if (!ref || typeof ref !== 'string') return { error: 'invalid ref' };
  if (!cfg || cfg.multimodal !== true) {
    return { error: 'model does not accept multimodal input' };
  }
  return withToolSettings((data) => {
    data.visionModelRef = ref;
    return { ok: true };
  });
}

/** Remove the vision model override (tools fall back to the session model). */
export async function clearToolVisionModelRef() {
  return withToolSettings((data) => {
    const had = data.visionModelRef != null;
    data.visionModelRef = null;
    return { ok: true, had };
  });
}

/**
 * Set the PER-TOOL model ref (tools.json `toolModels[name]`). `name` must be
 * a known vision tool; `ref` must resolve to an existing model entry whose
 * effective multimodal capability is on — callers pre-validate and pass the
 * resolved config so the write is the last step (same discipline as
 * setToolVisionModelRef).
 */
export async function setToolModelRef(name, ref, cfg) {
  if (!name || typeof name !== 'string') return { error: 'missing tool name' };
  if (!ref || typeof ref !== 'string') return { error: 'invalid ref' };
  if (!cfg || cfg.multimodal !== true) {
    return { error: 'model does not accept multimodal input' };
  }
  return withToolSettings((data) => {
    data.toolModels = { ...(data.toolModels || {}), [name]: ref };
    return { ok: true };
  });
}

/**
 * Remove one tool's per-tool model override (falls back to the suite default,
 * then the session model). Pass no name to clear EVERY per-tool override.
 */
export async function clearToolModelRef(name) {
  return withToolSettings((data) => {
    if (!name) {
      const had = Object.keys(data.toolModels || {}).length;
      data.toolModels = {};
      return { ok: true, had };
    }
    const had = data.toolModels ? data.toolModels[name] != null : false;
    if (had) {
      const next = { ...(data.toolModels || {}) };
      delete next[name];
      data.toolModels = next;
    }
    return { ok: true, had };
  });
}

/** Resolve every configured per-tool ref to a concrete multimodal config.
 * Stale/unresolvable/non-multimodal entries are silently dropped — callers
 * fall through to the next resolution tier for that tool. Never throws.
 * Returns { [toolName]: { cfg, source: 'tools' } } to match the runtime
 * shape consumed by effectiveVisionFor. */
export async function resolveToolModels() {
  let settings;
  try {
    ({ toolModels: settings } = await loadToolSettings());
  } catch { return {}; }
  const out = {};
  for (const [name, ref] of Object.entries(settings || {})) {
    try {
      const cfg = await resolveModelRef(ref);
      if (cfg && cfg.multimodal === true) out[name] = { cfg, source: 'tools' };
    } catch { /* stale — drop */ }
  }
  return out;
}

/** Enable or disable a vision tool by name; unknown names are rejected. */
export async function setToolEnabled(name, enabled, knownNames = []) {
  if (!name || typeof name !== 'string') return { error: 'missing tool name' };
  if (Array.isArray(knownNames) && knownNames.length > 0 && !knownNames.includes(name)) {
...
/** Re-enable every disabled vision tool. */
export async function resetToolDisabled() {
  return withToolSettings((data) => {
    const had = data.disabled.length;
    data.disabled = [];
    return { ok: true, had };
  });
}
...
/**
 * Transactional read-modify-write for projects.json — same contract as
 * withModels (issue #7): fn mutates the loaded snapshot in place; its
 * return value is passed through to the caller, the snapshot is what gets
 * persisted. registerProject / updateProject / removeProject /
 * setCurrentProject and the phase/default-model ref setters all go through
 * this, so concurrent project registrations or /model set-phase calls from
 * multiple hk2 processes can no longer lose each other's writes.
 */
export async function withProjects(fn) {
  await ensureHome();
  return withLock(PROJECTS_PATH, async () => {
...
export async function registerProject(opts) {
  ...
  // withProjects: locked load→mutate→save so a concurrent registration in
  // another hk2 process can't be clobbered by our full-file write (issue #7).
  await withProjects((data) => {
...
export async function updateProject(id, patch) {
  return withProjects(async (data) => {
...
/**
 * Persist the per-project default model ref (written by
 * `/model set-default current <ref>`). Returns the updated project record,
 * or null when the project is unknown or `ref` is empty (use
 * clearProjectDefaultModelRef to remove the override).
 */
export async function setProjectDefaultModelRef(projectId, ref) {
  if (!ref || typeof ref !== 'string') return null;
  return updateProject(projectId, { defaultModel: ref });
}

/**
 * Remove the per-project default model override (the project then falls back
 * to the global models.json default). Returns the updated project record (or
 * null); no-op when nothing was configured.
 */
export async function clearProjectDefaultModelRef(projectId) {
  return withProjects(async (data) => {
    const cur = data.projects[projectId];
    if (!cur) return null;
    if (typeof cur.defaultModel !== 'string' || !cur.defaultModel) return cur;
    delete cur.defaultModel;
    cur.updatedAt = new Date().toISOString();
    return cur;
  });
}
