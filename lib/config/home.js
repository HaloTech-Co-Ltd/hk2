...
/**
 * Strip a trailing bracketed context-window hint from a model id, e.g.
 * 'glm-5.2[1m]' -> 'glm-5.2'. The bracketed suffix is a provider-specific
 * context-window selector (observed on Volcengine ark: `[1m]` for the
 * 1-million-token variant) that some Anthropic-compatible gateways ACCEPT
 * (ark) but others REJECT (BigModel open.bigmodel.cn/api/anthropic returns
 * `modelCode不存在` when the whole `glm-5.2[1m]` is treated as the model
 * code). Stripping it for the WIRE request keeps the stored id / display ref
 * intact (so two providers hosting the same base model stay visually distinct
 * and the active context length stays visible) while sending only the bare
 * model code to the API. No-op when there is no trailing bracketed suffix.
 */
export function stripContextHint(modelId) {
  if (typeof modelId !== 'string' || !modelId) return modelId;
  return modelId.replace(/\s*\[[^\]]*\]\s*$/, '');
}
...
export async function resolveModelRef(ref) {
  ...
  return {
    ...
    // Strip the trailing [1m] / [ctx] context-window hint for the WIRE model
    // code: the stored m.id and the display ref keep it (so the provider is
    // distinguishable and the context length stays visible), but several
    // Anthropic-compatible gateways reject `model-<id>[1m]` as an unknown
    // modelCode. See stripContextHint for the full rationale.
    model: stripContextHint(m.id),
