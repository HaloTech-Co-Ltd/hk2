...
// safe to retry. (The adapters surface err.cause.code in the message.)
const ESTABLISH_CODES = new Set(['ECONNREFUSED', 'ENOTFOUND', 'EAI_AGAIN', 'UND_ERR_CONNECT_TIMEOUT']);

/**
 * HK2_LLM_RETRY_UNKNOWN_POST=1 opts into retrying unknown-outcome failures
 * (mid-flight transport errors and 5xx). Default off: a duplicate request
 * and duplicate billing are worse than a failed turn. Providers here
 * expose no idempotency key, so classification is the only guard.
 */
...
export function classifyLlmError(err) {
  ...
  if (transport) {
    for (const c of ESTABLISH_CODES) {
      if (msg.includes(`(${c})`)) return 'safe';
    }
    return 'unknown';
  }
