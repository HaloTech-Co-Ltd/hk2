 ...
 *     times (default 10) with exponential backoff; unknown-outcome classes
 *     (5xx / mid-flight transport) are retried by default — opt out with
 *     HK2_LLM_RETRY_UNKNOWN_POST=0. A `{ type: 'retry' }`
...
export class LLMClient {
  ...
  _buildArgs(messages, opts = {}) {
    const args = {
      ...
      // UNSET temperature must stay undefined down to the adapter: the
      // OpenAI adapter falls back to 0.2 itself, while the Anthropic adapter
      // OMITS the field (official default 1.0; the parameter is deprecated —
      // models after Claude Opus 4.6 reject any value other than 1.0 with a
      // 400, so baking 0.2 in here would break every official-Anthropic
      // request with reasoning off). An explicit --temperature (per-call opts
      // or per-model config) still wins and is forwarded as-is.
      temperature: opts.temperature ?? this.config.temperature ?? undefined,
      ...
      // Server-side built-in tools declared by default (anthropic dialect);
      // resolved from the entry flag + type capability in resolveModelRef.
      builtinTools: opts.builtinTools ?? this.config.builtinTools ?? undefined,
  ...
  _applyDialectSpecifics(args, style) {
    // The OpenAI chat-completions dialect has no server-side built-in tools;
    // the `builtinTools` flag is anthropic-dialect-only and must never leak
    // into a chat-completions request body.
    if (style !== 'anthropic') delete args.builtinTools;
    return args;
  }
  ...
  async* stream(messages, opts = {}) {
    ...
    this._applyDialectSpecifics(args, style);
  ...
  async complete(messages, opts = {}) {
    ...
    this._applyDialectSpecifics(args, 'openai');
