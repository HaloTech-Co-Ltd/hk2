...
export async function* streamOpenAI({ baseUrl, apiKey, model, messages, maxChars, temperature, enableReasoning, modelType, modelOptions, timeoutMs, signal, tools, headers, onRetry }) {
...
export async function completeOpenAI({ baseUrl, apiKey, model, messages, maxChars, temperature, modelType, modelOptions, enableReasoning, timeoutMs, headers, onRetry }) {
  const url = `${baseUrl.replace(/\/$/, '')}/v1/chat/completions`;
  const body = {
    model,
    messages,
