 ...
 * key is left alone.
 *
 * glm-5.3 / glm-5.3-flash (BigModel): deep-reasoning model —
 ...
 *
 * glm-5.3-flash additionally accepts multimodal input (MODEL_TYPE_FEATURES
 * multimodal: true — the per-model --multimodal=on flag). Multimodal
 * messages[].content arrays (text + image_url / video_url / input_audio
 * blocks, built by lib/agent/attachments.js) pass through UNTOUCHED on this
 * endpoint — the BigModel chat-completions API accepts them natively.
 */
const GLM53_TYPES = new Set(['glm-5.3', 'glm-5.3-flash']);

/**
 * Full request URL for an OpenAI-style chat-completions call.
 *
 * baseUrl ending in `/chat/completions` is used AS-IS (full-endpoint form):
 * gateways whose OpenAI-compatible path is not rooted at `/v1` (BigModel:
 * /api/paas/v4/chat/completions) can then be configured directly, e.g.
 *   /model add bigmodel glm-5.3-flash --api=openai \
 *     --base-url=https://open.bigmodel.cn/api/paas/v4/chat/completions
 * Any other baseUrl keeps the historical convention of appending
 * `/v1/chat/completions`.
 */
...
/**
 * Dialect-boundary guard: modelOptions.tools declares Anthropic server-side
 * built-in tools (ToolUnion entries like web_search / web_fetch). The OpenAI
 * chat-completions protocol has no such concept — and neither do
 * OpenAI-compatible endpoints (DeepSeek, BigModel, vLLM, ...). Silently
 * dropping the key made users believe server-side web access was enabled
 * while the model truthfully refused real-time fetches; fail loud instead.
 * Shared by streamOpenAI and completeOpenAI.
 */
function assertNoBuiltinToolDeclarations(modelOptions) {
  if (!modelOptions || !Array.isArray(modelOptions.tools) || modelOptions.tools.length === 0) return;
  throw new Error(
    "modelOptions.tools declares server-side built-in tools, which only the " +
    "Anthropic Messages API supports (--api=anthropic); this provider uses " +
    "the OpenAI-compatible chat-completions dialect and has no server-side " +
    "web_search/web_fetch. Remove --model-options='{\"tools\":[...]}' or switch " +
    "the model to an Anthropic-style endpoint."
  );
}

export async function* streamOpenAI({ baseUrl, apiKey, model, messages, maxChars, maxOutputTokens, temperature, enableReasoning, modelType, modelOptions, timeoutMs, signal, tools, headers }) {
  ...
  // Explicit --max-tokens wins; derive from maxChars only when unset (issue #5).
  if (maxOutputTokens) body.max_tokens = maxOutputTokens;
  else if (maxChars) body.max_tokens = Math.min(32768, Math.max(256, Math.floor(maxChars / 4)));
  if (enableReasoning === false) {
    body.chat_template_kwargs = { enable_thinking: false };
  }
  applyModelTypeFeatures(body, { modelType, modelOptions, enableReasoning });
  assertNoBuiltinToolDeclarations(modelOptions);
  ...
  try {
    ...
    // failure ("(ETIMEDOUT/connect)") never left the client (always
    // retried); a read/write-phase one (ECONNRESET, ETIMEDOUT/read) may
  ...
  if (!finishReason && !streamEnded) {
    throw new Error('OpenAI request failed: incomplete stream (missing finish_reason or [DONE])');
  }
...
export async function completeOpenAI({ baseUrl, apiKey, model, messages, maxChars, maxOutputTokens, temperature, modelType, modelOptions, enableReasoning, timeoutMs, signal, headers }) {
  const url = openaiChatUrl(baseUrl);
  const body = {
    model,
    messages,
  ...
  // Bridge an external abort signal into this request (issue #6): before,
  // completeOpenAI had NO signal parameter at all, so callers of
  // LLMClient.complete() could never cancel a slow call — only wait out the
  // timeout. Mirrors streamOpenAI's listener discipline: forward once and
  // detach when the request settles so a retried call reusing the same
  // signal doesn't accumulate listeners.
  const onUserAbort = () => ctrl.abort(signal.reason);
  if (signal) {
    if (signal.aborted) ctrl.abort(signal.reason);
    else signal.addEventListener('abort', onUserAbort, { once: true });
  ...
  try {
    ...
    // Transport failures (fetch-level AND body-read) must carry the
    // "OpenAI request failed:" prefix + undici cause, exactly like
    // streamOpenAI — otherwise retries.js cannot classify them and a raw
    // TypeError('fetch failed') fails fast without a single retry.
    // Already-shaped HTTP status errors pass through untouched.
    if (!/^OpenAI \d{3}: /.test(String(err?.message || ''))) {
      const causeCode = err?.cause?.code;
      const cause = causeCode
        ? ` (${causeCode}${err.cause.syscall ? `/${err.cause.syscall}` : ''})`
        : '';
      throw new Error(`OpenAI request failed: ${err?.message || err}${cause}`);
    }
    throw err;
  } finally {
    clearTimeout(timeoutHandle);
