 ...
 *   - SAFE (request never left / was refused before execution):
 *     connection-establishment failures (ECONNREFUSED, ENOTFOUND, EAI_AGAIN,
 *     undici UND_ERR_CONNECT_TIMEOUT) and HTTP 429 (the server refused the
 *     request; honors Retry-After). Retrying these cannot duplicate work.
 *   - UNKNOWN OUTCOME (may have been executed): ECONNRESET / ETIMEDOUT /
 *     EPIPE / socket hang up after the request was sent, AND HTTP 502/503/
 *     504 — a reverse proxy can return 502/504 AFTER the upstream finished
 *     the inference, so a re-POST may duplicate the request and the billing.
 ...
 */
// 429 alone is outcome-safe (refused before execution). 502/503/504 join
// the network-unknown class: the gateway may speak for an upstream that
// already ran the request.
const SAFE_STATUS = new Set([429]);
const UNKNOWN_STATUS = new Set([502, 503, 504]);
const ATTEMPTS = 3;
...
export async function fetchWithRetry(url, init, { attempts = ATTEMPTS, onRetry } = {}) {
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    try {
      ...
      const safe = SAFE_STATUS.has(resp.status);
      const unknown = UNKNOWN_STATUS.has(resp.status);
      if (!safe && !unknown) return resp;
      if (unknown && !retryUnknownEnabled() && i < attempts - 1) return resp;
