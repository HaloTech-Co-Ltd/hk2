...
> `models.json`, `projects.json`, permission settings, history, `kb/`, `sessions/`, `logs/`) *and* the default
...
> them back (destructive replacement requires both `--preserve-data=off` and
> `--confirm-data-loss`). Prefer `npm link`
...
| `HK2_ENABLE_AUTOCOMPACT` | When 1 (default), hk2 auto-compacts the prior conversation at the start of a turn once the measured context length reaches `HK2_AUTOCOMPACT_PCTUSED`% of the model's context window — AND recovers mid-run from provider "prompt is too long" rejections (e.g. Anthropic 400 code 1261): the conversation is compacted at the interruption point, a resume instruction (original request + live plan progress) is injected, and the agent loop re-runs on the compacted history so the interrupted task continues instead of dying. At most 2 such recoveries per turn; if compaction cannot make the prompt fit, the original error surfaces. Compaction keeps the last 4 user/assistant turns verbatim and LLM-summarizes earlier turns (including their tool results) into a single system message, falling back to naive truncation if the LLM fails. The threshold-based compaction fires only at turn boundaries, never interrupting an in-flight action; overflow recovery additionally fires mid-run after a provider context-length rejection. Before the turns are summarized away, durable user-stated facts are extracted into the session facts store (see `/remember`), and the summarizer input keeps the conversation's head AND tail so opening-stated facts reach the summary verbatim — auto-compaction no longer loses them. | `1` |
