...
| `HK2_WELCOME` | TUI welcome card tier: `full` always the logo card with the tips panel, `compact` always the single-column fact card, `auto` (default) full on first run and compact for returning users / screens shorter than 30 rows. | `auto` |
