...
| `HK2_LLM_RETRY_UNKNOWN_POST` | LLM requests with an UNKNOWN outcome are NOT retried by default, to avoid duplicate requests and duplicate billing: mid-flight transport failures (reset / read- or write-phase timeout AFTER the request was sent), and HTTP 500/502/503/504 (a reverse proxy can return these AFTER the upstream already ran the inference). Set `1` to opt in. Connection-establishment failures (refused / DNS / undici connect-timeout / connect-phase `ETIMEDOUT`, surfaced as `(CODE)` or `(CODE/connect)` in the error message) and HTTP 408/429 (refused before execution) are outcome-safe and always retried, bounded by `HK2_LLMAPI_NUMOFRETRIES`. | `0` |
