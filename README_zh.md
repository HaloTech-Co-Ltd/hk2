...
一个以知识库（KB）驱动、专为编码场景而生的智能体。会话中的收获可以沉淀
为可复用的知识，后续任务因此能够从之前学到的东西出发——前提是相关知识
已保存并被检索命中。
...
重新犯同样的错误。hk2 反其道而行：用 `/project init` 注册项目、`/kb init` 构建它的**知识
库**——符号、代码知识图谱与沉淀的知识条目——对实质性请求，hk2 通常会在智能体循环前
尝试预取相关 KB 上下文。预取属于尽力而为；快速通道后续输入或检索失败时，循环可能在没有
新预取块的情况下继续，智能体仍可按需调用 KB 工具。让
...
- **项目级三空间知识库**——Holy Space（稳定的设计知识）、Eden Space（频繁
  ...
  策略。
- **Tree-sitter AST 索引**——15 个语法（14 个包）的原生解析；语法不可用时
  大多数语言有正则回退（C# 没有），符号、调用链、类继承与导入构成可查询
  的图谱。
- **实质性请求的上下文预取**——对实质性请求，hk2 通常会在 LLM 回答前尝试预取
  相关符号、调用链、类成员、知识条目与文档。预取属于尽力而为。
- **知识库优先的智能体**——工具注册表引导智能体先查知识库工具再用
  `bash grep`，中途守卫兜底；内置本地路径工具都运行在项目外默认拒绝的
  r/w/x 权限模型之后。
- **深度研读**——`/kb knowledge learn` 让 LLM 通读代码库（或文档）并撰写
  可复用的知识条目；大项目自动切换为目录级规划。
- **项目最高准则（Project Supreme Code）**——一个受保护的 Holy 条目，存放
  项目不可协商的法则；写入条目后即以最高优先级渲染进每次系统提示（模型层
  遵从；条目本身的存储层保护是硬限制）。
- **规划与审查**——交互式 REPL/TUI 会话可呈现用户确认的计划、实时进度面板、
  可选计划审查与完成结果审查。没有确认/进度回调时，自动采纳推荐策略且不维护真实
  进度面板。
