...
| `HK2_ENABLE_AUTOCOMPACT` | 为 1 时（默认开），两层保障：（1）当已使用的上下文长度达到模型上下文窗口的 `HK2_AUTOCOMPACT_PCTUSED`% 后，hk2 会在下一轮开始时自动压缩历史对话；（2）任务中途碰到提供商的“prompt is too long / 上下文过长”拒绝（如 Anthropic 400 错误码 1261）时，立即在中断点压缩对话、注入恢复指令（含原始请求与进行中的计划进度），并在压缩后的历史上重跑 agent 循环，被中断的任务自动继续而不是整轮报废。每轮最多 2 次恢复；若压缩后仍超限则浮出原始错误。压缩会原样保留最近 4 轮 user/assistant，并将其之前的对话（含工具结果）用 LLM 总结为一条 system 消息；LLM 失败时回退为朴素截断。阈值触发的压缩只在轮次边界发生，绝不中断进行中的动作；溢出恢复则额外在任务中途、提供商拒绝上下文过长请求后触发。在对话被总结掉之前，用户陈述的持久事实会先被抽取进会话事实层（见 `/remember`），且摘要器输入同时保留对话的头部与尾部——开头陈述的事实逐字进入摘要，自动压缩不再丢失它们。 | `1` |
