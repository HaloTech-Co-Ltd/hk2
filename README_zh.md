...
当 `HK2_ENABLE_PLANREVIEW=1`（默认关闭）时，用户确认计划后，LLM 会重新分析需求，检查计划的需求覆盖（每个必要部分是否被交付）、步骤顺序、可行性、风险与未声明假设，并在执行开始前将发现的问题逐一呈现给用户确认。审查分析实时流式展示；无法解析出判定 JSON 时显式报 UNKNOWN，绝不伪装成"未发现问题"；详见环境变量表。
