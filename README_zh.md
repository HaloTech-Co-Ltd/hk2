...
- **选择适合你的终端体验**——经典行式 REPL 与内联 TUI（`hk2 --tui`）
  共享智能体能力、会话与命令。
- **附带图片、视频与语音**——通过 `--multimodal=on` 配置的模型（如
  glm-5.3-flash）自动看见媒体：智能体 `read` 图片 / 视频 / 语音文件时，
  真实内容自动注入对话（无需手动步骤）；也可用 `/attach` 手动附带附件；
  本地文件在发送时 Base64 编码。参见
  [模型、项目与会话](docs/zh-CN/guides/models-projects-and-sessions.md)。
