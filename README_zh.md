...
库**——符号、代码知识图谱与沉淀的知识条目——对每条实质性请求，hk2 都会把相关 KB 上下文预取
...
- **Tree-sitter AST 索引**——15 个语法（14 个包）的原生解析；语法不可用时
  ...
  的图谱。
- **实质性请求的上下文预取**——对实质性请求，相关符号、调用链、类成员、
  知识条目与文档会在 LLM 回答前被预取并注入提示词。
...
- **项目最高准则（Project Supreme Code）**——一个受保护的 Holy 条目，存放
  ...
  遵从；条目本身的存储层保护是硬限制）。
- **规划与审查**——交互会话可使用用户确认的计划与实时进度面板；非交互运行
  自动采纳推荐策略，并支持可选的计划审查与完成结果代码审查。
