...
export const HELP_TEXT = {
  model: [
    ...
    `  del-mcpserver <provider>/<model-id> --name=<n> | --all`,
    ...
    `  --multimodal=on|off                               Multimodal input (image/video/audio; capable types only,`,
    `                                                    default off — see /help attach)`,
    `  --built-in-tools=on|off                           Server-side built-in tools in the request body (anthropic`,
    `                                                    dialect; default on — --built-in-tools=off opts out)`,
    ``,
    `Flags (set-phase):`,
    ...
    ``,
    `Flags (del-mcpserver) — remove MCP server(s) from a model:`,
    `  --name=NAME               Remove the single server with that exact name (--name=all = --all)`,
    `  --all                     Remove every MCP server on the model`,
    ``,
    `Examples:`,
    `  /model list`,
    ...
    `  /model del-mcpserver bigmodel2/glm-5.3[1m] --name=web-reader`,
    `  /model del-mcpserver bigmodel2/glm-5.3[1m] --all`,
...
`                           entry. CODE mode always writes Eden.`,
  ...
  attach: [
    `Usage: /attach <file-or-url> [<file-or-url> ...]`,
    `Stage image / video / audio attachment(s) for your NEXT message.`,
    ``,
    `  /attach <path> [...]    stage local file(s) or http(s) image/video URL(s)`,
    `  /attach                 list what is currently staged`,
    `  /attach clear           drop everything staged`,
    ``,
    `Requires the session model to have multimodal input enabled:`,
    ...
    `(mp4 mov mkv avi webm flv m4v), audio (wav mp3 m4a aac ogg flac opus).`,
    `Local files are read and Base64-encoded as data URLs at send time`,
    `(limit 20 MB per file); remote URLs pass through untouched. Staged`,
  ...
  tool: [
    ...
    `View and configure the multimodal vision tool suite (10 tools).`,
    ``,
    `The tools forward images / video to a DEDICATED multimodal model and`,
    `return its analysis as text — any session model gains multimodal`,
    `capabilities, even non-multimodal ones. Settings persist in`,
    `~/.hk2/tools.json (visionModelRef + toolModels + disabled[]).`,
    ``,
    `Subcommands:`,
    `  list                        List the vision tools and their status`,
    `  show <name>                 Describe one tool (inputs and parameters)`,
    `  enable <name>               Re-enable a disabled tool`,
    `  disable <name>              Disable a tool (it disappears from the agent)`,
    `  set-model <provider>/<id>   Set the default multimodal model for the suite`,
    `  set-model <tool> <p>/<id>   Set ONE tool's own multimodal model (wins over`,
    `                              the default for that tool only)`,
    `  clear-model [tool]          Remove the suite override, or one tool's own`,
    `                              override when a tool name is given`,
    `  reset                       Re-enable every disabled tool`,
    ``,
    `The 10 vision tools (agent-facing):`,
    `  ui_to_artifact                UI screenshot → code / prompt / design spec /`,
    `                                description`,
    `  extract_text_from_screenshot  OCR: code, terminal output, documents, text`,
    `  diagnose_error_screenshot     error dialogs / stacks / logs → diagnosis`,
    `  understand_technical_diagram  architecture / flow / UML / ER interpretation`,
    `  analyze_data_visualization    dashboards & charts → trends, anomalies`,
    `  ui_diff_check                 design vs implementation visual diff`,
    `  image_analysis                general image understanding`,
    `  capture_and_analyze           LIVE screenshot of the current screen →`,
    `                                multimodal analysis (issue locating);`,
    `                                macOS / Windows / Linux`,
    `  record_and_analyze            LIVE 1-60s screen recording → video`,
    `                                analysis; macOS; Windows/Linux need ffmpeg;`,
    `                                Linux Wayland recording unsupported`,
    `  video_analysis                video scenes, key frames, events`,
    `                                (local files ≤ 8 MB)`,
    ``,
    `Setup:`,
    `  /model add bigmodel glm-5.3-flash --model-type=glm-5.3-flash --multimodal=on`,
    ...
    ``,
    `Resolution per tool: toolModels[name] → visionModelRef → a multimodal`,
    `session model → the tool is not registered. Without any configured model`,
    `the tools are not registered with the agent at all.`,
  ],
  help: [
    `Usage: /help [command]`,
