...
export const HELP_TEXT = {
  ...
  kb: [
    ...
    `  /kb code add --code-content="API Key 绝对禁止出现在任何代码文件中"`,
    `  /kb code del 2`,
  ...
  code: [
    ...
    `Manage the project's Supreme Code (法典) — the permanent Holy entry`,
    ...
    `  /kb code list`,
    `  /kb code add --code-content="API Key 绝对禁止出现在任何代码文件中"`,
    `  /kb code add 1 --code-content="代码规范必须严格遵循 **KB(project-code-format)**"`,
    `  /kb code add --code-gen="制定一条关于提交信息格式的法条"`,
    `  /kb code del 2`,
  ],
  session: [
    ...
    ``,
    `Subcommands:`,
    `  info                Show session id, project, message and tool-call counts`,
