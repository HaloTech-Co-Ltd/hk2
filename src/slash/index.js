 ...
 *
 * Lines whose first token satisfies looksLikeSlashCommand are command
 * attempts; path-like or prose inputs beginning with / go to the agent loop.
 *
 * ctx interface (constructed by interactive.js):
 *   - print(text)               general output
 *   - confirm(prompt) → Promise<boolean> asynchronous y/N confirmation
...
export const SLASH_COMMANDS = [
  ...
  { name: '/session', handler: cmdSession, description: 'Session management (info / list / new / resume / compact)' },
...
/**
 * Dispatch a line when its entire first whitespace-separated token matches
 * the anchored slash-command shape; otherwise return false for agent input.
 *
