...
that point unless `replaceAll:true`, and later entries may match text produced by earlier entries. If a
...
Matching tries exact text first, then LF-to-CRLF adaptation, then a unique
sequence of lines with leading/trailing whitespace and blank lines ignored.
The fuzzy fallback adapts replacement indentation and line endings to the file;
it does not tolerate changed words or arbitrary internal whitespace. The
`fuzzyAdapted` response field reports that fallback, while `eolAdapted` reports
an exact LF-to-CRLF anchor adaptation. The `replaceAll` path also reports an
`eolAdapted` item with `lineEndings: LF→CRLF` as metadata even when the anchor
matched verbatim and the replacement contains no line break, so that item does
not prove that every replacement was line-ending-converted. Failures include
the failing edit index and nearby-line diagnostics where available; no edits
are written when a later anchor fails.

Top-level `replaceAll:true` replaces all non-overlapping occurrences of an exact
or EOL-adapted anchor. Its reported `count` is computed by advancing the search
one character at a time, so overlapping anchor matches can make `count` larger
than the number of replacements actually written. It does **not** permit
multiple fuzzy matches: re-read the file and choose a unique anchor in that
case.
