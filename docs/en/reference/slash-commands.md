...
| Subcommand | Effect |
|---|---|
| `init [--full] [--checkpoint-interval=N] [--no-checkpoint] [--no-resume] [--skip-summary]` | Build the KB — **always a full re-index** in the current implementation (with or without `--full`; use `/kb update` for incremental updates); checkpointed and resumable; summary generation is attempted only when a model is configured and `--skip-summary` is not passed, with each non-empty success written independently. An LLM failure or empty result skips that entry and later calls continue; a storage write error may abort remaining summaries. `--checkpoint-interval=` falls back to the environment/default wrapper; `0`, negative, and invalid non-empty values do not disable checkpointing; use `--no-checkpoint` to disable |
...
| `housekeep <eden\|holy\|all> [--model=<provider>/<model-id>]` | LLM-assisted: broken-entry scan, duplicate/similar merge (y/N), Eden↔Holy conflict resolution (`all`, per-pair choice). Supreme-code never touched; indexes rebuild only when a change was written (otherwise remain untouched) |
| `empty <eden\|holy\|all>` | Deletes every **ordinary** entry in the selected space(s); the permanent Supreme Code entry is preserved. If ordinary entries exist, the operation is irreversible and confirms y/N; an already-empty selection reports that state without prompting |
...
(bare, or `--base-dir` = an indexed path, including a single indexed file)
deep-studies indexed source — an optional Phase 0 survey (runs only when not
`--dry-run`, no `--base-dir`, not `--no-survey`), Phase 1 topic planning, Phase 2
extraction. Each planned batch is split by the `--per-batch-chars` budget when
needed; every execution sub-batch starts its own extraction call (an
unparseable response may trigger one retry), so one planned batch does not
necessarily mean exactly one call. DOC mode reconciles the plan
to add omitted parsed files and runs an individual fallback for files dropped
by the plan. CODE mode executes an accepted plan as returned and does not add
omitted indexed files; only an absent, unusable, or discarded plan falls back
to deterministic directory grouping with full coverage of the indexed scope.
...
delete project-level `taskstate.json`. `/session resume` prepares the target
transcript before clearing old task state and applying the restored messages.
It restores the task anchor when saved `userRequest` exists and the session ID
matches; restoring the plan panel additionally requires unfinished steps.
It does not restore the in-memory `lastCompletedTask` snapshot.
