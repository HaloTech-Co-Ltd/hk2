...
- **Cause**: the Tree-sitter native bindings are missing, cannot be loaded, or
  do not match the current platform/Node ABI; a skipped or incomplete
  `npm install` is another possibility. The error alone does not identify
  which of these causes applies.
- **Fix**: hk2 already fell back to its regex parsers — languages with a
  ...
  (Node 24 Active LTS is recommended as of September 2026; Node 22 Maintenance LTS is a fallback)
...
- **Cause**: `npm install --omit=optional` failed midway (for example because
  of the network or a missing toolchain), so the copied install may have
  incomplete native dependencies.
...
- **Cause**: the `tree-sitter` package is not loadable at all (not installed,
  `--no-npm-install` was used, or the native package cannot be loaded).
...
- **Cause**: `models.json` has no resolvable default model (the default is
  missing or its provider/model reference is stale).
- **Fix**: `/model add <provider> <id> ...` then
  `/model set-default <provider>/<id>`; or export `ANTHROPIC_API_KEY` /
  `OPENAI_API_KEY` before first run to seed a provider; or, when there is no
  default and no `claude` provider, run `hk2 --tui` once to import from Claude
  Code's config.
...
- **Fix**: `--plan-timeout-ms=600000` on the learn command, or set
  ...
  `HK2_LLMAPI_TIMEOUT_MS_SIMPLE` globally. An explicit `0` means "no timer"
  for those two LLM timeout variables when no per-call or per-model timeout
  overrides it;
  `HK2_PLAN_TIMEOUT_MS=0` falls back to the default instead.
...
- **Cause**: tier-1 deterministic continuation rules did not match. Tier-2
  ...
  `0.6`), and a prior conversational referent exists: a plan, `lastTask`, or
...
- **Fix**: hk2 retries once with reasoning disabled. In CODE mode, an accepted
  plan is executed as returned, so omitted files are not automatically added;
  an unavailable or discarded CODE plan falls back to deterministic directory
  grouping, which covers every indexed file in the selected scope. DOC mode reconciles every
  successfully read, parsed, non-empty study part and adds single-file
  fallback batches for planner omissions. Other errors can still stop or skip
  content. For slow providers raise the budget:
...
- **Cause**: non-TTY stdin/output, `TERM=dumb`, or a CI console. Capability is
...
- **Cause**: the saved plan panel is restored only when taskstate has unfinished
  steps; completed plans do not restore a plan panel (task context may still
  restore when taskstate records an interrupted task).
...
- `HK2_DEBUG` — any non-empty value prints error stacks (also stacks under
  `/command` errors).
- `HK2_ASCII` — any non-empty value forces ASCII fallbacks on non-UTF-8
  terminals.
- `HK2_NO_COLOR` or `NO_COLOR` — any non-empty value disables ANSI colors
  (`0` also counts); remove these overrides when colors are desired.
