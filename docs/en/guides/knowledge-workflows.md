...
- `/kb update` incrementally re-parses changed source files, rebuilds the
  ...
  parser-owned `doc:<relpath>` Eden entries (successfully parsed new/changed
  docs are written or replaced; deleted/excluded docs' parser-owned entries are
  removed; a failed parse of a previously indexed document can leave its prior
  entry in place). It auto-detects a
...
- **CODE mode** — no `--file`, or `--base-dir` pointing at an *indexed* path
  (including a single indexed file). Three-phase study of indexed source: an **optional** Phase 0
  ...
  and NOT `--no-survey` (`--base-dir` scopes to the indexed path and skips
  the whole-project survey; `--dry-run` and `--no-survey` skip it too);
  Phase 1 plans topic batches; Phase 2 performs one extraction pass per execution
  sub-batch (with one reasoning-disabled retry if the output is unparseable)
  ...
  written.
- **DOC mode** — `--file=<path>` or `--base-dir=<dir>` that resolves to a real
  directory but does not match an indexed path in the project's indexed-path
  coordinate system. Deep-studies Markdown / PDF / Word / PowerPoint / text
...
once with reasoning disabled. In CODE mode, a plan accepted by that mode's
parsing/expansion checks is executed as returned; omitted files are not
automatically reconciled. Only an unavailable or discarded CODE plan falls
back to deterministic directory grouping, which covers every indexed file in
the selected scope.
DOC mode always reconciles successfully read, parsed, non-empty study parts
and adds single-file fallback batches for planner omissions. File access,
parsing, model, permission, disk errors, or a user interrupt can still stop
the run or skip content.
...
menu. The supreme-code entry is never touched; indexes for the selected
space(s) are rebuilt when anything changes. `--model=<provider>/<model-id>`
picks the model.
