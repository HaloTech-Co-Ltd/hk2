...
flowchart LR
    ...
    K --> L[doc_index.json]
    C --> J[Attempted LLM summaries<br/>project-overview etc.]
```
...
5. **Summaries** — at the end of `/kb init`, **when a model is configured
   and `--skip-summary` is not passed**, hk2 independently attempts the three
   structural Eden summaries. Each non-empty successful call writes its own
   fixed-id entry; an LLM call failure or empty result skips that entry while
   later calls are still attempted. A storage write error can abort the
   remaining summary generation. Without a configured LLM the index is still
   built and all summary entries are skipped.
...
- **Incremental update** — `/kb update` re-hashes files (sha256) and
  ...
  `backup/pre-upgrade-<ts>/` first; if that safety backup fails, migration
  aborts rather than upgrading. The migration is then applied (a parser-version
  change triggers a full re-index).
