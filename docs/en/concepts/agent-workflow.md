...
2. **Auto-compact check** — at the *start* of the turn (a safe boundary),
   if `HK2_ENABLE_AUTOCOMPACT=1` (default on) and the
   ...
   automatic extraction is best-effort. This threshold-based check never
   interrupts an active task. Separately, if a provider rejects an in-flight
   request because the context is too long, hk2 compacts at that interruption
   point, injects resume context, and reruns the agent loop. This recovery is
   limited to two attempts per turn; unrelated errors are not retried through
   compaction.
...
4. **Query rewrite** (`HK2_ENABLE_QUERYREWRITE`, default on) — one rewrite
   phase rewrites the query into English function names + keywords for BM25
   retrieval. A phase model (`rewrite-query`) can drive it; if that configured
   model is unreachable, `HK2_ENABLE_PHASEMODEL_FALLBACK` (default on) retries
   the same phase on the session model, or skips it when set to 0. A stale ref
   resolving to `null` is silently treated as no override, while a resolution
   exception warns and uses the session model. A clarification later in the
   turn intentionally runs a separate second rewrite pass.
...
6. **Request-clarity assessment** (`HK2_ENABLE_REQUEST_ASSESS`, default on) —
   one bounded assessment phase judges whether the request is clear, *against the
   session context* (available task context, active plan, the assistant's latest
   message, recent turns, recorded session facts). The assessor considers that
   context to reduce false positives; it is still an LLM judgment, not a
   guarantee. If a configured assessment phase model is unreachable, the same
   fallback policy may run one retry on the session model. hk2 always requests
   `enableReasoning:true` for this assessment;
...
`plan` with a one-line summary and the recommended shape of 2–5 ordered steps,
each with 2–4 candidate strategies (one marked recommended). Runtime
normalization requires at least 2 usable steps and at least 2 named strategies
per step, but accepts larger arrays and repairs a missing/non-unique
recommendation. Simple tasks skip straight to execution.
...
A fresh launch without `--resume` can load the project's saved task anchor,
but keeps an unfinished plan pending instead of displaying it immediately.
The REPL/TUI prints a `[recovery]` notice. On the first agent turn, `continue`
(or a tier-2 continuation upgrade) activates that plan; a new task discards
the pending plan. Slash commands do not themselves consume an agent turn.
Replacing the session or switching projects clears the old pending recovery.
This lightweight recovery does not replay the previous conversation; use
`--resume` for transcript replay and session-ID-matched task restoration.
