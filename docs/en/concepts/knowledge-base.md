...
| **Index** | Code index (BM25 over symbols), knowledge graph (call / import / inheritance relationships), and per-space indexes over Holy/Eden entries. | Explicit `/kb init` / `/kb update` run immediately; the end-of-turn automatic update is controlled by `HK2_ENABLE_AUTOUPDATEKB`. |
...
- **Protection (hard limits)**: the entry itself cannot be deleted, renamed,
  moved, imported over, or auto-updated through the supported knowledge
  mutation paths. Its items are managed only by `/kb code add|del`, so deleting
  the last item can leave the permanent entry present but empty; the supported
  command/storage guards protect the entry while `writeSupremeCode` validates
  item writes.
...
successful write overwrites the previous version of that fixed id. If an LLM
call fails or returns empty, that entry is skipped and later LLM calls are still
attempted; a storage write error can abort the remaining summary generation:
