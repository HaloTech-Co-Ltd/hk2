...
- Tests that use persisted state should redirect `HK2_HOME` to a temporary
  directory before importing modules that resolve it; `_learn_setup.js`
  implements this pattern. Follow it in new tests and check subprocess
  environments too; a passing suite alone does not prove filesystem isolation.
