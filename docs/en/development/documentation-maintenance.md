...
3. **Language switch** — every page under `docs/en/` or `docs/zh-CN/` carries,
...
capability highlights, requirements, the shortest install, a short quick
start, one example, and documentation navigation. The MIT license remains in
the root `LICENSE` file; the READMEs omit a license section. They do **not**
...
- every checked page has exactly one H1 and heading levels do not skip;
  for paired pages under `docs/`, the language switch must be in the first
  block after the H1 (ATX headings may be indented by
...
- local Markdown links and images in the root READMEs, paired Markdown
  pages under `docs/`, and `docs/README.md` resolve to real files;
...
The checker validates file targets, but does not check heading fragments,
external URLs, or HTML image sources. The language-switch position check
does not apply to the root READMEs or the language picker.
