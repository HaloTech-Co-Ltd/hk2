...
- **CLI layer** — `bin/hk2` (single executable entry) imports
  ...
  (`project-init`, `build-kb`, `update-kb`), `--run-mode=serve` (legacy REPL), and
...
- **Interactive front-ends** — `src/commands/interactive.js` (line REPL:
  ...
  streaming markdown, modals). Each frontend creates and shares its own
  session object with the non-rendering layers: slash dispatch, the turn
  pipeline, and the transcript.
...
5. Independently attempt the three LLM summary entries (`summarize.js`); each
   is written only when its LLM call returns non-empty content. A failed or
   empty LLM call skips that entry and later calls are still attempted, while a
   storage write error can abort the remaining summary generation.
