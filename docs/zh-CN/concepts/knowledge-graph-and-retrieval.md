...
flowchart LR
    ...
    K --> L[doc_index.json]
    C --> J[尝试 LLM 摘要<br/>project-overview 等]
```
...
2. **解析**——`lib/parser/ast.js` 按文件扩展名分发。Tree-sitter 原生绑定
   ...
   由 `lib/parser/doc_parser.js` 处理，并以 `doc:<relpath>` 条目归入 Eden
   空间。
...
5. **摘要**——`/kb init` 结束时，**在已配置模型且未传 `--skip-summary` 的
   情况下**，hk2 会分别尝试生成三个结构性 Eden 摘要。每次调用只有在返回非空内容
   时才写入对应固定 id；某次 LLM 调用失败或返回空内容时会跳过该条目，随后仍会
   尝试后续调用。存储写入错误可能中止剩余摘要。未配置 LLM 时索引照常构建，
   所有摘要条目都会跳过。

解析在有界并行池中运行——`HK2_INDEX_PARALLEL` 固定并行度（默认：自动，
取宿主 CPU 数并遵循 cgroup 配额）。

## 符号模型

知识库统一使用 Symbol 记录表示代码符号。各源码解析器都返回结构一致的 `Symbol[]`。
只有 Tree-sitter 的语法定义和对应的提取器提供了相关信息，才会填充
`qualName`、`parentSymbolId`、`superClass`、`implements`、`imports` 与 `docString` 等
可选字段。BM25、图谱、大纲和调用链都基于 Symbol。因此，缺少 Tree-sitter
语法支持时，配有正则回退解析器的语言仍可提取符号，但精度会降低；没有回退
解析器的语言（尤其是 C#）则完全不产出符号。
...
- **增量更新**——`/kb update` 对文件重新计算 sha256，只对发生变化的源码文件
...
- **检查点**——`/kb init` 每处理 N 个文件保存一次检查点
  ...
  检查点，`--no-resume` 从头开始。
- **大型项目**——解析并行度会随 CPU 数量扩展；`/kb knowledge learn` 的规划器
...
- [智能体工作流](agent-workflow.md)——检索在回合处理流程中的位置
