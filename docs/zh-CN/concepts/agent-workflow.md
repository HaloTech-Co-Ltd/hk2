...
- **工具结果缓存**——同一 `runLoop` 内完全相同的只读工具调用复用结果；
  ...
  见工具参考）。
- **卡死检测与纠正**——同一工具调用签名与结果指纹连续出现时，首次不计为
  重复，其后重复三次（即第四个相同轮次）触发**纠正性 system 提示**注入：要求
  模型打破循环（诊断失败调用的根因、把尚未就绪的轮询改为单次更长等待、或停止
  调用工具直接汇报已知结果），随后重置重复计数窗口。重复的工具调用本身绝不
  致命：只有在整个纠正预算（`HK2_STUCK_NUDGE_LIMIT`，默认 10 次）耗尽且模型仍
  未跳出循环时才会中止；一旦出现真实进展（签名或结果变化）预算即重新满额。
  另有 1000 轮的绝对上限兜底。代码中
