...
2. **自动压缩检查**——在轮次**开始时**（安全边界），若
   ...
   然后重新运行智能体循环。每轮最多恢复两次；其他类型的错误不会通过压缩重试。
...
4. **查询改写**（`HK2_ENABLE_QUERYREWRITE`，默认开启）——模型将查询改写为
   ...
   会话模型。之后若用户补充了澄清信息，还会在当前轮结合这些信息再次改写查询。
...
6. **请求清晰度评估**（`HK2_ENABLE_REQUEST_ASSESS`，默认开启）——模型在限定的
   ...
   计划、助手最近消息、近期对话轮次、已记录的会话事实），以降低脱离上下文造成
...
另一条常驻消息——`## Session facts`——会在每一轮紧跟主系统提示词出现，
...
1. **自动知识库更新 / `[kb learn]` 询问**——这一整块**只有在本轮记录到
...
4. **代码审查**——另一条独立流程，受 `HK2_ENABLE_CODEREVIEW=1` 门控：
   循环正常返回后，当本轮涉及计划（或从继续计划开始），且计划面板已在
