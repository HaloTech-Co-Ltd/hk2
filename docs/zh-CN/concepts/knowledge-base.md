...
`hk2-supreme-code`——存放项目的**最高准则**：一组简短、使用祈使语气的强制
规则。在本项目的**一切操作**（读、写、编辑、规划、回答）中，hk2 都会以
最高优先级要求智能体遵守这些规则（模型层遵从；见下方注入说明）。它由 `/kb init` 创建，
...
- **保护（硬限制）**：通过受支持的知识变更路径，条目本身不能被删除、重命名、
  移动、导入覆盖或自动更新。其规则只能由 `/kb code add|del` 管理，因此删除
  最后一条规则后，永久条目仍在但可以为空；受支持的命令和存储层守卫保护的是
  条目本身，而 `writeSupremeCode` 负责校验规则写入。
...
的 `/kb code add` 追加为第 N+1 条；编号 > N+1 会被拒绝）。规则应简短，并使用
祈使语气；真正复杂的规则应放入独立的 Holy 条目，并在准则中以
`**KB(entry-id)**` 引用。`/kb status` 会显示当前条数。

## 自动生成的 Eden 条目

`/kb init` 与 `/kb knowledge learn` 会生成一组相互补充、由 LLM 撰写的 Eden
条目——无需手动编写。

**`/kb init`** 在**已配置模型且未传 `--skip-summary`** 时尝试生成 3 个
固定 id 的结构条目（未配置 LLM 时索引照常构建，摘要被跳过）。每个摘要是
独立的 LLM 调用：仅当对应调用返回非空内容才写入该条目，成功写入会覆盖
该固定 id 的旧版本。若某次 LLM 调用失败或返回空内容，则跳过该条目并继续
尝试后续 LLM 调用；存储写入错误可能中止剩余摘要生成：
...
副作用。首次加载 `KBRuntime` 时也会尝试同样的缺失条目自愈。
