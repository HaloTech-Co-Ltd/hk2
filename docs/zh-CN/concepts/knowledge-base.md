...
每个已注册的项目都有独立的知识库，以项目 UUID 隔离在
`~/.hk2/kb/<projectId>/` 下。项目之间互不共享；移除项目注册会保留其知识
库，直到你显式删除。
...
1. **创建**——条目进入 Holy 或 Eden 的途径：手动 `/kb knowledge add`、
   ...
   轮末 `[kb learn]` 捕获，或 `/kb init` 的自动摘要。
2. **使用**——智能体通过 `kb_knowledge` / `kb_search_knowledge` 检索条目，
   相关条目会作为按请求上下文注入。
3. **写入校验**——每个学习到的条目在写入前都会与现有条目比对（重复跳过、
   相近条目原地合并、冲突裁决——与 Holy 冲突必须由用户裁决）。可用
   `HK2_KB_LEARN_VALIDATE=0` 关闭。
...
规则，hk2 在本项目中的**一切操作**（读、写、编辑、规划、回答）都必须
遵守、不可违反。它由 `/kb init` 创建，初始为**空**（旧项目会自动补建一个
空条目），因此在写入规则之前不会强制任何内容。

- **设计目的**：为项目所有者提供一个单一、始终可见的位置，定义不可协商的
  约束——安全策略、代码规范、合规要求等——其优先级高于智能体的一般偏好
  与任何其他知识库条目。
- **注入方式**：每次请求时，这些规则会被渲染进系统提示词的
  `# Project Supreme Code (MUST OBEY — never violate)` 章节，位于**所有**
  其他注入上下文*之前*。当某项操作会违反任意规则时，智能体必须拒绝执行、
  引用该规则的编号，并提出合规的替代方案。
- **保护机制**：该条目本身永远不能被删除、重命名、移动、清空、导入覆盖或
  自动更新——在命令层与存储层双重强制。
...
**`/kb knowledge learn`** 的代码模式写入概览 + 主题条目（完整模式矩阵见
