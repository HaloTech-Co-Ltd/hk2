...
- `/kb update` 增量重新解析发生变化的源码文件，重建派生符号 / 索引 / 图谱结构与
  ...
  变化文档写入或覆盖；已删除或被排除文档的 parser-owned 条目被移除；已索引文档
  若解析失败，其旧条目可能会保留）。它自动检测旧版知识库：
  先把知识条目备份到 `backup/pre-upgrade-<ts>/`，再按当前迁移代码处理；
  解析器版本变化触发全量重建。

**运行建议**：`/project init` 之后运行 `/kb init`；日常编辑后运行
`/kb update`（或让 `HK2_ENABLE_AUTOUPDATEKB=1` 在智能体回退 bash 搜索时
自动执行）；切换分支或大型重构后再跑一次 `/kb init`——它本身就是全量重建。

### 按入口区分检查点

交互式 `/kb init` 会把环境变量未设置、空值、`0` 或非数字转换为 100；正整数按原值使用，
负数原样传入并导致几乎每处理一个文件就保存一次。显式参数会直接解析，因此 `0`、负数或 `NaN`
同样几乎每处理一个文件就保存一次。`/kb update`、自动更新及旧版命令直接调用索引器时，
不会像交互式入口那样用 `|| 100` 替换这些值：环境值 `0`、负数或非法值原样传给 `Checkpoint`。显式空值
`--checkpoint-interval=` 会改用环境变量或默认值，而不是变成 `NaN`。文档中
提供的禁用方式只有 `/kb init --no-checkpoint`。
...
- **代码模式（CODE mode）**——不带 `--file`，或 `--base-dir` 指向*已索引*
  ...
  校验，可能被跳过、合并、交由冲突裁决流程处理或写入。
- **文档模式（DOC mode）**——`--file=<path>`，或 `--base-dir=<dir>`
  指向一个确实存在、且在项目索引路径坐标系中不对应任何已索引路径的目录。深度研读 Markdown / PDF / Word / PowerPoint / 文本文档并写入所选
  空间。文件可以在项目之外；大文件会切分为顺序分片，保证*已提取出的
  文本*不被静默截断。系统会核对批次是否覆盖每个成功读取、成功解析且非空的研读
  分片，并为规划遗漏的文件补充单文件批次——但读取失败、解析
  失败、空文本与其他错误仍可能跳过内容。

默认 `HK2_KB_LEARN_VALIDATE=1` 时，候选条目写入前会与现有知识库比对校验
（重复 → 跳过，相近 → 原地合并，与 Holy 冲突 → 交由你裁决，Eden 以
...
逐对选择菜单处理 Eden↔Holy 冲突。不会触碰最高准则条目；有变更则重建所选
空间的索引。`--model=<provider>/<model-id>` 指定所用模型。
...
- **新成员上手**——`/kb init` → `/kb knowledge learn` → 引导其查看
