...
- **解决**：hk2 已自动回退到正则解析器（符号覆盖率略低，其余行为不变）。
...
- **原因**：`tree-sitter` 包完全不可加载（未安装，或使用了
  `--no-npm-install`）。
- **解决**：在安装目录执行 `npm install`。该警告仅为提示；一切功能正常，
  只是符号精度降低。
...
- **原因**：hk2 版本之间存储的解析器版本发生了变化——为保证正确性必须
  全量重建。旧版布局会被无损升级（知识内容先快照到
  `backup/pre-upgrade-<ts>/`）。
- **解决**：无需处理，让它跑完即可。
...
- **不会。**每 N 个文件保存一次检查点（默认 100）。重新运行 `/kb init`
  即从检查点恢复；`--no-resume` 从头开始，`--no-checkpoint` 禁用检查点。
