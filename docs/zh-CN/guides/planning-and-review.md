...
- **手动**——`/review code` 审查原始任务请求、运行期间排队且扩展需求的指令
  ...
  `--model` 所引用模型缺失或无效时快速失败；项目阶段引用过期时告警并使用会话模型。
  `lastCompletedTask` 快照只在内存中存在；`/session new` 与恢复都会清除它，
  恢复的会话改用确定性的 transcript 扫描。`/review plan` 为预留，尚未实现。

两条路径都会收集相对 `HEAD` 的 tracked 变更（已暂存与未暂存）以及符合条件的
untracked 文件。在请求 Git 返回文件正文前，每条路径都必须通过当前读权限检查，
包括符号链接解析后的目标。rename/copy 只有旧、新路径均可读时才纳入，否则整项
省略。被拒绝的路径名也不会进入 `changedFiles`，审查输入只加入通用的省略标记。
untracked 正文先限制为前 50 个允许文件、每文件 16,000 字符，之后还受审查提示词
...
`planProgress` 从已有的 pre-commit plan 或启动时暂存的待恢复计划中恢复；只有 pre-commit task 存在时才
恢复 `lastTask`，只有恢复后的 `lastTask` 非空时才注入恢复上下文。即使只有历史对话，也可能
触发升级，但不会虚构 plan、任务锚点或恢复上下文，只避免把输入提交为新的任务快照。
