# 规划与审查

[English](../../en/guides/planning-and-review.md) | 简体中文

本指南讲解 hk2 的规划与审查机制：智能体何时创建计划、交互式确认与非交互
自动接受的区别、实时进度面板、可选的计划审查与代码审查，以及审查者无法
给出判定时会发生什么。相关环境变量默认全部**关闭**。

## 计划

由智能体决定是否规划——不存在执行前的强制计划阶段。当模型判定任务足够
复杂、适合显式拆解策略（多个独立阶段、设计选择或涉及多个子系统）时，它
调用 `plan` 工具提交：

- 一行摘要。
- 建议包含 2–5 个有序步骤，每步提供 2–4 个候选策略，并标记一个推荐项。
  这是提示词中的建议；实际校验只要求至少 2 个有效步骤、每步至少
  2 个有效策略，不限制数量上限。

简单任务完全跳过 `plan`，直接执行。

### 确认

存在交互式确认回调时，计划以菜单形式呈现：你逐步选择策略（或采纳推荐项），
并以 `{ confirmed:true, plan }` 返回定稿计划。取消菜单返回
`{ cancelled:true, ... }`，智能体自行调整。没有确认回调时，工具自动采纳推荐
策略，返回 `{ confirmed:true, plan:..., autoAccepted:true }`——这是工具接受，
不是用户确认；此模式没有用户菜单或真实进度面板。

### 进度面板

交互式计划确认后，状态栏上方会固定显示一个实时面板：

```text
▣ Plan: 同步 README 文档与代码
  ✓ 1. 补全缺失的 plan_step 工具
  ▶ 2. 为进度面板编写文档
    3. 修正 tree-sitter 包数量
    4. 提交并推送
```

交互模式每完成一个已确认步骤，智能体调用一次 `plan_step`；每次调用把**当前**
in_progress 步骤标记为完成并推进面板。`step` 参数不决定推进哪个步骤，
即使其取值非法、越界或顺序不符，也不会跳步。最后一步完成后面板自动清除；
回合正常结束时还有一次收尾兜底，会清掉未被推进的面板。没有进度回调时，
`plan_step` 只对报告的完成情况作确认，不维护进度状态或面板。从未调用 `plan` 的任务不显示
面板。

面板会在中断后保留：中断任务状态（原始请求、摘要、计划进度）会被持久化，
`hk2 --resume` 可恢复它（见
[智能体工作流](../concepts/agent-workflow.md#中断与恢复)）。

## 计划审查

`HK2_ENABLE_PLANREVIEW=1`（默认关闭）时，用户确认计划后、执行开始前，
LLM 会对定稿计划做一次复审。复审者会：

1. 把需求重新分析为编号清单；
2. 逐点检查覆盖情况（每个需求点由哪个步骤覆盖——完整 / 部分 / 缺失）、
   步骤顺序与相互矛盾、每个所选策略的可行性、以及未声明的风险与假设；
3. 把发现的问题逐一呈现给用户确认——采纳复审建议、忽略该问题、或自定义
   解决方案。确认后的解决方案会附加到返回给智能体的计划中。

复审者的思考流以 `✎ thinking`（暗色斜体，默认上限 9 行——
`HK2_HIDE_THINKING=0` 显示全部）实时展示，后续分析也会实时流式输出。
审查会开启推理且 hk2 侧不设截止时间——hk2 自身不会中断审查，但用户仍可
中止，网络故障、提供商断连或进程终止仍可能使其结束。

计划审查仅在交互式 TTY 模式下启用，且采用尽力而为策略：任何失败都原样返回已
确认的计划。

## 代码审查

两种形式的模型解析路径各不相同：

- **自动**——`HK2_ENABLE_CODEREVIEW=1`（默认关闭）：智能体正常返回最终
  文本时，如果本轮确认或继续了计划，收尾逻辑可能触发审查，检查工作区
  diff、变更文件与智能体最终总结。即使模型没有为每个步骤调用 `plan_step`，
  普通最终文本也可能清理面板并触发审查。
- **手动**——`/review code` 审查原始任务请求、运行期间排队且扩展需求的指令
  以及完成结果；工具调用、推理过程和中间回合会被刻意排除。显式指定的
  `--model` 所引用模型缺失或无效时快速失败；项目阶段引用过期时警告并使用会话模型。
  `lastCompletedTask` 快照只在内存中存在；`/session new` 与恢复都会清除它，
  恢复的会话改用确定性的会话记录扫描。`/review plan` 为预留，尚未实现。

两条路径都会收集相对 `HEAD` 的已跟踪（tracked）变更（已暂存与未暂存）以及符合条件的
未跟踪文件。在请求 Git 返回文件正文前，每条路径都必须通过实际生效的读权限检查，
包括符号链接解析后的目标。rename/copy 只有旧、新路径均可读时才纳入，否则整项
省略。被拒绝的路径名也不会进入 `changedFiles`，审查输入中只保留一个通用的省略标记。
未跟踪文件正文先限制为前 50 个允许文件、每文件 16,000 字符，之后还受审查提示词
整体截断约束。收集采用尽力而为方式：Git 错误、文件不可读或项目路径不是仓库时，
diff 可能为空。

### UNKNOWN 判定

只有机器可读的判定 JSON 会被解析，原始内容绝不展示。无法解析出判定的
回复会被报告为 **UNKNOWN**——绝不伪装成“未发现问题”。请把 UNKNOWN 当作
“审查无结论”，而不是通过。

## 审查模型

自动审查与手动审查使用相关但不同的模型解析路径：

| 审查路径 | 项目阶段引用过期 | 显式缺失 `--model` | 选定模型调用失败 |
|---|---|---|---|
| 自动 plan/code 审查 | 静默使用会话模型 | 不适用 | 警告并跳过 |
| 手动 `/review code` | 警告并使用会话模型 | 终止 | 警告并跳过 |

自动审查把过期引用视为没有覆盖，不警告，也不产生 fallback/skip 审计事件。
解析异常则警告并使用会话模型。手动 `/review code` 对过期阶段引用和解析异常
都会警告并使用会话模型；显式 `--model=<ref>` 的引用无效或缺失时绝不回退。
选定模型成功解析后实际调用失败，会跳过审查，不再选择其他模型。


## 推理设置

- 审查始终开启推理且不设固定超时，与模型的 `--reasoning` 标志无关。
- 请求清晰度评估由 hk2 以 `enableReasoning:true` 发起；这不保证每个提供商都
  会返回独立的 reasoning 流。该评估是尽力而为的，超时使用
  `HK2_LLMAPI_TIMEOUT_MS_SIMPLE`——见[环境变量](../reference/environment-variables.md)。

## 后续输入分类

hk2 使用两层分类，而不是对每条输入都运行 LLM 分类器：

```mermaid
flowchart TD
    U[用户输入] --> T1{tier-1 确定性匹配？}
    T1 -- 是 --> C[直接继续]
    T1 -- 否 --> A[正常评估路径]
    A --> V{followup=true + 达到阈值 + 存在先前会话指代？}
    V -- 否 --> F[保持新任务分类]
    V -- 是 --> UPGRADE[tier-2 continuation upgrade<br/>恢复可用的 plan/task 状态<br/>仅在存在 lastTask 时注入恢复上下文<br/>记录 followupUpgrade 元数据]
```

`HK2_ENABLE_CONTINUATION_UPGRADE` 默认对未被 tier 1 识别的输入启用，并依赖
请求评估实际运行。`HK2_CONTINUATION_UPGRADE_MIN_CONFIDENCE` 默认 `0.6`，用
`parseFloat()` 解析并钳制到 `0–1`（非法值使用 `0.6`）。评估器（assessor）必须返回
`followup:true` 且置信度达到阈值，同时必须存在先前的会话指代：提交前的计划
（pre-commit plan）、`lastTask`，或之前的用户/助手消息。升级复用已有评估结果，
不增加额外 LLM 调用；它只恢复实际存在的状态：
`planProgress` 从已有的提交前的计划或启动时暂存的待恢复计划中恢复；只有 pre-commit task 存在时才
恢复 `lastTask`，只有恢复后的 `lastTask` 非空时才注入恢复上下文。即使只有历史对话，也可能
触发升级，但不会注入计划、任务锚点或恢复上下文，只避免把输入提交为新的任务快照。
tier-1 快速通道会跳过评估，也不使用该升级。

## 中断行为

- `esc` 会像中止其他回合阶段一样，终止正在运行的审查；已流出的部分分析保留在
  屏幕上，不记录判定。
- 被中断的*任务*保留计划进度；恢复后还原面板，继续提示（“continue”）会注入
  任务状态，让智能体继续而非重新开始。
- 审查失败或 UNKNOWN 绝不阻塞回合：会报告结果，本轮正常结束。

## 相关文档

- [智能体工作流](../concepts/agent-workflow.md)——审查在处理流程中的位置
- [模型、项目与会话](models-projects-and-sessions.md)——阶段模型
- [环境变量](../reference/environment-variables.md)——`HK2_ENABLE_PLANREVIEW`、`HK2_ENABLE_CODEREVIEW` 等
