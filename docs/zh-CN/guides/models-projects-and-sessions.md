...
本指南讲解构成一次 hk2 会话的三类数据：多提供商模型注册表
（`models.json`）、项目注册表（`projects.json`）与会话记录，以及阶段模型、
...
`anthropic`）、一个 base URL 和一个 API 密钥。**模型（model）**是提供商
...
2. **项目默认**——`/model set-default current <ref>`（覆盖该项目的全局
   默认；`--clear` 清除）
...
未设置时阶段使用会话模型。自动处理阶段与手动 `/review code` 命令的模型选择
行为不同：
...
模型提示来自 `ANTHROPIC_DEFAULT_*_MODEL` 与 `ANTHROPIC_MODEL`（重复项会去重；
...
- `--name` 在模型内唯一；再次添加同名服务器会替换原配置。
- `--options` 中的 `$APIKEY` 在**使用时**替换为该提供商的 `--api-key`——存储的
  配置只保留占位符，绝不保存密钥。
- 每个智能体回合都会在内置工具之后挂载 MCP 工具；无法访问的服务器会被跳过并发出警告。
...
- **`sourceRoot` / `extraRoots`**——项目的索引根目录。主源码根与所有命名
  额外根都会被 `/kb init` 遍历。
- **include/exclude globs——整体替换，不是追加。**传入任一项都会完整替换
  ...
  而 `/project set exclude vendor/**` 会静默移除默认的 `node_modules` /
...
### 共享 current 指针与会话固定绑定

`projects.json.current` 是共享注册表的默认项目指针；`/project list` 用 `*` 标记它，
`/project set current` 修改它并切换当前交互会话。`hk2 --project=<name>` 与
`--project-id=<id>` 只固定当前会话，不修改共享指针。因此，会话固定绑定可以不同于
`current`，多个进程可以同时使用不同的绑定。
