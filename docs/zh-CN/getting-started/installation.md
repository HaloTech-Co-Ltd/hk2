...
本页介绍安装 hk2 需要的全部内容：环境要求、两条安装路径（`install.sh` 与
...
`install.sh` 将当前本地源码检出复制到 `~/.hk2`，形成一份自包含目录，并在
PATH 中创建 `hk2` 符号链接（默认 `/usr/local/bin/hk2`——写入该位置可能
需要提权；普通用户可传 `--prefix="$HOME/.local"`），并运行 `npm install --omit=optional`
构建 Tree-sitter 原生绑定。脚本本身不会执行 git clone；请在完整检出中运行。
...
优先于新版本树中的同名条目）。安装器会保留可用于恢复的同级暂存与备份
...
数据也不会因刷新该目标而被触碰。源码树先复制到一个新建的同级暂存目录（目录
名每次运行唯一）；已有清单数据移入 `.hk2-preserve`，旧安装移入 `.hk2-old`。
只有新启动器验证成功且清单记录的每项都已存在后，恢复目录才会删除。后续运行
发现中断的数据保留事务时，会先尝试恢复已记录条目，再开始下一次升级。这是
文件系统层面的尽力恢复，不是跨目录原子事务；重要数据仍应保留外部备份。

`npm install` 执行失败只会打印警告并继续使用正则解析回退；如果启动器验证
成功，这条警告不会使安装器保留旧安装备份。暂存、代码树替换、数据恢复或
启动器验证失败时，会在适用情况下留下同级恢复状态，供以后运行继续处理。
...
内置的启发式提取器，尽力抽取可打印文本（两者都不是完整的 Office 渲染器——
...
`hk2 --help` 会打印版本、CLI 用法、各类斜杠命令与配置位置。能看到输出即说明
启动器与 Node 运行时均正常。
...
要删除**默认数据主目录**及其中的模型、项目、会话与知识库：
`rm -rf ~/.hk2`——先备份需要保留的内容。自定义 `HK2_HOME` / `HK2_KB_DIR`
路径中的数据和独立的源码安装目录不在这条命令的删除范围内。
