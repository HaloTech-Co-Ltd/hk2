...
本页介绍 hk2 的命令行界面（参数、一次性模式和互斥关系），以及各语言的解析支持级别。
CLI 的事实来源是 `src/cli.js`；语言相关事实来源是 `package.json` 与 `lib/parser/*`。
...
- `--project` 与 `--project-id` **互斥**——二选一。重名会被拒绝并提示改用
  `--project-id`。
- 项目选择会固定本次会话所用的项目（不重写全局共享的 `current` 指针，因此并行的
  `hk2 --project=<其他>` 进程不会切换本会话所用的项目）。
...
由 Tree-sitter 解析的符号*可以*填充更丰富的字段——限定名、父级链接、基类、
...
Tree-sitter 不可用时（未 `npm install`、ABI 不匹配、缺少语法），hk2 自动
