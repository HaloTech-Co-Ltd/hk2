...
| 变量 | 用途 | 默认值 | 说明 |
|---|---|---|---|
| `HK2_HOME` | 配置 / 数据主目录 | `~/.hk2` | 存放 models.json、projects.json、sessions/、logs/；仅当未设置 `HK2_KB_DIR` 时，知识库才位于 `HK2_HOME/kb/` |
| `HK2_KB_DIR` | 知识库根目录覆盖 | `$HK2_HOME/kb` | 注意：旧版 `--mode` 命令按默认 `$HK2_HOME/kb` 路径检测当前项目知识库——自定义 `HK2_KB_DIR` 时，该自动检测可能找不到真实 KB 而回退到 `default`；此时请显式设置 `HK2_KB_NAME` |
| `HK2_KB_NAME` | 旧版 `--mode` 命令使用的知识库名。解析顺序：先取非空的 `HK2_KB_NAME`；否则，若共享当前项目的知识库目录已有 `meta.json`，则使用其 UUID；否则使用字面量 `default` | 非空值 / 项目 UUID / `default` | |
| `HK2_PREFIX` | `install.sh` 放置符号链接的安装前缀 | `/usr/local` | 仅 install.sh |
| `HK2_INSTALL_DIR` | `install.sh` 自包含源码副本位置 | `~/.hk2` | 仅 install.sh |
| `HK2_PROJECT_SOURCE` | 工具沙箱使用的项目源码根目录 | - | 交互模式下自动设置 |
| `HK2_PROJECT_ID` | 用于定位项目级权限文件的项目 ID | - | 交互模式下自动设置；缺省时按 source 路径反查 `projects.json` |
...
| 变量 | 用途 | 默认值 | 说明 |
|---|---|---|---|
| `HK2_LLMAPI_TIMEOUT_MS` | 所有 LLM API 请求（流式与非流式）的默认超时（毫秒） | `3600000`（3600 秒） | 优先级：单次调用 `opts.timeoutMs` > 每模型 `config.timeout` > 本环境变量。当本环境变量实际生效时，显式 `0` = 不设超时（不启动中止定时器——计划 / 代码审查依赖此行为）。未设置 / 非法 / 负数回退默认值 |
| `HK2_LLMAPI_TIMEOUT_MS_SIMPLE` | 轻量单次 LLM 阶段的超时（毫秒）：查询改写与请求清晰度评估（轮次开始的两趟调用与 `kb_search` 的内联改写） | `300000`（300 秒） | 由 `lib/llm/timeout.js` 的 `llmApiTimeoutMsSimple()` 解析；单次调用传入 `opts.timeoutMs` 仍优先。当本环境变量实际生效时，显式 `0` = 不设超时。此前硬编码为 15000ms |
| `HK2_LLMAPI_NUMOFRETRIES` | 暂态 LLM 故障（网络错误、HTTP 408/429/5xx、请求超时）的最大连续重试次数；指数退避从 1s 起、封顶 30s；尝试间发出 `{type:'retry'}` 事件。确定性 4xx 与用户中止不重试。显式 `0` = 仅尝试一次 | `10` | |
| `HK2_LLM_RETRY_UNKNOWN_POST` | 结果不确定的失败（请求发出后的传输层错误——连接重置、读 / 写阶段超时，以及 HTTP 500/502/503/504；反向代理可能在上游已执行后才返回）默认重试；设 `0` 可显式关闭（担心重复请求 / 重复计费时使用——提供商不提供幂等键）。连接建立失败（拒绝连接、DNS 错误、连接超时；错误消息显示为 `(CODE)` 或 `(CODE/connect)`）以及 HTTP 408/429 属于结果安全类，始终重试。重试次数受 `HK2_LLMAPI_NUMOFRETRIES` 约束 | `1` | |

### 重试可见文本边界

每次重试只重启当前 LLM 调用。失败尝试的正文增量、推理缓冲区与尚未执行的
...
| 变量 | 用途 | 默认值 | 说明 |
|---|---|---|---|
| `HK2_STUCK_NUDGE_LIMIT` | 智能体循环卡死检测器的纠正预算：同一工具调用签名且结果完全相同的轮次连续出现 4 次时，注入递进式纠正性 system 提示（诊断根因 / 改为单次更长等待 / 停止等待直接作答）并重置重复窗口；仅当连续忽略这么多次纠正仍未跳出循环时才会中止。任何真实进展都会重新满额预算。由 `lib/agent/loop.js` 的 `stuckNudgeLimit()` 解析 | `10` | 显式 `0` 恢复旧的立即中止行为（首次触发即中止）。未设置 / 非法 / 负数回退默认值 |
...
| `HK2_KB_LEARN_VALIDATE` | `1`：学习条目在写入磁盘前对照现有知识库校验（预筛 + 一次语义判定）——重复跳过、相近合并、冲突裁决（Holy 交由用户）。尽力而为 | `1` | |

### 按入口解析检查点

不同入口处理检查点间隔的方式不同。下表列出各入口的有效值或执行频率：

| 入口 | 输入 | 有效间隔 |
|---|---|---|
| 交互式 `/kb init` 环境变量 | 未设置 / 空值 / `0` / 非法 | `100` |
| 交互式 `/kb init` 环境变量 | 正整数 `N` | `N` |
| 交互式 `/kb init` 环境变量 | 负数 | 负数；几乎每处理一个文件就保存一次 |
| 显式 flag | `--checkpoint-interval=0` | `0`；几乎每处理一个文件就保存一次 |
| 显式 flag | 非法值或裸 flag | `NaN`；几乎每处理一个文件就保存一次 |
| 显式 flag | `--checkpoint-interval=` | 回退到环境变量/默认值 |
...
| 变量 | 用途 | 默认值 | 说明 |
|---|---|---|---|
| `HK2_ENABLE_AUTOCOMPACT` | `1`（默认开启）：轮次开始时，若上下文使用率达到阈值则压缩。最近 4 条 user/assistant **消息**原样保留（不是 4 个完整轮次），更早的由 LLM 总结为一条 system 消息；朴素截断回退只拼接并按长度截断较早的消息，不做语义总结。阈值检查仅在轮次边界运行。另有任务中错误恢复：若提供商因上下文过长而拒绝请求，系统会立即压缩对话、注入恢复上下文并重新运行智能体循环（每轮最多两次）；其他错误不会走此路径。经 `/remember` 或 `remember` 工具显式保存的事实必然免受压缩；用于保护开头陈述事实的压缩时抽取与首尾摘要输入则属于**尽力而为** | `1` | |
...
| 变量 | 用途 | 默认值 | 说明 |
|---|---|---|---|
| `HK2_AUTOIMPORT_CLAUDE` | `0`、`no`、`false` 或 `off`（不区分大小写）禁用首次启动时从 Claude Code 的 `~/.claude/settings.json` 导入模型（仅 TUI）；其他值（包括空值 / 未设置）保持导入开启 | 开 | 仅填充缺失项；绝不覆盖已有默认 |
...
| 变量 | 用途 |
|---|---|
| `NO_COLOR` | 非空时禁用 ANSI 颜色（效果同 `HK2_NO_COLOR=1`）；空值 / 未设置无效果 |
...
## 从 Claude Code settings 首次导入时读取的环境变量

TUI 首次启动导入从 `~/.claude/settings.json` 的 `env` 对象读取以下键——
**不是** hk2 进程环境变量：
...
进程级 `ANTHROPIC_API_KEY` 通过另一条路径为注册表提供初始值（首次创建时）；
settings 文件中的同名键只供导入使用。

## 相关文档

- [智能体工作流](../concepts/agent-workflow.md)——各流程开关作用在哪里
