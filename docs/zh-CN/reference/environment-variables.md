...
每次重试只重启当前 LLM 调用。失败 attempt 的正文 delta、推理缓冲区与尚未
...
检查点间隔没有统一包装。下表列出各入口的有效值或 cadence：

| 入口 | 输入 | 有效间隔 |
|---|---|---|
| 交互式 `/kb init` 环境变量 | 未设置 / 空值 / `0` / 非法 | `100` |
| 交互式 `/kb init` 环境变量 | 正整数 `N` | `N` |
| 交互式 `/kb init` 环境变量 | 负数 | 负数；近乎每文件保存 |
| 显式 flag | `--checkpoint-interval=0` | `0`；近乎每文件保存 |
| 显式 flag | 非法值或裸 flag | `NaN`；近乎每文件保存 |
| 显式 flag | `--checkpoint-interval=` | 回退到环境变量/默认值 |
| 交互式禁用 | `--no-checkpoint` | 禁用 |
| 直接 `buildIndex()` 环境变量 | `0`、负数或非法 | 原样传入；近乎每文件保存 |

交互式 `/kb init` 对 `parseInt(value, 10)` 套用 `|| 100`；直接 indexer 调用不套用。
