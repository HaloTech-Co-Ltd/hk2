...
| `HK2_ENABLE_REQUEST_ASSESS` | `1`（且改写开启）：先由 LLM 判断请求是否清晰；不清晰的请求弹出编号澄清菜单，选定答案反馈回改写。结合会话摘要判断以免误判后续输入；仅交互式 TTY；一轮有界；尽力而为。判定字段记入会话记录的 `assess` 元数据 | `1` | |
...
| 变量 | 用途 | 默认值 | 说明 |
|---|---|---|---|
| `HK2_ENABLE_AUTOCOMPACT` | `1`（默认开启）：轮次开始时，若上下文使用率达到阈值则压缩。最近 4 轮 user/assistant 原样保留，更早的由 LLM 总结为一条 system 消息；失败回退朴素截断。仅在轮次边界触发，绝不中途。在对话被总结掉之前，用户陈述的持久事实会先被抽取进会话事实层（见 `/remember`），且摘要器输入同时保留对话的头部与尾部——开头陈述的事实逐字进入摘要，自动压缩不再丢失它们 | `1` | |
