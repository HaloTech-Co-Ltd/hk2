...
2. **范围相同**——章节、表格、警告与命令示例一一对应。任何一方都不能只做
   另一方的缩略版。
3. **语言切换**——`docs/en/` 或 `docs/zh-CN/` 下的每页都在 H1 正下方标注：英文页为
...
| 智能体工具 | `lib/agent/tools.js` 及相关智能体模块 |
...
根 README 是首页，不是手册。保留：定位、核心能力、环境要求、最简安装步骤、
简短的快速开始、一个示例与文档导航。MIT 许可证保留在根目录 `LICENSE`
文件中，README 不展示许可证章节。**不**保留：完整命令表、工具
表、环境变量表、配置 Schema、TUI 键位表、权限内部实现、源码目录树。改为
链接到详情页。如果某一节看起来像参考资料，就应放入 `docs/`。
...
- `README.md` 与 `README_zh.md` 具有相同的标题层级序列、表格数量 / 结构与
  代码围栏语言序列；
- 每对页面互相链接（语言切换）；
- 每个被检查页面恰好一个 H1，且标题层级不跳级；`docs/` 下的成对页面还
  要求语言切换链接位于 H1 之后的首个内容块（ATX
  标题允许最多 3 个前导空格；4 个空格视为代码缩进）；
- 每对 docs 页面具有相同的有序标题层级序列、表格数量与列/数据行结构，以及代码
  围栏语言序列；
- 两个根 README、`docs/` 下的成对 Markdown 页面及 `docs/README.md` 的
  本地 Markdown 链接与图片指向真实存在的文件；
- 每个代码围栏（反引号或波浪线、长度 ≥ 3 均可）都带语言标签，且没有未
  闭合的围栏；
- 质量门禁：文档任何位置（含代码示例的原始文本）都没有未填充的仓库地址
  占位符与未完成工作标记，两个根 README 互相链接并各自链接对应语言的
  文档索引，`docs/README.md` 同时链接两种语言的索引。

检查器校验文件目标，不检查章节锚点、外部 URL 或 HTML 图片地址。语言切换
链接的位置检查不适用于根 README 和语言选择页。

检查器强制检查路径、链接、围栏与结构同构，但无法证明两种语言的翻译正文
具有完全相同的事实含义；语义一致性仍需人工审校。

检查器会在以非零状态退出前报告全部问题——请全部修复列出的问题。
