...
- **CLI 层**——`bin/hk2`（单一可执行入口）引入 `src/cli.js`，后者解析
  ...
  列出项目、执行一次性 `--mode` 命令（`project-init`、`build-kb`、`update-kb`）、
  `--run-mode=serve`（旧版 REPL），以及默认的交互前端（TUI 或行式 REPL，
  含 TTY 能力检测与回退）。
- **交互前端**——`src/commands/interactive.js`（行式 REPL：readline、状态
  栏、工具卡片）与 `src/tui/*`（带边框输入框、流式 markdown、弹窗）。每个
  前端都会创建自己的会话对象，并与非渲染层共享该会话：斜杠分发、回合处理流程、
  会话记录。
- **斜杠命令层**——`src/slash/index.js` 注册命令表并分词（shell 风格引号）；
  ...
  `theme.js`）负责修改注册表。
- **回合处理流程**——`src/commands/turn.js`（`runTurn`）编排一条用户消息：门禁、
...
- **智能体核心**——`lib/agent/loop.js` 以缓存与卡死检测机制运行 LLM/工具
  ...
  `session_facts.js` 维护不受压缩影响的 `## Session facts` 常驻消息
...
4. 全部写入 `$HK2_KB_DIR/<projectId>/`（默认 `$HK2_HOME/kb/<projectId>/`；
   `store/*`）。
5. 分别尝试生成三个 LLM 摘要条目（`summarize.js`）；每次只有在 LLM 调用返回非空
...
索引、图谱、权限、斜杠命令、回合处理流程、TUI（部分经 PTY 运行器）——阅读陌生
代码时，它们是了解预期行为最快的途径。

## 相关文档

- [智能体工作流](../concepts/agent-workflow.md)——回合处理流程细节
- [知识图谱与检索](../concepts/knowledge-graph-and-retrieval.md)——索引流程
