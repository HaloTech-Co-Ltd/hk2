...
欢迎阅读 hk2 文档。hk2 是一个以知识库（KB）驱动、专为编码场景而生的智能体：每个项目拥有自己的知识库——符号、代码知识图谱与沉淀的知识条目——对实质性请求，hk2 会把相关 KB 上下文预取进提示词（明确的会话性后续输入改走快速通道），后续任务因此能在先前所学的基础上继续——前提是相关知识已保存并被检索命中。
